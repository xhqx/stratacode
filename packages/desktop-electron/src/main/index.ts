...
process.env.STRATA_DISABLE_EMBEDDED_WEB_UI = "true"
...
async function initialize() {
  ...
  const loadingTask = (async () => {
    ...
    serverReady.resolve({
      url,
      username: "strata", // stratacode_change
      password,
    })
...
async function getSidecarPort() {
  const fromEnv = process.env.STRATA_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "opencode", "strata.db"))
}
