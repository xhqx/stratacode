...
const CLI_INSTALL_DIR: &str = ".strata/bin";
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "STRATA_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "STRATA_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("STRATA_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                ...
                format!(
                    "  curl -fsSL https://strata.ai/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "STRATA_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "STRATA_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "STRATA_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "STRATA_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "STRATA_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "STRATA_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("STRATA_SERVER_USERNAME", "opencode".to_string()),
        ("STRATA_SERVER_PASSWORD", password.to_string()),
    ];
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("STRATA_CLIENT"), Some(&"desktop".to_string()));
    }
