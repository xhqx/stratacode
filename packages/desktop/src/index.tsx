...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__STRATA__ ??= {}
  const pending = window.__STRATA__.deepLinks ?? []
  window.__STRATA__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__STRATA__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__STRATA__?.wsl) return result
  ...
  return {
    ...
    getWslEnabled: async () => {
      ...
      return window.__STRATA__!.wsl ?? false
    },
