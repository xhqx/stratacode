...
import { ConfigProtection } from "../../../src/stratacode/permission/config-paths"
...
describe("ConfigProtection.isRequest", () => {
  ...
  const legacy = StratacodePaths.globalDirs().map((d) => path.resolve(d))
  ...
  test("returns false for file-tool external_directory targeting global config", () => {
    const result = ConfigProtection.isRequest({
      permission: "external_directory",
      patterns: [config + "/*"],
      metadata: { filepath: path.join(config, "strata.json"), parentDir: config },
    })
  ...
  test("returns true for edit targeting relative config path via patterns", () => {
    const result = ConfigProtection.isRequest({
      permission: "edit",
      patterns: [".strata/command/foo.md"],
    })
    expect(result).toBe(true)
  })
  ...
  test("returns true for edit targeting root config files", () => {
    for (const file of ["strata.json", "strata.jsonc", "AGENTS.md"]) {
      const result = ConfigProtection.isRequest({
        permission: "edit",
        patterns: [file],
      })
