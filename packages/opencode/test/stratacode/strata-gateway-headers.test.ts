...
import { buildStrataHeaders, getFeatureHeader, getEditorNameHeader } from "@stratacode/strata-gateway"
import { HEADER_FEATURE, ENV_FEATURE, ENV_VERSION, DEFAULT_EDITOR_NAME } from "@stratacode/strata-gateway"
...
describe("getEditorNameHeader", () => {
  ...
  it("returns default editor name without version when STRATACODE_VERSION is not set", () => {
    delete process.env[ENV_VERSION]
    expect(getEditorNameHeader()).toBe(DEFAULT_EDITOR_NAME)
  })

  it("appends version when STRATACODE_VERSION is set", () => {
    process.env[ENV_VERSION] = "1.2.3"
    expect(getEditorNameHeader()).toBe(`${DEFAULT_EDITOR_NAME} 1.2.3`)
  })
})

describe("buildStrataHeaders", () => {
  const original = process.env[ENV_FEATURE]
  ...
  it("includes feature header when env var is set", () => {
    ...
    const headers = buildStrataHeaders()
  ...
  it("always includes editor name header", () => {
    delete process.env[ENV_FEATURE]
    const headers = buildStrataHeaders()
    expect(headers["X-STRATACODE-EDITORNAME"]).toBe(getEditorNameHeader())
  })
