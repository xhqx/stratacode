...
process.env.STRATA_SESSION_RETRY_LIMIT = "2"
...
afterEach(() => {
  delete process.env.STRATA_SESSION_RETRY_LIMIT
})
...
describe("session processor retry limit", () => {
  ...
  it.effect("only positive integers enable the limit", () =>
    Effect.promise(async () => {
      ...
      delete process.env.STRATA_SESSION_RETRY_LIMIT
      expect(Flag.STRATA_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.STRATA_SESSION_RETRY_LIMIT = "0"
      expect(Flag.STRATA_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.STRATA_SESSION_RETRY_LIMIT = "-1"
      expect(Flag.STRATA_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.STRATA_SESSION_RETRY_LIMIT = "abc"
      expect(Flag.STRATA_SESSION_RETRY_LIMIT).toBeUndefined()

      process.env.STRATA_SESSION_RETRY_LIMIT = "2"
      expect(Flag.STRATA_SESSION_RETRY_LIMIT).toBe(2)
    }),
  )
  ...
  it.effect("reads env at access time (dynamic getter)", () =>
    Effect.promise(async () => {
      ...
      delete process.env.STRATA_SESSION_RETRY_LIMIT
      expect(Flag.STRATA_SESSION_RETRY_LIMIT).toBeUndefined()
      process.env.STRATA_SESSION_RETRY_LIMIT = "5"
      expect(Flag.STRATA_SESSION_RETRY_LIMIT).toBe(5)
      delete process.env.STRATA_SESSION_RETRY_LIMIT
    }),
  )
})
