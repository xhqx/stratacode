...
import { StratacodeConfigInjector } from "../../src/stratacode/config-injector"
import { tmpdir } from "../fixture/fixture"
import path from "path"
import fs from "fs/promises"

describe("StratacodeConfigInjector", () => {
  describe("buildConfig", () => {
    test("returns empty config when no modes exist", async () => {
      await using tmp = await tmpdir()

      const result = await StratacodeConfigInjector.buildConfig({ projectDir: tmp.path, skipGlobalPaths: true })
  ...
  - slug: code
    ...
    test("includes workflows as commands in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".stratacode", "workflows")
          await Bun.write(
    ...
    test("includes rules in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".stratacode", "rules", "main.md"), "# Rules")
        },
      })

      const result = await StratacodeConfigInjector.buildConfig({
        projectDir: tmp.path,
  ...
  - slug: translate
    ...
    test("includes ignore patterns in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".stratacodeignore"), "secrets/\n*.env")
        },
      })
  ...
  - slug: translate
    ...
    test("handles negation patterns in ignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".stratacodeignore"), "*.env\n!.env.example")
        },
      })
  ...
  describe("getEnvVars", () => {
    test("returns empty object for empty config", () => {
      const envVars = StratacodeConfigInjector.getEnvVars("{}")
    ...
    test("returns STRATA_CONFIG_CONTENT for non-empty config", () => {
      const config = JSON.stringify({ agent: { test: {} } })
      const envVars = StratacodeConfigInjector.getEnvVars(config)

      expect(envVars).toEqual({
        STRATA_CONFIG_CONTENT: config,
      })
    })
  })
})
