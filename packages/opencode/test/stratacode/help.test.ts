...
import { generateHelp, generateCommandTable } from "../../src/stratacode/help"
...
import { ProvidersCommand } from "../../src/cli/cmd/providers" // stratacode_change — upstream renamed auth → providers
...
import { HelpCommand } from "../../src/stratacode/help-command"
...
const AttachStub = {
  command: "attach <url>",
  describe: "attach to a running strata server",
  handler() {},
}
...
describe("strata help --all (markdown)", () => {
  test("contains ## heading for each known top-level command", async () => {
    ...
    for (const cmd of ["run", "auth", "debug", "mcp", "session", "agent"]) {
      expect(output).toContain(`## strata ${cmd}`)
    }
  })

  test("contains headings for nested subcommands", async () => {
    const output = await generateHelp({ all: true, format: "md", commands })
    expect(output).toContain("strata auth login")
    expect(output).toContain("strata auth logout")
    expect(output).toContain("strata debug config")
  })
})

describe("strata help --all (text)", () => {
...
describe("strata help <command>", () => {
  test("strata help auth contains auth subcommand headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).toContain("strata auth login")
    expect(output).toContain("strata auth logout")
    expect(output).toContain("strata auth list")
  })

  test("strata help auth does NOT contain run or debug headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).not.toContain("## strata run")
    expect(output).not.toContain("## strata debug")
  })
})
...
describe("edge cases", () => {
  ...
  test("strata help nonexistent throws unknown command error", async () => {
...
describe("generateCommandTable", () => {
  ...
  test("contains rows for known commands", async () => {
    ...
    for (const name of ["run", "auth", "debug", "mcp"]) {
      expect(output).toContain(`strata ${name}`)
    }
  })

  test("default command appears as strata [project], not $0", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`strata [project]`")
  ...
  test("skips commands with no describe", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).not.toContain("`strata generate`")
  })

  test("contains strata completion row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`strata completion`")
  })

  test("contains strata help row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`strata help")
  })
})
...
describe("commands.ts stays in sync with index.ts", () => {
  test("every .command() in index.ts has an entry in the commands array", async () => {
    ...
    const barrel = await Bun.file(path.resolve(import.meta.dir, "../../src/stratacode/commands.ts")).text()
