...
import { McpMigrator } from "../../src/stratacode/mcp-migrator"
...
describe("McpMigrator", () => {
  describe("convertServer", () => {
    test("converts local server with command and args", () => {
      const server: McpMigrator.StratacodeMcpServer = {
        command: "npx",
  ...
  describe("migrate", () => {
    ...
    test("migrates servers from project .strata/mcp.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".strata")
          await Bun.write(
    ...
    test("reads from legacy .stratacode/mcp.json when .strata/mcp.json is absent", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".stratacode")
          await Bun.write(
            path.join(settingsDir, "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    // Regression: malformed .stratacode/mcp.json must not prevent .strata/mcp.json from loading
    test("loads .strata/mcp.json even when .stratacode/mcp.json is malformed", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".stratacode", "mcp.json"), "{ corrupt json !!!")
          await Bun.write(
            path.join(dir, ".strata", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test(".strata/mcp.json overrides .stratacode/mcp.json for same server name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".stratacode", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test("does not read project-level .strata/mcp_settings.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".strata")
          await Bun.write(
