...
import type { RemoteWS } from "../../../src/strata-sessions/remote-ws"
import type { RemoteProtocol } from "../../../src/strata-sessions/remote-protocol"
import { SessionPrompt } from "../../../src/session/prompt"
import { Question } from "../../../src/question"
import { Permission } from "../../../src/permission"
import { Suggestion } from "../../../src/stratacode/suggestion" // stratacode_change
...
describe("RemoteSender", () => {
  ...
  test("send_message normalizes string model without prefix", async () => {
    ...
    expect(prompt).toHaveBeenCalledWith({
      sessionID: "ses_x",
      parts: [{ type: "text", text: "hello" }],
      model: { providerID: "strata", modelID: "anthropic/claude-sonnet-4-20250514" },
    })
  })

  test("send_message keeps stratacode-prefixed model unchanged before internal conversion", async () => {
    const { conn } = fakeConn()
    const prompt = spyOn(SessionPrompt, "prompt").mockResolvedValue({} as never)
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      type: "command",
      id: "req_model_stratacode",
      command: "send_message",
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "stratacode/gpt-5-mini",
      },
    })
    ...
    expect(prompt).toHaveBeenCalledWith({
      sessionID: "ses_x",
      parts: [{ type: "text", text: "hello" }],
      model: { providerID: "strata", modelID: "gpt-5-mini" },
    })
  })
  ...
  test("send_message rejects structured model on remote path", () => {
    ...
    sender.handle({
      ...
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: "stratacode", modelID: "gpt-5-mini" },
      },
    })
  ...
  test("send_message does not special-case strata-prefixed model", async () => {
    const { conn, sent } = fakeConn()
    const prompt = spyOn(SessionPrompt, "prompt").mockResolvedValue({} as never)
    const sender = RemoteSender.create({
      conn,
    ...
    sender.handle({
      ...
      data: {
        sessionID: "ses_x",
        parts: [{ type: "text", text: "hello" }],
        model: "strata/gpt-5-mini",
      },
    })

    await new Promise((r) => setTimeout(r, 0))

    expect(sent[0]).toEqual({ type: "response", id: "req_model_strata", result: {} })
    expect(prompt).toHaveBeenCalledWith({
      sessionID: "ses_x",
      parts: [{ type: "text", text: "hello" }],
      model: { providerID: "strata", modelID: "strata/gpt-5-mini" },
    })
  })
  // stratacode_change end
