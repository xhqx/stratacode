...
import { StrataSession } from "../../src/stratacode/session"
import { StrataSessionPrompt } from "../../src/stratacode/session/prompt"
    ...
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<StrataSession.CloseReason>()
          const unsub = yield* bus.subscribeCallback(StrataSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(StrataSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("StrataSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = StrataSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: StrataSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = StrataSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: StrataSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
