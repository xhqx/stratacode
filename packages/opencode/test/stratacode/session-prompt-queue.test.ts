...
import { StrataSessionPromptQueue } from "../../src/stratacode/session/prompt-queue"
import { Suggestion } from "../../src/stratacode/suggestion"
...
describe("session prompt queue", () => {
  test("scopes queued turns without moving prior assistant history", async () => {
    ...
    const ids = await Effect.runPromise(
      StrataSessionPromptQueue.enqueue(
        sessionID,
        two,
        Effect.sync(() => StrataSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)),
        Effect.succeed([]),
      ),
    )
  ...
  test("retarget keeps older queued prompts hidden", async () => {
    ...
    const ids = await Effect.runPromise(
      StrataSessionPromptQueue.enqueue(
        ...
        base,
        Effect.sync(() => {
          StrataSessionPromptQueue.retarget(sessionID, injected)
          return StrataSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)
        }),
        Effect.succeed([]),
      ),
    )
  ...
  test("hasFollowup reports true only for prompts enqueued after the active slot started", async () => {
    ...
    const first = Effect.runPromise(
      StrataSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "first:start", value: StrataSessionPromptQueue.hasFollowup(sessionID) })
          firstStarted.resolve()
          yield* Effect.promise(() => firstReleased.promise)
          observed.push({ where: "first:end", value: StrataSessionPromptQueue.hasFollowup(sessionID) })
          return "first"
        }),
    ...
    const second = Effect.runPromise(
      StrataSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "second:start", value: StrataSessionPromptQueue.hasFollowup(sessionID) })
    ...
    expect(StrataSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
    ...
    const third = Effect.runPromise(
      StrataSessionPromptQueue.enqueue(
        ...
        Effect.sync(() => {
          observed.push({ where: "third:start", value: StrataSessionPromptQueue.hasFollowup(sessionID) })
          return "third"
        }),
    ...
    expect(StrataSessionPromptQueue.hasFollowup(sessionID)).toBe(false)
        ...
        fn: async () => {
          ...
          const ids = await Effect.runPromise(
            StrataSessionPromptQueue.enqueue(
              ...
              Effect.succeed(StrataSessionPromptQueue.scope(session.id, []).map((item) => item.info.id)),
              Effect.succeed([]),
            ),
          )
          expect(ids).toEqual([])
          expect(StrataSessionPromptQueue.hasFollowup(session.id)).toBe(false)
        },
      })
    } finally {
      server.stop(true)
