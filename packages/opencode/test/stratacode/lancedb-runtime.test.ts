...
const entry = "file:///tmp/strata-cache/node_modules/@lancedb/lancedb/dist/index.js"
const add = mock(async () => ({ directory: "/tmp/strata-cache", entrypoint: entry }))
...
const env = "STRATA_LANCEDB_PATH"
const prev = process.env[env]

describe("LanceDBRuntime", () => {
  beforeEach(async () => {
    const { LanceDBRuntime } = await import("../../src/stratacode/lancedb")
    LanceDBRuntime.clear()
    add.mockClear()
    add.mockImplementation(async () => ({ directory: "/tmp/strata-cache", entrypoint: entry }))
  })
  ...
  test("retries after a failed install", async () => {
    ...
    add
      ...
      .mockImplementationOnce(async () => ({ directory: "/tmp/strata-cache", entrypoint: entry }))
