    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".strata", "agent", "skip.md"),
          `---
mode: "banana"
---
Broken agent prompt`,
        ...
        await Filesystem.write(
          path.join(dir, ".strata", "agent", "keep.md"),
          `---
model: test/model
---
Valid agent prompt`,
...
Broken agent prompt`,
  ...
  test("skips invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".strata", "command", "skip.md"),
          `---
subtask: "banana"
---
Broken command template`,
        ...
        await Filesystem.write(
          path.join(dir, ".strata", "command", "keep.md"),
          `---
...
Broken command template`,
  ...
  test("collects warnings for invalid agent markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".strata", "agent", "broken.md"),
          `---
mode: "banana"
---
Broken agent`,
  ...
  test("collects warnings for invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".strata", "command", "broken.md"),
          `---
subtask: "banana"
---
Broken command`,
  ...
  test("collects warnings for invalid JSON in .strata directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".strata", "strata.json"), "{ not valid json !!!")
      },
    })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(warns.some((w) => w.path.includes("strata.json") && w.message.includes("not valid JSON"))).toBe(true)
      },
    })
  })

  test("collects warnings for invalid schema in .strata directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".strata", "strata.json"), JSON.stringify({ unknownField: true }))
      },
    })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(cfg).toBeDefined()
        expect(warns.some((w) => w.path.includes("strata.json") && w.message.includes("invalid"))).toBe(true)
      },
    })
  })
