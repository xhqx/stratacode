...
import { formatTodos, generateHandover, PlanFollowup, PlanFollowupRuntime } from "../../src/stratacode/plan-followup"
...
describe("plan follow-up", () => {
  ...
  test("ask - hides custom answer row on VS Code where the main prompt input handles typed replies", () =>
    withInstance(async () => {
      const prev = process.env.STRATA_CLIENT
      try {
        process.env.STRATA_CLIENT = "vscode"
        const seeded = await seed({ text: "1. Build" })
        const pending = PlanFollowup.ask({
          sessionID: seeded.sessionID,
          messages: seeded.messages,
        ...
        process.env.STRATA_CLIENT = prev
      }
    }))
  ...
  test("ask - retargets prompt queue so injected message is visible in scope", () =>
    withInstance(async () => {
      const { StrataSessionPromptQueue } = await import("../../src/stratacode/session/prompt-queue")
      ...
      StrataSessionPromptQueue.retarget(seeded.sessionID, original)
      ...
      const scoped = StrataSessionPromptQueue.scope(seeded.sessionID, all)
