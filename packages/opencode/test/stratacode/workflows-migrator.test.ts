...
import { WorkflowsMigrator } from "../../src/stratacode/workflows-migrator"
...
Actual description here.`
  ...
  describe("discoverWorkflows", () => {
    test("discovers project workflows", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".strata", "workflows")
    ...
    test("discovers workflows from legacy .stratacode/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("discovers global workflows from ~/.strata/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".strata", "workflows", "global.md"), "# Global\n\nGlobal workflow")
      ...
      expect(
        workflows.some((w) => w.source === "global" && w.path.includes(path.join(".strata", "workflows", "global.md"))),
      ).toBe(true)
    })
  })
  ...
  describe("convertToCommand", () => {
    test("converts workflow to command format", () => {
      const workflow: WorkflowsMigrator.StratacodeWorkflow = {
        name: "code-review",
    ...
    test("project workflows override global workflows with same name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const projectDir = path.join(dir, ".strata", "workflows")
