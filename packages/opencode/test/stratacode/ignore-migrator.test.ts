...
import { IgnoreMigrator } from "../../src/stratacode/ignore-migrator"
...
# Comment 2`
  ...
  *.env  `
  ...
  describe("migrate", () => {
    test("returns empty permission for project without .stratacodeignore", async () => {
    ...
    test("loads project .stratacodeignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
    ...
    test("handles complex .stratacodeignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".stratacodeignore"),
            `# Secrets
secrets/
*.env
!.env.example
...
`,
  ...
  describe("loadIgnoreConfig", () => {
    test("returns empty object for project without .stratacodeignore", async () => {
      await using tmp = await tmpdir()
    ...
    test("returns permission config for project with .stratacodeignore", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".stratacodeignore"), "secrets/")
        },
