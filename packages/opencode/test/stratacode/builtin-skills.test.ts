...
import { BUILTIN_SKILLS } from "../../src/stratacode/skills/builtin"
...
test("built-in skill has correct metadata", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const skill = await Skill.get("strata-config")
      expect(skill).toBeDefined()
      expect(skill!.name).toBe("strata-config")
      expect(skill!.location).toBe(Skill.BUILTIN_LOCATION)
      expect(skill!.content).toContain("strata")
    },
  })
})
...
test("user skill overrides built-in with same name", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".strata", "skill", "strata-config")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: strata-config
description: User override of strata-config.
---

# Custom strata-config
...
`,
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const skill = await Skill.get("strata-config")
      expect(skill).toBeDefined()
      expect(skill!.description).toBe("User override of strata-config.")
      expect(skill!.location).not.toBe(Skill.BUILTIN_LOCATION)
      expect(skill!.location).toContain(path.join("skill", "strata-config", "SKILL.md"))
    },
  })
})
