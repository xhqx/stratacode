import { RemoteProtocol } from "@/strata-sessions/remote-protocol"
import type { RemoteWS } from "@/strata-sessions/remote-ws"
import { GlobalBus } from "@/bus/global"
// stratacode_change - AppRuntime is imported lazily inside dispatch() below to break a
// module init cycle: Worktree → project/bootstrap → strata-sessions → remote-sender →
...
// stratacode_change start — lazy init to avoid circular dependency
...
function normalizeModel(model: string | undefined) {
  ...
  return {
    providerID: ProviderID.make("strata"),
    modelID: ModelID.make(model.startsWith("stratacode/") ? model.slice("stratacode/".length) : model),
  }
}
