...
import { StrataSession } from "@/stratacode/session"
...
import { IngestQueue } from "@/strata-sessions/ingest-queue"
import { clearInFlightCache, withInFlightCache } from "@/strata-sessions/inflight-cache"
import type * as SDK from "@stratacode/sdk/v2"
import z from "zod"
import { STRATA_API_BASE } from "@stratacode/strata-gateway"
import { Config } from "@/config"
import { Instance } from "@/project/instance"
import { Vcs } from "@/project"
import simpleGit from "simple-git"
import { RemoteWS } from "@/strata-sessions/remote-ws"
import { RemoteSender } from "@/strata-sessions/remote-sender"
...
export namespace StrataSessions {
  export const Event = {
    RemoteStatusChanged: BusEvent.define(
      "strata-sessions.remote-status-changed",
      z.object({
        enabled: z.boolean(),
        connected: z.boolean(),
      }),
  ...
  const log = Log.create({ service: "strata-sessions" })

  const Uuid = z.uuid()
  type Uuid = z.infer<typeof Uuid>

  const tokenValidKeyTemplate = "strata-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"

  const tokenKey = "strata-sessions:token"
  const orgKey = "strata-sessions:org"
  const clientKey = "strata-sessions:client"
  const gitUrlKeyPrefix = "strata-sessions:git-url:"
  ...
  async function authValid(token: string) {
    ...
    return withInFlightCache(tokenValidKey, 15 * 60_000, async () => {
      const response = await fetch(`${STRATA_API_BASE}/api/user`, {
        headers: {
  ...
  async function stratacodeToken() {
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      const token = await stratacodeToken()
  ...
  const shareDisabled = process.env["STRATA_DISABLE_SHARE"] === "true" || process.env["STRATA_DISABLE_SHARE"] === "1"
  const ingestDisabled =
    process.env["STRATA_DISABLE_SESSION_INGEST"] === "true" || process.env["STRATA_DISABLE_SESSION_INGEST"] === "1"
  const debugIngest =
    process.env["STRATA_DEBUG_SESSION_INGEST"] === "true" || process.env["STRATA_DEBUG_SESSION_INGEST"] === "1"
  ...
  const remoteEnabled = process.env["STRATA_REMOTE"] === "1"
  ...
  export async function init() {
    ...
    Bus.subscribe(Session.Event.Updated, async (evt) => {
      const sessionID = evt.properties.sessionID // stratacode_change
      const session = await Session.get(sessionID).catch(() => null) // stratacode_change
      if (!session) return
  ...
  export async function enableRemote() {
    ...
    enabling = (async () => {
      const token = await stratacodeToken()
      if (!token) {
        throw new Error("Unable to enable remote: no Strata credentials found. Run `strata auth login`.")
      }

      const valid = await authValid(token)
      if (valid === false) {
        throw new Error("Unable to enable remote: invalid or expired Strata credentials. Run `strata auth login`.")
      }
      if (valid === undefined) throw new Error("Unable to enable remote: failed to verify Strata credentials.")

      const url = (process.env["STRATA_SESSION_INGEST_URL"] ?? "https://ingest.stratasessions.ai")
        .replace(/^https:\/\//, "wss://")
        .replace(/^http:\/\//, "ws://")
  ...
  export async function share(sessionId: string) {
    if (ingestDisabled) {
      throw new Error("Session ingest is disabled (STRATA_DISABLE_SESSION_INGEST=1)")
    }

    if (shareDisabled) {
      throw new Error("Sharing is disabled (STRATA_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to share session: no Strata credentials found. Run `strata auth login`.")
    }
    ...
    const url = `https://app.strata.ai/s/${result.public_id}`
  ...
  export async function unshare(sessionId: string) {
    ...
    if (shareDisabled) {
      throw new Error("Unshare is disabled (STRATA_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to unshare session: no Strata credentials found. Run `strata auth login`.")
    }
  ...
  async function meta(sessionId?: string) {
    const override = sessionId ? StrataSession.getPlatformOverride(sessionId) : undefined
    const platform = override || process.env["STRATA_PLATFORM"] || "cli"
  ...
  async function getOrgId(): Promise<Uuid | undefined> {
    const env = process.env["STRATA_ORG_ID"]
