...
//
// Strata-specific provider logic extracted from packages/opencode/src/provider/provider.ts
...
// calls at well-defined injection points (each marked with stratacode_change).

import { createStrata, type StrataProvider, AI_SDK_PROVIDERS, PROMPTS } from "@stratacode/strata-gateway"
...
export const STRATA_BUNDLED_PROVIDERS: Record<string, () => Promise<(options: any) => BundledSDK>> = {
  "@stratacode/strata-gateway": async () => createStrata as unknown as (options: any) => BundledSDK,
}
...
export const STRATA_MODEL_SCHEMA_EXTENSIONS = {
...
// ---------------------------------------------------------------------------
// fromModelsDevModel patch — returns strata-specific fields
// ---------------------------------------------------------------------------

export function patchModelsDevModel(providerID: string, source: any) {
  return {
    variants: providerID === "strata" ? (source.variants ?? {}) : {},
...
// ---------------------------------------------------------------------------
// Config model patch — merges strata-specific fields from config + existing
// ---------------------------------------------------------------------------
...
export function strataCustomLoaders(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    strata: Effect.fnUntraced(function* (input: any) {
      ...
      const hasKey = yield* Effect.gen(function* () {
        ...
        if ((yield* dep.config()).provider?.["strata"]?.options?.apiKey) return true
        return false
      })
      ...
      return {
        ...
        options,
        async getModel(sdk: StrataProvider, modelID: string) {
...
export function patchCustomLoaderResult(
  ...
  switch (providerID) {
    ...
    case "cerebras":
      result.options.headers = {
        ...result.options.headers,
        "X-Cerebras-3rd-Party-Integration": "strata",
      }
      break
    case "azure": {
    ...
    // in provider.ts with single-line stratacode_change markers
  }
}
...
export function strataSmallModelPriority(providerID: string): string[] | undefined {
  if (providerID.startsWith("strata")) return ["strata-auto/small"]
  return undefined
}
