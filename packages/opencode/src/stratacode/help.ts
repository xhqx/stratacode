...
async function getHelpText(name: string, cmd: Cmd): Promise<string> {
  const inst = yargs([])
    .scriptName(name ? `strata ${name}` : "strata")
    .wrap(null)
  if (cmd.builder) {
...
async function getSubcommands(
  ...
  const inst = yargs([]).scriptName(`strata ${name}`).wrap(null)
  builder(inst)
  ...
  try {
    ...
    for (const [sub, handler] of Object.entries(handlers as Record<string, any>)) {
      ...
      const full = `${name} ${sub}`
      const subInst = yargs([]).scriptName(`strata ${full}`).wrap(null)
...
function formatMarkdown(
  ...
  for (const section of sections) {
    parts.push(`## ${section.name ? `strata ${section.name}` : "strata"}`)
    parts.push("")
    if (section.hidden) {
...
function formatText(
  ...
  for (const section of sections) {
    parts.push(rule)
    const display = section.name ? `strata ${section.name}` : "strata"
    ...
    for (const sub of section.subs) {
      const sublabel = sub.hidden ? `--- strata ${sub.name} [internal] ---` : `--- strata ${sub.name} ---`
      parts.push(sublabel)
      parts.push("")
      parts.push(sub.help)
      parts.push("")
...
export async function generateCommandTable(options?: { commands?: Cmd[] }) {
  ...
  for (const cmd of cmds) {
    ...
    const display = raw.startsWith("$0") ? "strata" + raw.slice(2) : "strata " + raw
