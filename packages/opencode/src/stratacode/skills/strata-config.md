# Strata CLI Configuration Reference

All config lives in `strata.json` (or `strata.jsonc`). Precedence low-to-high: remote well-known, global (`~/.config/strata/strata.json`), env `STRATA_CONFIG`, project `./strata.json`, `.strata/strata.json`, `STRATA_CONFIG_CONTENT`, managed (see Config File Locations). Deep-merged; later wins.

This also covers where Strata looks for config files, commands, agents, and skills across project, global, and legacy paths such as `.strata/`, `.stratacode/`, `.opencode/`, and `~/.config/strata/`.

## Commands (`.strata/command/*.md`)

Markdown files with YAML frontmatter. The filename (minus `.md`) becomes the command name invoked via `/name`. Commands can live in `.strata/`, `.stratacode/`, `.opencode/`, and global config roots, with both `command/` and `commands/` directory names supported. See Config File Locations for the full search order.
...
3. `~/.stratacode/`
4. `~/.opencode/`
5. The `STRATA_CONFIG_DIR` directory (if the env var is set)
6. project `.strata/`, `.stratacode/`, and `.opencode/` directories from the current working directory up to the worktree root
...
## Agents (`.strata/agent/*.md`)

Also loaded from `.stratacode/` and `.opencode/` directories (legacy), and plural `agents/` variants.
...
Markdown files in `.strata/workflows/` or `.stratacode/workflows/` (project-level) and `~/.strata/workflows/` or `~/.stratacode/workflows/` (global). These are automatically converted to commands at startup. The filename (minus `.md`) becomes the command name. Project workflows override global ones with the same name.
...
For example, this configuration will hide all models from the built-in Strata Gateway as well as any from the OpenAI provider which may be enabled automatically through environment variables.

```jsonc
{
  "$schema": "https://app.strata.ai/config.json",
  "disabled_providers": ["strata", "openai"],
}
```

The provider ID is the lowercase name used in the `provider/model` format (e.g., `strata`, `openai`, `anthropic`, `google`, `groq`).
...
Skills are markdown files at `skills/<name>/SKILL.md` (or `skill/<name>/SKILL.md`) with `name` and `description` in frontmatter. Discovered inside `.strata/`, `.stratacode/`, and `.opencode/` directories.
...
Three directory names are scanned: `.strata` (modern), `.stratacode` (legacy), `.opencode` (legacy). All three are checked at each level:

- **Project**: walks up from CWD to the git worktree root, checking for all three at each directory level
- **Home**: `~/.strata/`, `~/.stratacode/`, `~/.opencode/`
- **XDG global**: `~/.config/strata/` (always loaded, lowest file-based precedence)
...
Example: `~/.config/strata/command/*.md` (modern global), `~/.stratacode/command/*.md` (legacy global), `.opencode/commands/*.md` (legacy project) all load commands.
...
| `STRATA_DISABLE_PROJECT_CONFIG` | Skip all project-level config (files and directories)            |
