/**
 * Custom OAuth handler for Strata Gateway
 *
...
import { DialogStrataOrganization } from "./dialog-strata-organization.js"
...
interface StrataAutoMethodProps {
  index: number
  providerID: string
  title: string
...
export function StrataAutoMethod(props: StrataAutoMethodProps) {
  ...
  onMount(async () => {
    try {
      ...
      const profileResponse = await sdk.client.strata.profile()
      ...
      console.warn("Failed to fetch Strata profile, using personal account:", error)
      setStatus("error")
