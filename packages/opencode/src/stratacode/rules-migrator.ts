...
export namespace RulesMigrator {
  // Only support .stratacoderules (no migration for .roorules or .clinerules)
  const LEGACY_RULE_FILE = ".stratacoderules"
  const home = () => process.env.STRATA_TEST_HOME || process.env.HOME || process.env.USERPROFILE || os.homedir()

  // Directory-based rules (read from both .strata and .stratacode)
  const STRATA_RULES_DIRS = [".strata/rules", ".stratacode/rules"]
  const globalRulesDirs = () => [path.join(home(), ".strata", "rules"), path.join(home(), ".stratacode", "rules")]
  ...
  export async function discoverRules(projectDir: string): Promise<RuleFile[]> {
    ...
    // 1. Global rules directories (~/.strata/rules/*.md and ~/.stratacode/rules/*.md)
    ...
    // 2. Project .strata/rules/ and .stratacode/rules/ directories
    const seen = new Set<string>()
    for (const rulesRel of STRATA_RULES_DIRS) {
    ...
    // 3. Legacy .stratacoderules file (only stratacode, not roo/cline)
    ...
    for (const mode of KNOWN_MODES) {
      // Mode-specific directories (.strata/rules-{mode}/*.md and .stratacode/rules-{mode}/*.md)
      const modeSeen = new Set<string>()
      for (const prefix of [".strata", ".stratacode"]) {
      ...
      // Legacy mode-specific file (.stratacoderules-{mode})
      const legacyModeFile = path.join(projectDir, `.stratacoderules-${mode}`)
  ...
  export async function migrate(options: {
    ...
    for (const rule of rules) {
      ...
      if (rule.source === "legacy") {
        warnings.push(
          `Legacy rule file '${path.basename(rule.path)}' found. Consider migrating to .strata/rules/ directory.`,
        )
      }
    }
