# Stratacode Rules Migration

This document explains how Stratacode rules are automatically migrated to Opencode's `instructions` config array.

## Overview

Stratacode stores rules in various file locations. When Opencode starts, it reads these files and injects their paths into the `instructions` config array, which Opencode then loads as part of the system prompt.
...
If you have existing opencode config with `instructions`, the Stratacode rules are **combined**, not replaced:

```typescript
// Example: User has opencode.json with:
{ "instructions": ["AGENTS.md", "custom-rules.md"] }

// Stratacode rules add:
{ "instructions": [".stratacoderules", ".stratacode/rules/coding.md"] }

// Result (combined, deduplicated):
{ "instructions": ["AGENTS.md", "custom-rules.md", ".stratacoderules", ".stratacode/rules/coding.md"] }
```

### 3. Restart to Pick Up Changes

If you change your Stratacode configuration (e.g., edit `.stratacoderules`), simply restart strata-cli to pick up the new config. No manual migration or conversion needed.
...
| `.stratacode/rules-{mode}/*.md` | `instructions: [".stratacode/rules-{mode}/file.md", ...]` |
| `~/.stratacode/rules/*.md`      | `instructions: ["~/.stratacode/rules/file.md", ...]`      |
...
Only Stratacode-specific files (`.stratacoderules`, `.stratacode/rules/`) are migrated.

## Mode-Specific Rules

Mode-specific rules (e.g., `.stratacoderules-code`, `.stratacode/rules-architect/`) are included by default. All mode-specific rules are loaded regardless of the current mode.

## Warnings

The migrator generates warnings for:

- **Legacy files**: When `.stratacoderules` is found, a warning suggests migrating to `.stratacode/rules/` directory structure
...
```
project/
├── .stratacoderules           # Legacy rules
├── .stratacoderules-code      # Code-mode specific
└── .stratacode/
    └── rules/
...
{
  "instructions": [
    "/path/to/project/.stratacode/rules/coding.md",
    "/path/to/project/.stratacode/rules/testing.md",
    "/path/to/project/.stratacoderules",
    "/path/to/project/.stratacoderules-code"
  ]
}
```
...
3. Restart strata-cli to pick up changes
