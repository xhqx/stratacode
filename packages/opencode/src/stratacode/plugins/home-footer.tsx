...
/**
 * Strata-specific home footer plugin.
 *
 ...
 */
import type { TuiPlugin, TuiPluginApi, TuiPluginModule } from "@stratacode/plugin/tui"
...
const id = "internal:strata-home-footer"

// ---------------------------------------------------------------------------
// RemoteIndicator – adapted from @/stratacode/remote-tui for plugin API usage
// ---------------------------------------------------------------------------

function RemoteIndicator(props: { api: TuiPluginApi; strata: boolean }) {
  ...
  return (
    <Show when={props.strata && status()?.enabled}>
...
// ---------------------------------------------------------------------------
// Sub-components (mirror upstream home/footer with strata additions)
// ---------------------------------------------------------------------------
...
function View(props: { api: TuiPluginApi }) {
  const strata = createMemo(() => props.api.state.provider.some((p) => p.id === "strata"))
  const theme = () => props.api.theme.current
  const sync = useSync()
  ...
  return (
    ...
    >
      ...
      <box gap={1} flexDirection="row" flexShrink={0}>
        <RemoteIndicator api={props.api} strata={strata()} />
        <Mcp api={props.api} />
