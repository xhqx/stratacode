...
/**
 * StrataClaw SolidJS reactive helpers
 *
...
/**
 * Poll the StrataClaw instance status every `interval` ms.
...
export function createClawStatus(sdk: any, interval = 10_000) {
  ...
  onMount(() => {
    const poll = async () => {
      const res = await sdk.client.strata.claw.status().catch(() => null)
...
export function createClawChat(sdk: any) {
  ...
  onMount(async () => {
    ...
    const res = await sdk.client.strata.claw.chatCredentials().catch((e: any) => {
