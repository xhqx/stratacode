...
const MARKER_START = "# >>> stratadev launcher >>>"
const MARKER_END = "# <<< stratadev launcher <<<"

export const DevSetupCommand = cmd({
  command: "dev-setup",
  describe: "install a `stratadev` shell alias for this checkout",
  builder: (y) =>
    y
      .option("shell", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    UI.empty()
    UI.println(`  ${B}Strata CLI dev launcher setup${N}`)
    UI.empty()
    ...
    UI.println(
      `  ${action === "install" ? "This will add" : "This will replace"} one line so ${H}stratadev${N} runs this checkout from any directory:`,
    )
    UI.empty()
    UI.println(`      ${snippet}`)
    UI.empty()
...
// Deprecated: kept so `strata dev-alias` keeps working for scripts/docs.
...
function aliasLine(shell: Shell, repo: string): string {
  const sh = path.join(repo, "bin", "stratadev")
  const bat = path.join(repo, "bin", "stratadev.cmd")
  const q = (s: string) => `'${s.replaceAll("'", `'\\''`)}'`
  if (shell === "fish") return `alias stratadev ${q(sh)}`
  if (shell === "powershell") return `function stratadev { & '${bat.replaceAll("'", "''")}' @args }`
  return `alias stratadev=${q(sh)}`
}
...
async function applyBlock(rc: string, snippet: string): Promise<{ created: boolean; backup?: string }> {
  ...
  const backup = `${rc}.stratadev.bak.${stamp()}`
...
export async function detectRepo(): Promise<string> {
  const hint = process.env.STRATA_DEV_REPO
  if (hint) return hint
  ...
  // For a local build it's <repo>/packages/opencode/dist/<target>/bin/strata, so
  ...
  throw new Error(
    "cannot locate stratacode source checkout; set STRATA_DEV_REPO=/path/to/stratacode or run ./bin/stratadev dev-setup from the repo",
  )
}
