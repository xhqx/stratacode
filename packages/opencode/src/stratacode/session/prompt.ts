...
import { PlanFollowup } from "@/stratacode/plan-followup"
import { StrataSession } from "@/stratacode/session"
import { environmentDetails, type EditorContext } from "@/stratacode/editor-context"
import { Identifier } from "@/id/id"
import { Filesystem } from "@/util"
import PROMPT_PLAN from "@/session/prompt/plan.txt"
import CODE_SWITCH from "@/session/prompt/code-switch.txt"

export namespace StrataSessionPrompt {
  /**
  ...
  export function shouldAskPlanFollowup(input: { messages: MessageV2.WithParts[]; abort: AbortSignal }) {
    ...
    if (!["cli", "vscode"].includes(Flag.STRATA_CLIENT)) return false
  ...
  export function resolveCloseReason(input: {
    sessionID: string
    closeReasons: Map<string, StrataSession.CloseReason>
    exit: Exit.Exit<any, any>
  }): StrataSession.CloseReason {
