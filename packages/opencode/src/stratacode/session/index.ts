...
export namespace StrataSession {
  const log = Log.create({ service: "session.strata" })
  ...
  // ---------------------------------------------------------------------------
  // Provider-reported cost (OpenRouter / Strata)
  // ---------------------------------------------------------------------------

  /**
   * Extract provider-reported cost from OpenRouter metadata when available.
   * For the Strata provider (BYOK), prefers `upstreamInferenceCost` over the
  ...
  export function providerCost(input: {
    ...
    const isStrata = (input.provider?.id ?? input.providerID) === "strata"
    const upstream = openrouterUsage.costDetails?.upstreamInferenceCost
    const regular = openrouterUsage.cost

    // Strata is always BYOK, so prefer upstream cost. For OpenRouter, use regular cost.
    const cost = isStrata && upstream !== undefined ? upstream : regular
  ...
  export async function shareSession(id: string): Promise<{ url: string }> {
    ...
    return StrataSessions.share(id)
  }

  export async function unshareSession(id: string): Promise<void> {
    const { StrataSessions } = await import("@/strata-sessions/strata-sessions")
    await StrataSessions.unshare(id)
  }

  export async function removeSession(id: string): Promise<void> {
    const { StrataSessions } = await import("@/strata-sessions/strata-sessions")
    await StrataSessions.remove(id).catch(() => {})
  }
...
export const strataSessionFork = fn(
  ...
  async (input) => {
    ...
    await StrataSession.remapChildren(session.id)
    return session
  },
)
