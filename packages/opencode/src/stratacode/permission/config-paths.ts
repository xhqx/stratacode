...
import { StratacodePaths } from "@/stratacode/paths"

export namespace ConfigProtection {
  /**
   * Config directory prefixes (relative paths, forward-slash normalized).
   * Matches .strata/, .stratacode/, .opencode/ at any depth within the project.
   */
  const CONFIG_DIRS = [".strata/", ".stratacode/", ".opencode/"]
   ...
   */
  const CONFIG_ROOT_FILES = new Set(["strata.json", "strata.jsonc", "opencode.json", "opencode.jsonc", "AGENTS.md"])
  ...
  export function isRelative(pattern: string): boolean {
    ...
    for (const dir of CONFIG_DIRS) {
      const bare = dir.slice(0, -1) // e.g. ".strata"
      // Match at root (e.g. ".strata/foo") or nested (e.g. "packages/sub/.strata/foo")
  ...
  export function isAbsolute(filepath: string): boolean {
    ...
    // ~/.config/strata/ (XDG config)
    if (within(resolved, path.resolve(Global.Path.config))) return true

    // ~/.strata/ and ~/.stratacode/ (legacy global dirs)
    for (const dir of StratacodePaths.globalDirs()) {
