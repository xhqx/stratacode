...
import { StrataSessionPromptQueue } from "@/stratacode/session/prompt-queue"

/**
 * Strata-specific helpers for the shared `@/question` module.
 *
 ...
 */
export namespace StrataQuestion {
  ...
  export const guardFollowup = <E>(sessionID: SessionID, makeError: () => E) =>
    Effect.gen(function* () {
      if (!StrataSessionPromptQueue.hasFollowup(sessionID)) return
