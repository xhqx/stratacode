...
export namespace McpMigrator {
  const log = Log.create({ service: "stratacode.mcp-migrator" })

  // Remote transport types used by the Stratacode extension
  const REMOTE_TYPES = new Set(["streamable-http", "sse"])

  function isRemote(server: StratacodeMcpServer): boolean {
    return !!server.type && REMOTE_TYPES.has(server.type)
  }

  // Stratacode MCP server structure
  export interface StratacodeMcpServer {
    command?: string
    args?: string[]
  ...
  export interface StratacodeMcpSettings {
    mcpServers: Record<string, StratacodeMcpServer>
  }
  ...
  export async function readMcpSettings(filepath: string): Promise<StratacodeMcpSettings | null> {
    if (!(await Filesystem.exists(filepath))) return null

    try {
      const content = await fs.readFile(filepath, "utf-8")
      return JSON.parse(content) as StratacodeMcpSettings
    } catch (err) {
  ...
  export function convertServer(name: string, server: StratacodeMcpServer): ConfigMCP.Info | null {
    if (isRemote(server)) {
      if (!server.url) {
  ...
  export async function migrate(options?: {
    ...
    const allServers: Array<{ name: string; server: StratacodeMcpServer }> = []

    if (!options?.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global MCP settings)
      const vscodeSettingsPath = path.join(StratacodePaths.vscodeGlobalStorage(), "settings", "mcp_settings.json")
    ...
    // Check .strata/mcp.json and .stratacode/mcp.json for project-level settings
    // (not "mcp_settings.json" which is only used for global settings)
    // .stratacode is loaded first (lower precedence), .strata second (higher precedence)
    if (options?.projectDir) {
      for (const dir of [".stratacode", ".strata"]) {
    ...
    const serversByName = new Map<string, StratacodeMcpServer>()
  ...
  /**
   * Load Stratacode MCP servers and return them as an opencode config partial.
  ...
  export async function loadMcpConfig(
    ...
    try {
      ...
      if (Object.keys(result.mcp).length > 0) {
        log.debug("loaded stratacode MCP servers", {
      ...
      for (const skipped of result.skipped) {
        log.debug("skipped stratacode MCP server", { name: skipped.name, reason: skipped.reason })
      }

      for (const warning of result.warnings) {
        log.warn("stratacode MCP migration warning", { warning })
      }

      return result.mcp
    } catch (err) {
      log.warn("failed to load stratacode MCP servers", { error: err })
      return {}
    }
  }
}
