...
export namespace IgnoreMigrator {
  const log = Log.create({ service: "stratacode.ignore-migrator" })

  const STRATACODEIGNORE_FILE = ".stratacodeignore"
  const GLOBAL_STRATACODEIGNORE = path.join(os.homedir(), ".stratacode", STRATACODEIGNORE_FILE)
  ...
  /**
   * Parse .stratacodeignore content into patterns.
  ...
  /**
   * Load patterns from a .stratacodeignore file
   */
  ...
  /**
   * Migrate .stratacodeignore to Opencode permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .stratacodeignore (lower priority)
    if (!options.skipGlobalPaths) {
      const globalPatterns = await loadIgnoreFile(GLOBAL_STRATACODEIGNORE, "global")
      allPatterns.push(...globalPatterns)

      if (globalPatterns.length > 0) {
        log.debug("loaded global .stratacodeignore", { count: globalPatterns.length })
      }
    }

    // 2. Load project .stratacodeignore (higher priority - added last)
    const projectIgnorePath = path.join(options.projectDir, STRATACODEIGNORE_FILE)
    const projectPatterns = await loadIgnoreFile(projectIgnorePath, "project")
    allPatterns.push(...projectPatterns)

    if (projectPatterns.length > 0) {
      log.debug("loaded project .stratacodeignore", { count: projectPatterns.length })
    }
  ...
  /**
   * Load .stratacodeignore and return permission config.
  ...
  export async function loadIgnoreConfig(
    ...
    try {
      ...
      if (result.patternCount > 0) {
        log.info("loaded .stratacodeignore patterns", {
      ...
      log.warn("failed to load .stratacodeignore", { error: err })
      return {}
    }
  }
}
