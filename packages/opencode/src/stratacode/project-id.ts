...
/**
 * Read project ID from .strata/config.json, falling back to .stratacode/config.json
...
async function getProjectIdFromConfig(directory: string): Promise<string | undefined> {
  ...
  for (const dir of [".strata", ".stratacode"]) {
...
/**
 * Resolve project ID with priority: .strata/config.json -> .stratacode/config.json -> git origin URL
 * @returns Normalized project ID or undefined
 */
async function resolveProjectId(): Promise<string | undefined> {
  const dir = Instance.directory

  // Priority 1: .strata/config.json (falls back to .stratacode/config.json)
...
export namespace StrataProjectID {
  export interface Interface {
    readonly get: () => Effect.Effect<string | undefined>
  }

  export class Service extends Context.Service<Service, Interface>()("@stratacode/StrataProjectID") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const state = yield* InstanceState.make(
        Effect.fn("StrataProjectID.state")(function* () {
...
const { runPromise } = makeRuntime(StrataProjectID.Service, StrataProjectID.defaultLayer)

/**
 * Get the project ID for the current Instance context (cached per-project)
 * @returns Normalized project ID or undefined
 */
export async function getStrataProjectId(): Promise<string | undefined> {
