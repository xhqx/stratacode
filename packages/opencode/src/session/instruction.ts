...
const FILES = [
  "AGENTS.md",
  ...(Flag.STRATA_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.STRATA_CONFIG_DIR) {
    files.push(path.join(Flag.STRATA_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.STRATA_DISABLE_CLAUDE_CODE_PROMPT) {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const relative = Effect.fnUntraced(function* (instruction: string) {
        ...
        if (!Flag.STRATA_CONFIG_DIR) {
          log.warn(
            `Skipping relative instruction "${instruction}" - no STRATA_CONFIG_DIR set while project config is disabled`,
          )
          return []
        }
        return yield* fs
          .globUp(instruction, Flag.STRATA_CONFIG_DIR, Flag.STRATA_CONFIG_DIR)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
      })
