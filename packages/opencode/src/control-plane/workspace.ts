...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    STRATA_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    STRATA_WORKSPACE_ID: config.id,
    STRATA_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.STRATA_EXPERIMENTAL_WORKSPACES) return
