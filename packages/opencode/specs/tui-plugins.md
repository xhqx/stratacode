...
- Author package entrypoint is `@stratacode/plugin/tui`.
...
- Import types from `@stratacode/plugin/tui`.
- `@stratacode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `STRATA_CONFIG_DIR`, OpenCode installs `@stratacode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@stratacode/plugin/tui`.
...
- `--pure` / `STRATA_PURE` skips external TUI plugins only.
