...
import { getStrataProfile, getStrataDefaultModel, promptOrganizationSelection } from "../api/profile.js"
import { STRATA_API_BASE, POLL_INTERVAL_MS } from "../api/constants.js"
...
async function initiateDeviceAuth(): Promise<DeviceAuthInitiateResponse> {
  const response = await fetch(`${STRATA_API_BASE}/api/device-auth/codes`, {
    method: "POST",
    headers: {
...
async function pollDeviceAuth(code: string): Promise<DeviceAuthPollResponse> {
  const response = await fetch(`${STRATA_API_BASE}/api/device-auth/codes/${code}`)
...
export async function authenticateWithDeviceAuth(): Promise<DeviceAuthResult> {
  ...
  const profileData = await getStrataProfile(token)
  ...
  const model = await getStrataDefaultModel(token, organizationId)
