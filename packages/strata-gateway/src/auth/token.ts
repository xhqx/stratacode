/**
 * Parse StrataCode URL from token
 * Some tokens contain encoded base URL information
 */
export function getStrataUrlFromToken(defaultUrl: string, token: string): string {
...
/**
 * Validate StrataCode token format
 */
export function isValidStratacodeToken(token: string): boolean {
 ...
 */
export function getApiKey(options: { stratacodeToken?: string; apiKey?: string }): string | undefined {
  return options.stratacodeToken ?? options.apiKey
}
