...
export async function strataCustomLoader(provider: ProviderInfo): Promise<CustomLoaderResult> {
  ...
  console.log(`[strata-provider] Loaded ${initialCount} models, hasAuth: ${hasKey}`)
  ...
  if (!hasKey) {
    ...
    console.log(
      `[strata-provider] Filtered to ${freeCount} free models (removed ${initialCount - freeCount} paid models)`,
    )
  }

  const autoload = Object.keys(provider.models).length > 0
  console.log(`[strata-provider] Autoload: ${autoload}`)
...
async function checkAuthentication(provider: ProviderInfo): Promise<boolean> {
  ...
  if (provider.options?.apiKey || provider.options?.stratacodeToken) {
    return true
  }
