import { z } from "zod"
import { STRATA_API_BASE, MODELS_FETCH_TIMEOUT_MS } from "./constants.js"
...
/**
 * Fetch custom modes for an organization from the Strata Cloud API.
 *
...
export async function fetchOrganizationModes(token: string, organizationId: string): Promise<OrganizationMode[]> {
  ...
  try {
    const url = `${STRATA_API_BASE}/api/organizations/${encodeURIComponent(organizationId)}/modes`
    ...
    if (!response.ok) {
      console.warn(`[Strata Gateway] Failed to fetch organization modes: ${response.status}`)
      return []
    }
    ...
    if (!parsed.success) {
      console.warn("[Strata Gateway] Organization modes response validation failed:", parsed.error.format())
      return []
    }
    ...
    console.warn("[Strata Gateway] Error fetching organization modes:", err)
    return []
  }
}
