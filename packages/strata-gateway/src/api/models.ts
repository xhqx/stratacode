...
/**
 * Fetch models from Strata API (OpenRouter-compatible endpoint)
 *
 * @param options - Configuration options
 * @returns Record of models in ModelsDev.Model format
 */
export async function fetchStrataModels(options?: {
  ...
  const finalBaseURL = token ? getStrataUrlFromToken(baseURL, token) : baseURL
  ...
  try {
    ...
    if (!result.success) {
      console.error("Strata models response validation failed:", result.error.format())
      return {}
    }
    ...
    for (const model of result.data.data) {
      ...
      // Skip models that don't support tools — Strata requires tool calling
    ...
    console.error("Error fetching Strata models:", error)
    return {}
  }
}
