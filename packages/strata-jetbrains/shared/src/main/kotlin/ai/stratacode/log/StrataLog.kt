...
/**
 * Logging interface for the Strata JetBrains plugin.
 *
 ...
 * system property), output is additionally written to a `strata-dev.log` file inside the sandbox log directory.
 ...
 * Usage:
 * ```kotlin
 * private val log = StrataLog.create(this::class.java)
 *
 ...
 * ```
 *
 * The log level for the sandbox file can be controlled via the `strata.dev.log.level` system property
...
interface StrataLog {
    ...
    companion object {
        ...
        fun create(cls: Class<*>): StrataLog {
...
internal class IntellijLog(cls: Class<*>) : StrataLog {
...
internal class FileLog(cls: Class<*>) : StrataLog {
    ...
    companion object {
        ...
        private val root: java.util.logging.Logger by lazy {
            val logger = java.util.logging.Logger.getLogger("ai.stratacode")
        ...
        private val handler: FileHandler by lazy {
            ...
            val path = dir.resolve("strata-dev.log")
            val h = FileHandler(path.toString(), true)
            h.formatter = StrataFormatter()
            h
        }
        ...
        private fun resolveLogDir(): Path {
            ...
            while (current.parent != null) {
                ...
                if (name.startsWith("log_run")) {
                    side = if (name.lowercase().contains("frontend")) "strata-frontend" else "strata-backend"
                }
                if (name == "strata.jetbrains" && side != null) {
        ...
        private fun resolveLevel(): Level {
            val prop = System.getProperty("strata.dev.log.level") ?: return Level.INFO
...
internal class StrataFormatter : Formatter() {
    ...
    override fun format(record: LogRecord): String {
        ...
        val category = record.sourceClassName ?: record.loggerName ?: "strata.dev"
...
internal class CompositeLog(vararg val delegates: StrataLog) : StrataLog {
    override val isDebugEnabled: Boolean
