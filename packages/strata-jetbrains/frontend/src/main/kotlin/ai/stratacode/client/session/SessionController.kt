...
import ai.stratacode.client.session.model.AgentItem
import ai.stratacode.client.session.model.ModelItem
...
import ai.stratacode.client.session.model.Permission
import ai.stratacode.client.session.model.PermissionMeta
import ai.stratacode.client.session.model.PermissionRequestState
...
import ai.stratacode.client.session.model.ToolCallRef
...
class SessionController(
    parent: Disposable,
    id: String?,
    private val sessions: StrataSessionService,
    private val workspace: Workspace,
    private val app: StrataAppService,
    ...
    companion object {
        private val LOG = StrataLog.create(SessionController::class.java)
    }
    ...
    fun prompt(text: String) {
        ...
        cs.launch {
            try {
                ...
                edt {
                    val msg = e.message ?: StrataBundle.message("session.error.prompt")
    ...
    init {
        ...
        cs.launch {
            app.state.collect { state ->
                if (state.status == StrataAppStatusDto.READY) app.fetchVersionAsync()
        ...
        cs.launch {
            workspace.state.collect { state ->
                fire(SessionControllerEvent.WorkspaceChanged) {
                    ...
                    if (state.status != StrataWorkspaceStatusDto.READY) return@fire
                ...
                if (state.status == StrataWorkspaceStatusDto.READY) {
                    fire(SessionControllerEvent.WorkspaceReady)
                }
            }
        }
    ...
    private fun seedStatus(dto: SessionStatusDto) {
        ...
        val state = when (dto.type) {
            "busy" -> SessionState.Busy(StrataBundle.message("session.status.considering"))
    ...
    private fun handle(event: ChatEventDto) {
        ...
        when (event) {
            ...
            is ChatEventDto.TurnOpen -> {
                partType = null
                tool = null
                model.setState(SessionState.Busy(StrataBundle.message("session.status.considering")))
            }
            ...
            is ChatEventDto.Error -> {
                partType = null
                tool = null
                val msg = event.error?.message ?: event.error?.type ?: StrataBundle.message("session.error.unknown")
                ...
                val state = when (event.status.type) {
                    ...
                    "busy" -> {
                        ...
                        if (current is SessionState.Idle || current is SessionState.Error)
                            SessionState.Busy(StrataBundle.message("session.status.considering"))
    ...
    private fun status(): String = when (partType) {
        "reasoning" -> StrataBundle.message("session.status.thinking")
        "text" -> StrataBundle.message("session.status.writing")
        "tool" -> when (tool) {
            "task" -> StrataBundle.message("session.status.delegating")
            "todowrite", "todoread" -> StrataBundle.message("session.status.planning")
            "read" -> StrataBundle.message("session.status.gathering")
            "glob", "grep", "list" -> StrataBundle.message("session.status.searching.codebase")
            "webfetch", "websearch", "codesearch" -> StrataBundle.message("session.status.searching.web")
            "edit", "write" -> StrataBundle.message("session.status.editing")
            "bash" -> StrataBundle.message("session.status.commands")
            else -> StrataBundle.message("session.status.considering")
        }
        else -> StrataBundle.message("session.status.considering")
    }
