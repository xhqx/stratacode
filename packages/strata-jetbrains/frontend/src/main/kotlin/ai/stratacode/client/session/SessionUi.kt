...
import ai.stratacode.client.session.ui.LabelPicker
import ai.stratacode.client.session.ui.PermissionPanel
import ai.stratacode.client.session.ui.PromptPanel
import ai.stratacode.client.session.ui.QuestionPanel
import ai.stratacode.client.session.ui.SessionPanel
import ai.stratacode.client.session.ui.StatusPanel
...
class SessionUi(
    ...
    companion object {
        ...
        private val LOG = StrataLog.create(SessionUi::class.java)
    }

    private val flushMs = Registry.intValue("strata.session.flushMs", EVENT_FLUSH_MS.toInt())
        .takeIf { it > 0 }
        ?.toLong()
        ?: EVENT_FLUSH_MS
    ...
    private val controller = SessionController(
        ...
        flushMs = flushMs,
        condense = Registry.`is`("strata.session.condense", true),
    )
