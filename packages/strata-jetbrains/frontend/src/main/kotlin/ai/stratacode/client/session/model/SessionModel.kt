package ai.stratacode.client.session.model

import ai.stratacode.rpc.dto.DiffFileDto
...
import ai.stratacode.rpc.dto.TodoDto
 ...
 *
 * **EDT-only access** — no synchronization. [ai.stratacode.client.session.SessionController] guarantees all
...
class SessionModel {
    ...
    var app: StrataAppStateDto = StrataAppStateDto(StrataAppStatusDto.DISCONNECTED)
    var version: String? = null

    var workspace: StrataWorkspaceStateDto = StrataWorkspaceStateDto(StrataWorkspaceStatusDto.PENDING)
    ...
    fun isReady(): Boolean = app.status == StrataAppStatusDto.READY && workspace.status == StrataWorkspaceStatusDto.READY
