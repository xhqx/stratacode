...
class SessionModelTest : UsefulTestCase() {
    ...
    fun `test initial app and workspace state`() {
        assertEquals(StrataAppStatusDto.DISCONNECTED, model.app.status)
        assertEquals(StrataWorkspaceStatusDto.PENDING, model.workspace.status)
        assertFalse(model.isReady())
        assertEquals(SessionState.Idle, model.state)
    }

    fun `test isReady requires app and workspace readiness`() {
        model.app = StrataAppStateDto(StrataAppStatusDto.READY)
        assertFalse(model.isReady())

        model.workspace = StrataWorkspaceStateDto(StrataWorkspaceStatusDto.READY)
