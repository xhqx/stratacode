...
import ai.stratacode.rpc.dto.QuestionInfoDto
...
class SessionRecoveryTest : SessionControllerTestBase() {
    ...
    fun `test pending permission is recovered on history load`() {
        ...
        appRpc.state.value = ai.stratacode.rpc.dto.StrataAppStateDto(ai.stratacode.rpc.dto.StrataAppStatusDto.READY)
    ...
    // ------ Status seeding from StrataSessionService.statuses ------
    ...
    fun `test busy status is seeded from statuses map`() {
        ...
        assertSession(
            """
            [code] [strata/gpt-5] [busy] [considering next steps]
            """,
            m, show = false,
        )
    }
    ...
    fun `test retry status is seeded with message attempt and next`() {
        ...
        assertSession(
            """
            [code] [strata/gpt-5] [retry] [Rate limited]
            """,
            m, show = false,
        )
    ...
    fun `test offline status is seeded with message and requestId`() {
        ...
        assertSession(
            """
            [code] [strata/gpt-5] [offline] [No network]
            """,
            m, show = false,
        )
    ...
    fun `test pending permission overrides a seeded busy status`() {
        ...
        assertSession(
            ...
            [code] [strata/gpt-5] [awaiting-permission]
            """,
            m, show = false,
        )
    }
    ...
    fun `test pending question overrides a seeded retry status`() {
        ...
        assertSession(
            ...
            [code] [strata/gpt-5] [awaiting-question]
            """,
            m, show = false,
        )
    }
