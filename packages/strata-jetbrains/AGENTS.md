# AGENTS.md — Strata JetBrains Plugin

## Architecture (Split Mode)

- **Split-mode plugin** with three Gradle modules: `shared/`, `frontend/`, `backend/`. The module descriptors are `strata.jetbrains.shared.xml`, `strata.jetbrains.frontend.xml`, `strata.jetbrains.backend.xml` — these must stay in sync with `plugin.xml`'s `<content>` block.
...
- Kotlin source goes under `{module}/src/main/kotlin/ai/stratacode/jetbrains/`. Package name is `ai.stratacode.jetbrains` (matches `group` in root `build.gradle.kts`).
...
- Non-light services that need XML registration go in `strata.jetbrains.backend.xml` under `<extensions defaultExtensionNs="com.intellij"><applicationService>` (or `<projectService>`).
...
- The plugin spawns `strata serve --port 0` (OS assigns random port) and reads stdout for `listening on http://...:(\d+)` to discover the port.
- A random 32-byte hex password is passed via `STRATA_SERVER_PASSWORD` env var for Basic Auth.
- Key env vars: `STRATA_CLIENT=jetbrains`, `STRATA_PLATFORM=jetbrains`, `STRATA_APP_NAME=strata-code`, `STRATA_ENABLE_QUESTION_TOOL=true`.
- This is the same protocol used by the VS Code extension (`packages/strata-vscode/src/services/cli-backend/server-manager.ts`).

## Build

- **Full build**: `bun run build` from `packages/strata-jetbrains/` (builds CLI + Gradle plugin).
- **Gradle only**: `./gradlew buildPlugin` from `packages/strata-jetbrains/` (requires CLI binaries already present).
- **Via Turbo**: `bun turbo build --filter=@stratacode/strata-jetbrains` from repo root.
- **Run in sandbox**: `./gradlew runIde` — launches sandboxed IntelliJ with the plugin. Does NOT build CLI binaries.

## Files That Must Change Together

- `plugin.xml` `<content>` entries ↔ module XML descriptors (`strata.jetbrains.{shared,frontend,backend}.xml`)
...
`frontend/src/main/kotlin/ai/stratacode/client/session/`.
...
- After history load, `recoverPending()` seeds state in this priority order: (1) pending permission, (2) pending question, (3) current session status (`busy`/`retry`/`offline` from `StrataSessionService.statuses`), (4) `Idle`.
...
**Do not use Kotlin Compose or `intellij.platform.compose` in this plugin.** The JetBrains modular template uses Compose for its demo tool window, but Strata should use standard Swing with IntelliJ Platform components only. Keep all plugin UI in the existing Swing-based stack.
...
- Register declaratively in module XML via `com.intellij.toolWindow` extension point (already done in `strata.jetbrains.frontend.xml`).
...
- Declare in module XML: `<notificationGroup id="Strata Code" displayType="BALLOON"/>`.
- Show: `Notification("Strata Code", "message", NotificationType.INFORMATION).notify(project)`.
...
- Tool-window-bound: `displayType="TOOL_WINDOW" toolWindowId="Strata Code"`.
