...
openApiGenerate {
    ...
    packageName.set("ai.stratacode.jetbrains.api")
    apiPackage.set("ai.stratacode.jetbrains.api.client")
    modelPackage.set("ai.stratacode.jetbrains.api.model")
...
val localCli by tasks.registering(PrepareLocalCliTask::class) {
    ...
    exe.set(platform.map { if (it.startsWith("windows")) "strata.exe" else "strata" })
}
