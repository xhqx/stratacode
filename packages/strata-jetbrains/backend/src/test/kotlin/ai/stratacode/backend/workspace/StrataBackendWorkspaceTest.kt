...
import ai.stratacode.backend.workspace.StrataBackendWorkspace
import ai.stratacode.backend.workspace.StrataWorkspaceState
import ai.stratacode.backend.testing.FakeCliServer
import ai.stratacode.backend.testing.MockCliServer
import ai.stratacode.backend.testing.TestLog
...
class StrataBackendWorkspaceTest {
    ...
    private fun setup(): StrataBackendAppService =
        StrataBackendAppService.create(scope, FakeCliServer(mock), log)

    private suspend fun ready(app: StrataBackendAppService): StrataBackendWorkspace {
        app.connect()
        withTimeout(10_000) {
            app.appState.first { it is StrataAppState.Ready }
        }
    ...
    fun `get creates workspace on demand after Ready`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is StrataWorkspaceState.Ready }
        }

        val state = ws.state.value as StrataWorkspaceState.Ready
    ...
    fun `same directory returns same workspace instance`() = runBlocking {
        val app = setup()
        app.connect()
        withTimeout(10_000) { app.appState.first { it is StrataAppState.Ready } }
    ...
    fun `workspaces stopped on app disconnect`() = runBlocking {
        ...
        assertIs<StrataWorkspaceState.Pending>(ws.state.value)
    ...
    fun `workspace reaches Ready after creation`() = runBlocking {
        ...
        assertIs<StrataWorkspaceState.Ready>(ws.state.value)
    }
    ...
    fun `providers failure retries then transitions to Error`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is StrataWorkspaceState.Error }
        }

        val err = ws.state.value as StrataWorkspaceState.Error
    ...
    fun `concurrent load calls on workspace produce valid final state`() = runBlocking {
        ...
        withTimeout(15_000) {
            while (true) {
                ...
                if (state is StrataWorkspaceState.Ready) {
                    delay(300)
                    if (ws.state.value is StrataWorkspaceState.Ready) break
                }
                delay(100)
            }
        }
        ...
        withTimeout(15_000) {
            ...
            while (true) {
                ...
                if (state is StrataAppState.Ready) {
                    delay(300)
                    if (app.appState.value is StrataAppState.Ready) break
                }
                delay(100)
            }
        }
