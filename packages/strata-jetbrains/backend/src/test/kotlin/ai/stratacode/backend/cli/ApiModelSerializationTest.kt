...
import ai.stratacode.jetbrains.api.infrastructure.Serializer
import ai.stratacode.jetbrains.api.model.Config
import ai.stratacode.jetbrains.api.model.GlobalHealth200Response
import ai.stratacode.jetbrains.api.model.StrataNotifications200ResponseInner
import ai.stratacode.jetbrains.api.model.StrataNotifications200ResponseInnerAction
import ai.stratacode.jetbrains.api.model.StrataProfile200Response
import ai.stratacode.jetbrains.api.model.StrataProfile200ResponseBalance
import ai.stratacode.jetbrains.api.model.StrataProfile200ResponseProfile
import ai.stratacode.jetbrains.api.model.StrataProfile200ResponseProfileOrganizationsInner
...
class ApiModelSerializationTest {
    ...
    @Test
    fun `StrataNotifications200ResponseInner with action`() {
        val src = """{
            "id": "notif-1",
        ...
        val obj = json.decodeFromString<StrataNotifications200ResponseInner>(src)
    ...
    @Test
    fun `StrataNotifications200ResponseInner without action`() {
    ...
    @Test
    fun `StrataNotifications200ResponseInner with showIn and suggestModelId`() {
        val src = """{
            "id": "n3",
            "title": "Try new model",
    ...
    fun `empty notifications array`() {
        val list = json.decodeFromString<List<StrataNotifications200ResponseInner>>("[]")
        assertTrue(list.isEmpty())
    }

    @Test
    fun `StrataProfile200Response with balance`() {
        val src = """{
            "profile": {"email": "user@test.com", "name": "User"},
            "balance": {"balance": 42.5},
            "currentOrgId": "org-1"
        }"""
        val obj = json.decodeFromString<StrataProfile200Response>(src)
    ...
    @Test
    fun `StrataProfile200Response with null balance`() {
        val src = """{
    ...
    @Test
    fun `StrataProfile200Response with organizations`() {
        val src = """{
            "profile": {
                "email": "user@test.com",
                "organizations": [
