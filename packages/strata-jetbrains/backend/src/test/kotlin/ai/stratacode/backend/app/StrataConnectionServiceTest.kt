...
import ai.stratacode.backend.app.ConnectionState
import ai.stratacode.backend.app.StrataConnectionService
...
class StrataConnectionServiceTest {
    ...
    fun `connect transitions to Connected`() = runBlocking {
        ...
        val svc = StrataConnectionService(
          scope,
          fake,
    ...
    fun `connect provides API client`() = runBlocking {
        val svc = StrataConnectionService(scope, fake, {}, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    fun `init error transitions to Error state`() = runBlocking {
        ...
        val svc = StrataConnectionService(scope, failing, {}, log)
        svc.connect()
    ...
    // extractType tests moved to StrataCliDataParserTest
