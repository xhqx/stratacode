...
import ai.stratacode.backend.cli.StrataCliDataParser
...
/**
 * Pure unit tests for [StrataCliDataParser].
 *
...
class StrataCliDataParserTest {
    ...
    fun `extractEventType - parses type from JSON data`() {
        val result = StrataCliDataParser.extractEventType(
    ...
    fun `extractEventType - returns unknown for missing type`() {
        assertEquals("unknown", StrataCliDataParser.extractEventType("""{"data":"something"}"""))
    }

    @Test
    fun `extractEventType - returns unknown for empty string`() {
        assertEquals("unknown", StrataCliDataParser.extractEventType(""))
    }
    ...
    fun `parseChatEvent - message updated with GlobalEvent wrapper`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("message.updated", data)
    ...
    fun `parseChatEvent - message part delta`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("message.part.delta", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - message part updated`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("message.part.updated", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - turn open`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("session.turn.open", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - turn close`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("session.turn.close", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session error`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("session.error", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - message removed`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("message.removed", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - unknown type returns null`() {
        val data = globalEvent("""
            ...
            "properties": { "sessionID": "ses_1" }
        """)
        assertNull(StrataCliDataParser.parseChatEvent("some.unknown.event", data))
    }

    @Test
    fun `parseChatEvent - malformed JSON returns null`() {
        assertNull(StrataCliDataParser.parseChatEvent("message.updated", "not json"))
    }

    @Test
    fun `parseChatEvent - missing properties returns null`() {
        assertNull(StrataCliDataParser.parseChatEvent("message.updated", """{"payload":{"type":"x"}}"""))
    }
    ...
    fun `parseChatEvent - missing sessionID returns null`() {
        ...
        """)
        assertNull(StrataCliDataParser.parseChatEvent("message.updated", data))
    }
    ...
    fun `parseSessionStatus - valid status event`() {
        ...
        val result = StrataCliDataParser.parseSessionStatus(data)
        assertNotNull(result)
    ...
    fun `parseSessionStatus - missing sessionID returns null`() {
        ...
        assertNull(StrataCliDataParser.parseSessionStatus(data))
    }
    ...
    fun `parseSession - full session response`() {
        ...
        val result = StrataCliDataParser.parseSession(raw)
    ...
    fun `parseMessages - empty array`() {
        assertEquals(emptyList(), StrataCliDataParser.parseMessages("[]"))
    }
    ...
    fun `parseMessages - user and assistant messages`() {
        ...
        val result = StrataCliDataParser.parseMessages(raw)
    ...
    fun `parseMessages - malformed JSON returns empty`() {
        assertEquals(emptyList(), StrataCliDataParser.parseMessages("not json"))
    }
    ...
    fun `buildPromptJson - text only`() {
        ...
        val result = StrataCliDataParser.buildPromptJson(prompt)
    ...
    fun `buildConfigPartial - model only`() {
        val result = StrataCliDataParser.buildConfigPartial(ConfigUpdateDto(model = "anthropic/claude-4"))
        assertEquals("""{"model":"anthropic/claude-4"}""", result)
    }

    @Test
    fun `buildConfigPartial - agent and temperature`() {
        val result = StrataCliDataParser.buildConfigPartial(
    ...
    fun `buildConfigPartial - empty update`() {
        val result = StrataCliDataParser.buildConfigPartial(ConfigUpdateDto())
        assertEquals("{}", result)
    }

    @Test
    fun `buildConfigPartial - temperature without agent defaults to ask`() {
        val result = StrataCliDataParser.buildConfigPartial(ConfigUpdateDto(temperature = 0.5))
    ...
    fun `parseChatEvent - permission asked`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("permission.asked", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - permission replied`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("permission.replied", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - question asked`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("question.asked", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - question replied`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("question.replied", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - question rejected`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("question.rejected", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session status idle`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("session.status", data) as ChatEventDto.SessionStatusChanged
    ...
    fun `parseChatEvent - message part removed`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("message.part.removed", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - todo updated`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("todo.updated", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session idle`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("session.idle", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session compacted`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("session.compacted", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - session diff`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("session.diff", data)
        assertNotNull(result)
    ...
    fun `parseChatEvent - part updated with callID`() {
        ...
        val result = StrataCliDataParser.parseChatEvent("message.part.updated", data) as ChatEventDto.PartUpdated
    ...
    fun `buildPermissionReplyJson - once reply`() {
        val result = StrataCliDataParser.buildPermissionReplyJson(PermissionReplyDto(reply = "once"))
        assertEquals("""{"reply":"once"}""", result)
    }

    @Test
    fun `buildPermissionReplyJson - always reply with message`() {
        val result = StrataCliDataParser.buildPermissionReplyJson(PermissionReplyDto(reply = "always", message = "approved"))
    ...
    fun `buildPermissionAlwaysRulesJson - approved list`() {
        val result = StrataCliDataParser.buildPermissionAlwaysRulesJson(
    ...
    fun `buildQuestionReplyJson - single question single answer`() {
        val result = StrataCliDataParser.buildQuestionReplyJson(QuestionReplyDto(answers = listOf(listOf("A"))))
        assertEquals("""{"answers":[["A"]]}""", result)
    }

    @Test
    fun `buildQuestionReplyJson - multiple questions`() {
        val result = StrataCliDataParser.buildQuestionReplyJson(
    ...
    fun `parsePermissionRequests - parses list`() {
        ...
        val result = StrataCliDataParser.parsePermissionRequests(raw)
        assertEquals(1, result.size)
    ...
    fun `parsePermissionRequests - empty list`() {
        assertEquals(emptyList(), StrataCliDataParser.parsePermissionRequests("[]"))
    }
    ...
    fun `parseQuestionRequests - parses list`() {
        ...
        val result = StrataCliDataParser.parseQuestionRequests(raw)
        assertEquals(1, result.size)
