...
/**
 * Manages [StrataBackendWorkspace] instances by directory path.
 *
 ...
 */
class StrataBackendWorkspaceManager(
    private val cs: CoroutineScope,
    private val sessions: StrataBackendSessionManager,
    private val log: StrataLog,
) {
    private val workspaces = ConcurrentHashMap<String, StrataBackendWorkspace>()
     ...
     * Called by [StrataBackendAppService] after [StrataAppState.Ready].
    ...
    /**
     * Deactivate all workspaces. Called by [StrataBackendAppService] on disconnect.
     */
    fun stop() {
     ...
     */
    fun get(dir: String): StrataBackendWorkspace {
        ...
        return workspaces.computeIfAbsent(dir) { d ->
            ...
            StrataBackendWorkspace(d, cs, client, ev, sessions, log).also { it.load() }
        }
    }
