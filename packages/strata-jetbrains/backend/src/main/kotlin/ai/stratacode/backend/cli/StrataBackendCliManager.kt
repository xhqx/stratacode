...
/**
 * Manages the Strata CLI binary lifecycle.
 *
 * Extracts the bundled CLI from JAR resources into IntelliJ's system directory,
 * spawns `strata serve --port 0`, and exposes the result as [State].
 *
 * Concurrency is handled by the owning [StrataBackendAppService] — all public
 * methods except [exited] are called under its mutex. [exited] is called from
 * [StrataConnectionService]'s IO dispatcher and is thread-safe via the stale-ref
 * guard and volatile [process] field.
 */
class StrataBackendCliManager(
    private val log: StrataLog = StrataLog.create(StrataBackendCliManager::class.java),
) : CliServer {
    ...
    private fun extractCli(): File {
        ...
        val exe = if (SystemInfo.isWindows) "strata.exe" else "strata"
        val resource = "cli/$platform/$exe"
        val loader = javaClass.classLoader

        val target = File(PathManager.getSystemPath(), "strata/bin/$exe")
    ...
    private suspend fun spawn(cli: File): CliServer.State =
        withContext(Dispatchers.IO) {
            ...
            val env = buildMap {
                ...
                put("STRATA_SERVER_PASSWORD", pwd)
                put("STRATA_CLIENT", "jetbrains")
                put("STRATA_ENABLE_QUESTION_TOOL", "true")
                put("STRATA_PLATFORM", "jetbrains")
                put("STRATA_APP_NAME", "strata-code")
                put("STRATA_DISABLE_CLAUDE_CODE", "true")
                put("STRATACODE_FEATURE", "jetbrains-plugin")
            ...
            log.info("CLI env: STRATA_CLIENT=jetbrains STRATA_PLATFORM=jetbrains STRATA_APP_NAME=strata-code")
            ...
            }, "strata-cli-stderr").apply { isDaemon = true; start() }
    ...
    private fun ideEnv(): Map<String, String> = buildMap {
        runCatching {
            ...
            put("STRATA_EDITOR_NAME", name)
            put("STRATACODE_EDITOR_NAME", "$name $build")
        }.onFailure { log.info("Could not read ApplicationInfo: ${it.message}") }

        runCatching {
            val version = PluginManagerCore
                .getPlugin(PluginId.getId("ai.stratacode"))?.version
            if (version != null) put("STRATA_APP_VERSION", version)
        }.onFailure { log.info("Could not read plugin version: ${it.message}") }

        runCatching {
            put("STRATA_MACHINE_ID", machineId())
        }.onFailure { log.info("Could not read machine ID: ${it.message}") }
    }

    private fun machineId(): String {
        val file = File(PathManager.getSystemPath(), "strata/machine-id")
