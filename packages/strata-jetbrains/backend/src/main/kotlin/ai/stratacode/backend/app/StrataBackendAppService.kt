...
import ai.stratacode.backend.cli.CliServer
import ai.stratacode.backend.cli.StrataBackendCliManager
import ai.stratacode.log.StrataLog
import ai.stratacode.backend.workspace.StrataBackendWorkspaceManager
import ai.stratacode.jetbrains.api.client.DefaultApi
import ai.stratacode.jetbrains.api.infrastructure.ClientError
import ai.stratacode.jetbrains.api.infrastructure.ClientException
import ai.stratacode.jetbrains.api.infrastructure.ServerError
import ai.stratacode.jetbrains.api.infrastructure.ServerException
  ...
  private val log: StrataLog,
    ...
    companion object {
        ...
        internal fun create(
          ...
          server: CliServer,
          log: StrataLog,
        ) = StrataBackendAppService(cs, server, log)
    }

    private val mutex = Mutex()
    private val connection = StrataConnectionService(cs, server, onReconnect = {
        cs.launch { reconnect() }
    }, log = log)
    ...
    private val _appState = MutableStateFlow<StrataAppState>(StrataAppState.Disconnected)
    val appState: StateFlow<StrataAppState> = _appState.asStateFlow()
    ...
    val sessions = StrataBackendSessionManager(cs, log)
    val chat = StrataBackendChatManager(cs, log)
    val workspaces = StrataBackendWorkspaceManager(cs, sessions, log)

    @Volatile var profile: StrataProfile200Response? = null
        private set

    @Volatile var config: Config? = null
        private set

    @Volatile var notifications: List<StrataNotifications200ResponseInner> = emptyList()
        private set

    suspend fun connect() {
        mutex.withLock {
            val current = _appState.value
            if (current is StrataAppState.Ready || current is StrataAppState.Connecting || current is StrataAppState.Loading) return
            ensureWatcher()
            connection.connect()
        }
    }
    ...
    private suspend fun reconnect() {
        mutex.withLock {
            val current = _appState.value
            if (current is StrataAppState.Ready || current is StrataAppState.Connecting || current is StrataAppState.Loading) {
    ...
    private fun ensureWatcher() {
        ...
        watcher = cs.launch {
            connection.state.collect { next ->
                when (next) {
                    ConnectionState.Disconnected -> _appState.value = StrataAppState.Disconnected
                    ConnectionState.Connecting -> _appState.value = StrataAppState.Connecting
                    is ConnectionState.Connected -> load()
                    is ConnectionState.Error -> _appState.value = StrataAppState.Error(next.message)
                }
            }
        }
    }
     ...
     *
     * Progress is tracked via [LoadProgress] and emitted as [StrataAppState.Loading].
     * On success, transitions to [StrataAppState.Ready].
     * On failure of required data, transitions to [StrataAppState.Error].
     */
    private fun load() {
        synchronized(loadLock) {
            ...
            loader = cs.launch {
                ...
                _appState.value = StrataAppState.Loading(progress.get())

                val errors = CopyOnWriteArrayList<LoadError>()
                var cfg: Config? = null
                var prof: StrataProfile200Response? = null
                var notifs: List<StrataNotifications200ResponseInner> = emptyList()
                ...
                try {
                    coroutineScope {
                        launch {
                            ...
                            progress.updateAndGet { it.copy(profile = status) }
                                .also { _appState.value = StrataAppState.Loading(it) }
                        }
                        launch {
                    ...
                    _appState.value = StrataAppState.Ready(
                        AppData(
                            profile = prof,
                            config = cfg!!,
                            notifications = notifs,
                    ...
                    _appState.value = StrataAppState.Error(
     ...
     * as failures.
     */
    private suspend fun fetchProfile(): FetchResult<StrataProfile200Response?> {
        val client = connection.api
            ?: return FetchResult.ok(null)
        return try {
            val response = client.strataProfile()
    ...
    private suspend fun fetchNotifications(): FetchResult<List<StrataNotifications200ResponseInner>> {
        val client = connection.api
            ?: return FetchResult.fail("notifications", detail = "Not connected")
        return try {
            FetchResult.ok(client.strataNotifications())
        } catch (e: Exception) {
     ...
     *   Re-fetches config and updates [StrataAppState.Ready] data in-place.
     *
                ...
                connection.events.collect { event ->
                    when (event.type) {
                        "global.config.updated" -> {
                            ...
                            launch {
                                ...
                                if (result.value != null) {
                                    config = result.value
                                    val current = _appState.value
                                    if (current is StrataAppState.Ready) {
                        ...
                        "global.disposed" -> {
                            ...
                            val current = _appState.value
                            if (current is StrataAppState.Ready) load()
                        }
    ...
    private fun clear() {
        ...
        _appState.value = StrataAppState.Disconnected
    }
