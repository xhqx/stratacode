 ...
 *
 * Concurrency is handled by the owning [StrataBackendAppService] — `connect`,
 * `restart`, and `reinstall` are called under its mutex. Internal reconnect
 * attempts delegate back to the owner via [onReconnect].
 *
 * Not a service — owned and instantiated by [StrataBackendAppService].
 */
class StrataConnectionService(
  private val cs: CoroutineScope,
  private val server: CliServer,
  private val onReconnect: () -> Unit,
  private val log: StrataLog = StrataLog.create(StrataConnectionService::class.java),
) {
     ...
     *
     * Called under [StrataBackendAppService]'s mutex — no internal guard needed.
     */
    suspend fun connect() {
        open()
    }
     ...
     *
     * Called under [StrataBackendAppService]'s mutex.
     */
    suspend fun restart() {
    ...
    private suspend fun open() {
        ...
        val ac = StrataBackendHttpClients.api(password)
        val hc = StrataBackendHttpClients.health(password)
        apiClient = ac
        healthClient = hc
    ...
    private val listener = object : EventSourceListener() {
        ...
        override fun onEvent(src: EventSource, id: String?, type: String?, data: String) {
            lastEvent.set(System.currentTimeMillis())
            val kind = type ?: StrataCliDataParser.extractEventType(data)
     ...
     * goes through [StrataBackendAppService]'s mutex for a full restart.
     */
    ...
    private fun close() {
        api = null
        apiClient?.let { StrataBackendHttpClients.shutdown(it) }
        apiClient = null
        healthClient?.let { StrataBackendHttpClients.shutdown(it) }
        healthClient = null
    }
    ...
    fun dispose() {
        ...
        log.info("StrataConnectionService disposed")
    }
}
