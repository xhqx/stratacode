 ...
 *
 * **Not an IntelliJ service** — owned by [StrataBackendAppService] which
 * calls [start] after [StrataAppState.Ready] and [stop] on disconnect.
 *
 * All JSON parsing is delegated to [StrataCliDataParser].
 */
class StrataBackendChatManager(
        ...
        watcher = cs.launch {
            sse.collect { event ->
                if (event.type in CHAT_EVENTS) {
                    val parsed = StrataCliDataParser.parseChatEvent(event.type, event.data)
                    if (parsed != null) {
    ...
    fun prompt(id: String, dir: String, prompt: PromptDto) {
        ...
        val body = StrataCliDataParser.buildPromptJson(prompt)
    ...
    fun messages(id: String, dir: String): List<MessageWithPartsDto> {
        ...
        return http.newCall(request).execute().use { response ->
            ...
            val parsed = StrataCliDataParser.parseMessages(raw)
    ...
    fun updateConfig(dir: String, update: ConfigUpdateDto) {
        ...
        val partial = StrataCliDataParser.buildConfigPartial(update)
    ...
    fun replyPermission(requestId: String, dir: String, reply: PermissionReplyDto) {
        ...
        val body = StrataCliDataParser.buildPermissionReplyJson(reply)
    ...
    fun savePermissionRules(requestId: String, dir: String, rules: PermissionAlwaysRulesDto) {
        ...
        val body = StrataCliDataParser.buildPermissionAlwaysRulesJson(rules)
    ...
    fun replyQuestion(requestId: String, dir: String, answers: QuestionReplyDto) {
        ...
        val body = StrataCliDataParser.buildQuestionReplyJson(answers)
    ...
    fun pendingPermissions(dir: String): List<PermissionRequestDto> {
        ...
        val parsed = StrataCliDataParser.parsePermissionRequests(raw)
    ...
    fun pendingQuestions(dir: String): List<QuestionRequestDto> {
        ...
        val parsed = StrataCliDataParser.parseQuestionRequests(raw)
