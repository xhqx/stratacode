...
import ai.stratacode.backend.app.SseEvent
import ai.stratacode.log.StrataLog
import ai.stratacode.jetbrains.api.client.DefaultApi
import ai.stratacode.jetbrains.api.model.Agent
 ...
 * [StrataBackendWorkspaceManager] for each directory. Receives a
 * pre-connected [DefaultApi] — no null checks needed.
 *
 * Session operations delegate to [StrataBackendSessionManager] with
 * this workspace's [directory], so the frontend only needs one
 * object per directory.
 */
class StrataBackendWorkspace(
    ...
    private val _state = MutableStateFlow<StrataWorkspaceState>(StrataWorkspaceState.Pending)
    val state: StateFlow<StrataWorkspaceState> = _state.asStateFlow()
    ...
    fun load() {
        synchronized(loadLock) {
            ...
            val progress = AtomicReference(StrataWorkspaceLoadProgress())
            _state.value = StrataWorkspaceState.Loading(progress.get())
            ...
            try {
                    ...
                    launch {
                        ...
                        if (result != null) {
                            ...
                            progress.updateAndGet { it.copy(providers = true) }
                                .also { _state.value = StrataWorkspaceState.Loading(it) }
                        } else {
                ...
                _state.value = StrataWorkspaceState.Ready(
                    providers = prov!!,
                    agents = ag!!,
                    commands = cmd!!,
                    skills = sk!!,
                ...
                _state.value = StrataWorkspaceState.Error(
    ...
    fun stop() {
        ...
        _state.value = StrataWorkspaceState.Pending
    }
