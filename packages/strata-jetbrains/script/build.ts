...
/**
 * Build the Strata JetBrains plugin.
 *
 * Usage:
 ...
 * 2. Copies them into backend/build/generated/cli/cli/{os}/strata[.exe]
 *    so they end up inside the backend jar at /cli/{os}/strata.
...
const platforms = [
  { os: "darwin-arm64", exe: "strata" },
  { os: "darwin-x64", exe: "strata" },
  { os: "linux-arm64", exe: "strata" },
  { os: "linux-x64", exe: "strata" },
  { os: "windows-x64", exe: "strata.exe" },
  { os: "windows-arm64", exe: "strata.exe" },
] as const
...
function distBinPath(os: string, exe: string): string {
  return join(distDir, `@stratacode/cli-${os}`, "bin", exe)
}
