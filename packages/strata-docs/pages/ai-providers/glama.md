...
Strata Code will automatically try to fetch a list of available models from the Glama API. Some models that are commonly available through Glama include:

- **Anthropic Claude models:** (e.g., `anthropic/claude-3-5-sonnet`) These are generally recommended for best performance with Strata Code.
...
Glama is not yet available as a CLI provider. Check the [Strata Code releases](https://github.com/Strata-Org/stratacode/releases) for updates on provider support.
{% /callout %}
