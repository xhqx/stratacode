...
In January 2026, Anthropic decided to restrict Claude Code CLI to official Claude Code clients. Claude Code credentials cannot be used in Strata Code or other third-party harnesses.

For continued use of Anthropic models in Strata Code, please use the [Anthropic API provider](/docs/ai-providers/anthropic) with an API key instead.
{% /callout %}

# Using Claude Code With Strata Code

Claude Code is Anthropic's official CLI that provides direct access to Claude models from your terminal. Using Claude Code with Strata Code lets you leverage your existing CLI setup without needing separate API keys.
...
If this environment variable is set on your system, the `claude` tool may use it for authentication instead of the interactive `/login` method. When Strata Code executes the tool, it will accurately reflect that an API key is being used, as this is the underlying behavior of the `claude` CLI itself.
{% /callout %}
...
Claude Code credentials no longer work in Strata Code. Please use the [Anthropic provider](/docs/ai-providers/anthropic) with an API key instead.
{% /callout %}
...
**Config file** (`~/.config/strata/strata.json` or `./strata.json`):
...
- **Enhance Prompt Feature:** Full compatibility with Strata Code's Enhance Prompt feature, allowing you to automatically improve and refine your prompts before sending them to Claude.
