---
title: "AI Providers"
description: "Configure and connect different AI model providers to Strata Code"
---

# AI Providers

Strata Code supports a wide variety of AI providers, giving you flexibility in how you power your AI-assisted development workflow. Choose from cloud providers, local models, or AI gateways based on your needs.

## Getting Started

The fastest way to get started is with **Strata Code's built-in provider**, which requires no configuration. Simply sign in and start coding.
...
{% callout type="note" %}
In the **VSCode (Legacy)** version, API keys use VS Code's Secret Storage. In the current **VSCode & CLI** version, keys are set via environment variables or referenced in `strata.json` config files. See individual provider pages for setup instructions for each platform.
{% /callout %}

## Disabling Built-in Providers

You can prevent specific providers from loading using `disabled_providers` in your `strata.json` (or `strata.jsonc`). This is useful to hide models from built-in or detected providers that you don't intend to use.
...
Both fields accept provider IDs — the lowercase identifier used in the `provider/model` format (e.g. `strata`, `anthropic`, `openai`, `google`, `groq`).

## Next Steps

- **New to Strata Code?** Start with the [Strata Code provider](/docs/ai-providers/stratacode) - no setup required
