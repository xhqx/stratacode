...
# Using OpenAI Compatible Providers With Strata Code

Strata Code supports a wide range of AI model providers that offer APIs compatible with the OpenAI API standard. This means you can use models from providers _other than_ OpenAI, while still using a familiar API interface. This includes providers like:
...
You'll find these settings in the Strata Code settings panel (click the {% codicon name="gear" /%} icon):
...
- **Base URL** — The provider's OpenAI-compatible API endpoint (e.g., `https://api.your-provider.com/v1`). Strata auto-fetches available models when a valid URL is entered.
...
For additional model configuration (token limits, tool calling, variants), edit the `strata.jsonc` config file directly — see the **CLI** tab or the [Custom Models](/docs/code-with-ai/agents/custom-models) guide.

### Automatic Model Detection

When configuring a custom OpenAI-compatible provider, Strata Code can automatically detect available models from your provider's `/v1/models` endpoint.

Once you enter a valid **Base URL** and **API Key**, Strata Code will query the provider and present a searchable model picker with all available models. You can:
...
Define a custom provider in your `strata.json` config file (`~/.config/strata/strata.json` or `./strata.json`). The provider key (e.g., `"vllm"`) is your chosen identifier — it can be any name you like.
...
Strata Code supports full endpoint URLs in the Base URL field, providing greater flexibility for provider configuration:
...
By using an OpenAI-compatible provider, you can leverage the flexibility of Strata Code with a wider range of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
