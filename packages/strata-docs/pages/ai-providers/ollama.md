...
# Using Ollama With Strata Code

Strata Code supports running models locally using Ollama. This provides privacy, offline access, and potentially lower costs, but requires more setup and a powerful computer.
...
An alternative to `qwen3-coder:30b` is `devstral:24b`. For other features of Strata Code such as Enhance Prompt or Commit Message Generation smaller models may suffice.

## Setting up Ollama

To set up Ollama for use with Strata Code, follow the instructions below.
...
By default, API requests time out after 10 minutes. Local models can be slow, if you hit this timeout you can consider increasing it here: VS Code Extensions panel > Strata Code gear menu > Settings > API Request Timeout.

### Configure Strata Code

{% tabs %}
{% tab label="VSCode (Legacy)" %}

- Open the Strata Code panel ({% strataCodeIcon size="1em" /%}).
...
If your Ollama model doesn't appear in the Strata model picker, register it as a custom model in your config file:
