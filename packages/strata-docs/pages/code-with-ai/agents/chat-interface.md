...
description: "Learn how to use the Strata Code chat interface effectively"
---

# Chatting with Strata Code

{% callout type="tip" %}
**Bottom line:** Strata Code is an AI coding assistant. You chat with it in plain English, and it writes, edits, and explains code for you.
{% /callout %}
...
Click the Strata Code icon ({% strataCodeIcon /%}) in VS Code's Primary Side Bar to open the sidebar chat. You can also pop it out into an editor tab for a larger workspace.

{% /tab %}
{% tab label="CLI" %}

Open your terminal and run `strata` to launch the interactive terminal interface (TUI). You'll see a prompt where you can start typing requests immediately. The TUI is fully keyboard-driven — no mouse required.

{% /tab %}
{% tab label="VSCode (Legacy)" %}

Find the Strata Code icon ({% strataCodeIcon /%}) in VS Code's Primary Side Bar. Click it to open the chat panel.

**Lost the panel?** Go to View > Open View... and search for "Strata Code"

{% /tab %}
{% /tabs %}

## How to Talk to Strata Code

**The key insight:** Just type what you want in normal English. No special commands needed.

{% image src="/docs/img/typing-your-requests/typing-your-requests.png" alt="Example of typing a request in Strata Code" width="800" caption="Example of typing a request in Strata Code" /%}
...
- **Action buttons** - Approve or reject Strata's proposed changes
- **Plus button** - Start a new task session
- **Mode selector** - Choose how Strata should approach your task
...
**Why it matters:** Strata Code works best when you communicate like you're talking to a smart teammate who needs clear direction.

## Suggested Responses

When Strata Code needs more information to complete a task, it asks a follow-up question and often provides suggested answers to make responding faster.

**How it works:**

1. **Question Appears** - Strata Code asks a question using the `question` tool
...
In the legacy extension, Strata Code uses the `ask_followup_question` tool instead. Suggestions appear as clickable buttons below the question. You can click a button to send the answer directly, or hold `Shift` and click (or click the pencil icon {% codicon name="edit" /%}) to copy the suggestion into the input box for editing before sending.

{% image src="/docs/img/suggested-responses/suggested-responses.png" alt="Example of Strata Code asking a question with suggested response buttons below it" width="800" caption="Suggested responses appear as clickable buttons below questions" /%}
{% /callout %}

**Benefits:**

- **Speed** - Quickly respond without typing full answers
- **Clarity** - Suggestions often clarify the type of information Strata Code needs
- **Flexibility** - Edit suggestions to provide precise, customized answers when needed

This feature streamlines the interaction when Strata Code requires clarification, allowing you to guide the task effectively with minimal effort.
...
{% callout type="tip" %}
**Move Strata Code to the Secondary Side Bar** for a better layout. Right-click on the Strata Code icon in the Activity Bar and select **Move To → Secondary Side Bar**. This lets you see the Explorer, Search, Source Control, etc. alongside Strata Code.

{% image src="/docs/img/move-to-secondary.png" alt="Move to Secondary Side Bar" width="600" caption="Move Strata Code to the Secondary Side Bar for better workspace organization" /%}
{% /callout %}
...
{% callout type="tip" %}
**Drag files directly into chat.** Once you have Strata Code in a separate sidebar from the file explorer, you can drag files from the explorer into the chat window (even multiple at once). Just hold down the Shift key after you start dragging the files.
{% /callout %}

{% /tab %}
{% /tabs %}

Ready to start coding? Start a session in Strata Code and describe what you want to build!
