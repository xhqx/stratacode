...
Strata Code ships with a curated list of models for each provider, but you can use **any model** your provider supports — including models that aren't in the built-in list. This is useful for:
...
- **Base URL** — The OpenAI-compatible API endpoint (e.g., `https://api.myprovider.com/v1`). When a valid URL is entered, Strata automatically fetches available models from the endpoint.
...
For additional model configuration (token limits, tool calling, reasoning, variants), edit the `strata.jsonc` config file directly — see the **CLI** tab for the format.

{% /tab %}
{% tab label="CLI" %}

**Config file** (`~/.config/strata/strata.jsonc` or `./strata.jsonc`):
...
If `modalities` is omitted and the model ID matches a models.dev catalog entry for that provider, Strata uses the catalog's modalities. For completely custom models with no catalog match, Strata defaults to text input and text output only. Set `attachment: true` alongside image, audio, video, or PDF input modalities when the provider supports sending those files as attachments.

### Token Limits (limit)

The `limit` object controls how Strata manages the model's context window and output length. These values are specified in **tokens**.
...
Strata resolves token limits in this order:

1. **Your config** — values you set under `provider.<id>.models.<model>.limit`
2. **Built-in catalog** — Strata ships a snapshot of [models.dev](https://models.dev) and refreshes it hourly. If your model ID matches a known model, catalog values are used as defaults.
...
- **Compaction is disabled.** Strata uses `context` to detect when the conversation exceeds the model's window and needs to be summarized. With `context: 0`, overflow detection is skipped and conversations will grow unbounded until the provider rejects the request.
- **Output falls back to 32,000 tokens.** When `output` is `0`, Strata uses its internal default of 32,000 tokens (configurable via the `STRATA_EXPERIMENTAL_OUTPUT_TOKEN_MAX` environment variable).
...
When Strata starts, it resolves the active model in this order:
...
- Run `strata models` to list all available models and confirm your provider is active
