...
Instead of maintaining a static list that's perpetually behind, we built something better — a real-time leaderboard showing which models Strata Code users are actually having success with right now.

## Check the Live Models List

**[👉 See what's working today at strata.ai/models](https://strata.ai/models)**
...
- Set per-agent defaults and a global default in the **Settings** panel (Models tab), or directly in the `strata.jsonc` config file.
- **Model precedence:** Session override → Last picked per agent → Per-agent config → Global config → Strata Auto (free).
...
- For non-interactive use, pass `--model` flag to `strata run` (e.g., `strata run --model claude-sonnet-4-20250514`).
- Set the global default with the `model` key in `strata.jsonc`, or configure per-agent models in the `agent` section.
...
- **Via config file** — edit `strata.jsonc`:
...
The AI model space moves fast. Bookmark [strata.ai/models](https://strata.ai/models) and check back when you're evaluating options. What's best today might not be best next month — and that's actually exciting.
