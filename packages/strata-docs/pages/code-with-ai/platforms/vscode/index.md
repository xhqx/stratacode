...
description: "Using Strata Code in Visual Studio Code"
---

# VS Code Extension

Strata Code is available as two VS Code extensions: the **VSCode (Legacy)** extension and the current **VSCode** version built on the Strata CLI core.
...
The extension bundles its own CLI binary and spawns `strata serve` as a background process. All communication happens over HTTP + SSE.
...
- **[JSONC config files](/docs/getting-started/settings)** — Portable settings in `strata.jsonc` instead of VS Code settings
- **[Granular permissions](/docs/getting-started/settings/auto-approving-actions)** — Per-tool permission rules with glob patterns
- **[Agents](/docs/code-with-ai/agents/using-agents)** — Customizable agents (`.strata/agents/*.md`) replacing the modes system
...
The extension shares its configuration with the CLI. Settings in `~/.config/strata/strata.jsonc` (global) and `./strata.jsonc` (project) apply to both the CLI and the extension.
...
- **[Custom Modes](/docs/customize/custom-modes)** — Define custom modes with `.stratacodemodes` YAML files
