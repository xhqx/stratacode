---
title: "Strata CLI"
description: "Using Strata Code from the command line"
platform: new
---

{% callout type="warning" title="Version Notice" %}
This documentation applies only to Strata version 1.0 and later. Users running versions below 1.0 should upgrade before proceeding.
{% /callout %}
...
The Strata Code CLI uses the same underlying technology that powers the IDE extensions, so you can expect the same workflow to handle agentic coding tasks from start to finish.

**Source code & issues (Strata CLI 1.0):** [Strata-Org/stratacode](https://github.com/Strata-Org/stratacode) · [Report an issue](https://github.com/Strata-Org/stratacode/issues)
...
Change directory to where you want to work and run strata:
...
After installation, run `strata` and use the `/connect` command to add your first provider credentials. This is the interactive way to configure API keys for model providers.

## Update

Upgrade the Strata CLI:

`strata upgrade`

Or use npm:

`npm update -g @stratacode/cli`

## What you can do with Strata Code CLI
...
- **Switch between hundreds of LLMs without constraints.** Other CLI tools only work with one model or curate opinionated lists. With Strata, you can switch models without booting up another tool.
...
#### Strata Gateway Commands (when connected)
...
- Config files in **`~/.config/strata/`**: use **`strata.jsonc`** for provider, model, permission, and **MCP** settings. Restart the CLI after editing. See [Using MCP in Strata Code](/docs/automate/mcp/using-in-strata-code) for MCP config format.
- `strata auth` for credential management
...
Strata Code uses the permission config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Strata was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `list`, `glob`, `grep`, and many bash commands).
...
The Strata CLI is a fork of [OpenCode](https://opencode.ai) and supports the same configuration options. The CLI you install with `npm install -g @stratacode/cli` (Strata CLI 1.0) is built from [Strata-Org/stratacode](https://github.com/Strata-Org/stratacode). For comprehensive configuration documentation, see the [OpenCode Config documentation](https://opencode.ai/docs/config).
...
Interactive mode is the default mode when running Strata Code without the `--auto` flag, designed to work interactively with a user through the console.

In interactive mode Strata Code will request approval for operations which have not been auto-approved, allowing the user to review and approve operations before they are executed, and optionally add them to the auto-approval list.
...
Autonomous mode allows Strata Code to run in automated environments like CI/CD pipelines without requiring user interaction.

```bash
# Run in autonomous mode with a message
strata run --auto "Implement feature X"
```
...
- name: Run Strata Code
  run: |
    strata run "Implement the new feature" --auto
```
...
Remote Connections let you access your local CLI sessions from the Cloud Agents web interface. Requires [Strata Gateway](/docs/gateway) connection.
...
Requires connection to Strata Gateway. The `/remote` command appears only when authenticated.

**Enable by default:**

Add to `~/.config/strata/config.json`:
...
Once enabled, start a CLI session and open [Cloud Agents](https://app.strata.ai/cloud). Your local session appears in the dashboard. See [Cloud Agent Remote Connections](/docs/code-with-ai/platforms/cloud-agent#remote-connections) for details.

### Requirements

- Connection to Strata Gateway
- Same Strata account on CLI and Cloud Agent
- CLI must remain running with internet connection

{% callout type="warning" title="Security Warning" %}
Anyone with access to your Strata account can send messages to your computer when remote mode is enabled.
{% /callout %}
...
- `STRATA_PROVIDER`: Override the active provider ID
- For `stratacode` provider: `STRATACODE_<FIELD_NAME>` (e.g., `STRATACODE_MODEL` → `stratacodeModel`)
- For other providers: `STRATA_<FIELD_NAME>` (e.g., `STRATA_API_KEY` → `apiKey`)

## Using the CLI in an Organization

If you belong to a Strata organization (Team or Enterprise), you can route CLI requests through that organization. The process differs slightly between interactive and non-interactive usage.
...
### Non-Interactive Usage (`strata run`)

There is no `--org` or `--team` flag on `strata run`. Instead, the organization is determined from the following sources, in order of priority (highest first):

1. **`STRATA_ORG_ID` environment variable** — Best for non-interactive and CI environments.
