...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in Strata Code. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.
...
- A [Strata Code account](https://app.strata.ai) (free to create)
...
## Step 4: Add Your Key via BYOK in Strata

1. Log into the [Strata platform](https://app.strata.ai).
2. Navigate to the [Bring Your Own Key (BYOK) page](https://app.strata.ai/byok), available in the sidebar under **Account**.
...
Once your BYOK key is saved, Strata Code's autocomplete will automatically use your Codestral key through the Strata Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the Strata Code extension installed.
...
The autocomplete status bar in VS Code shows the current provider ("Strata Gateway") and tracks cumulative cost. With BYOK, requests are billed directly by Mistral at their rates (Codestral has a free tier) and show as $0.00 on your Strata balance.
...
```
Your Editor → Strata Gateway (with your key) → Mistral
```

- The Strata Gateway detects your BYOK key and routes autocomplete requests using it.
- You are billed directly by Mistral — Strata does not add any markup.
- If your BYOK key is invalid, the request will fail (it does not fall back to Strata's keys).

## Troubleshooting

- **Autocomplete not appearing?** Check that autocomplete is enabled in Strata Code settings (it is on by default). Also verify you are signed into Strata Code in the extension.
...
- **Seeing charges on your Strata balance?** If you haven't configured BYOK, autocomplete defaults to using your Strata credits. Add your Codestral key via BYOK to route requests through your own Mistral account.
...
{% youtube url="https://www.youtube.com/embed/0aqBbB8fPho" caption="Setting up Mistral for free autocomplete in Strata Code" /%}

## Step 1: Open Strata Code Settings

In VS Code, open the Strata Code panel and click the **Settings** icon (gear) in the top-right corner.

![Open Strata Code Settings](/docs/img/mistral-setup/01-open-strata-code-settings.png)
...
{% callout type="note" %}
When creating an autocomplete profile, you don't need to select a specific model—Strata Code will automatically use the appropriate Codestral model optimized for code completions.
{% /callout %}
...
## Step 9: Paste API Key in Strata Code

Return to Strata Code settings and paste your API key into the **Mistral API Key** field.
