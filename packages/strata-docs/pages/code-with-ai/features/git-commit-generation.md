...
Generate descriptive commit messages automatically based on your staged git changes. Strata Code analyzes your staged files and creates conventional commit messages that follow best practices.
...
2. In the VS Code Source Control panel, look for the `Strata Code` logo next to the commit message field)
...
Configuration is handled through the extension's settings or the shared `strata.jsonc` config file.
...
- [Settings Management](/docs/getting-started/settings) - Manage all your Strata Code preferences
