---
title: "Slack"
description: "Using StrataClaw with Slack"
---

# Slack

This page covers everything you need to use StrataClaw with Slack: connecting your bot, controlling who can DM it, and adding it to channels.

## Connecting StrataClaw to Slack

{% youtube url="https://youtu.be/Q5bt-qH-_pY" title="Slack Setup Guide" caption="How to connect your StrataClaw agent to Slack" /%}

Create a Slack app from the OpenClaw manifest and link it to your StrataClaw dashboard.
...
4. Customize the manifest before creating:
   ...
   - Update the slash command if desired (e.g., `/strataclaw`)
5. Click **Create**
...
### Step 3: Connect Slack to StrataClaw

1. In the [StrataClaw UI](https://app.strata.ai/claw), find the Slack integration section (may show "not configured")
2. Enter both tokens:
   - The `xapp-` app-level token
   - The `xoxb-` bot user OAuth token
3. Click **Save**
4. Scroll to the top of the StrataClaw UI and click **Redeploy**. Wait for the instance to come back up

### Step 4: Pair Slack with StrataClaw

1. In Slack, DM the app and send any message — this triggers the pairing flow
2. The app will return a pairing code
3. Return to [app.strata.ai/claw](https://app.strata.ai/claw) and confirm the pairing code and approve
...
By default, StrataClaw can respond to any DMs and will not respond in Slack channels, even if added.

## Making StrataClaw DM-Only (from you)

By default, StrataClaw will respond to DMs from any user in Slack.
...
## Adding StrataClaw to a Slack Channel

By default, StrataClaw will not respond in Slack channels, even if added. To have StrataClaw participate in a Slack channel:
...
Tell your StrataClaw agent (via DM):
