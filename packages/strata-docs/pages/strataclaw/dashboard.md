---
title: "StrataClaw Dashboard Reference"
description: "Managing your StrataClaw instance from the dashboard"
---

# StrataClaw Dashboard

This page covers everything you can do from the StrataClaw dashboard. For getting started, see [StrataClaw Overview](/docs/strataclaw/overview).

{% image src="/docs/img/strataclaw/dashboard.png" alt="Connect account screen" width="800" caption="The StrataClaw Dashboard" /%}
...
For access to the full catalog of 335+ models, use the `/model` and `/models` commands in the [Control UI Chat](/docs/strataclaw/control-ui#changing-models).

### Channels

You can connect Telegram, Discord, and Slack by entering bot tokens in the Settings tab. See [Connecting Chat Platforms](/docs/strataclaw/chat-platforms) for setup instructions.
...
See [Version Pinning](/docs/strataclaw/control-ui/version-pinning) for details.
...
> 💡 **Tip**
> If your instance is in a crash loop and you can't access the Control UI, try **Restore Config** from the StrataClaw dashboard first before redeploying.
...
When your instance is running you can access the [OpenClaw Control UI](/docs/strataclaw/control-ui) — a browser-based dashboard for managing your agent, channels, sessions, exec approvals, and more:

1. Click **Open** to launch the OpenClaw web interface in a new tab

See the [Control UI reference](/docs/strataclaw/control-ui) for a full overview of its capabilities.

{% callout type="warning" %}
Do not use the **Update** feature in the OpenClaw Control UI to update StrataClaw. Use **Redeploy** from the StrataClaw Dashboard instead. Updating via the Control UI will not apply the correct StrataClaw platform image and may break your instance.
{% /callout %}
...
You need to **approve** each request before the user or device can interact with your agent. See [Pairing Requests](/docs/strataclaw/chat-platforms#pairing-requests) for details.

## Changelog

The dashboard shows recent StrataClaw platform updates. Each entry is tagged as a **feature** or **bugfix**, and some include a deploy hint:
...
- [OpenClaw Control UI](/docs/strataclaw/control-ui)
