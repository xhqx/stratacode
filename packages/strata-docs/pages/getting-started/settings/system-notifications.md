...
description: "Configure native OS notifications for Strata Code"
platform: legacy
---
...
Strata Code uses system notifications to inform you about:
...
Strata Code's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. Strata Code uses macOS's built-in command to display notifications.
...
**How it works:** Strata Code first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
