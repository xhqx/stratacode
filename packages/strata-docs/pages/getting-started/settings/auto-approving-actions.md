...
The UI reads and writes to the same `strata.jsonc` config files used by the CLI, so changes made in either place are reflected in both.
...
When a tool is set to `"ask"`, Strata pauses and displays a permission prompt with two options:
...
The CLI uses a granular, per-tool permission system configured in `strata.jsonc`. Instead of broad categories like "read" or "write," each tool has its own permission level with glob-pattern rules for fine-grained control.
...
Permissions are configured under the `permission` key in `strata.jsonc`. The following tool-specific permission levels are available:
...
When a tool is set to `"ask"`, Strata pauses and displays a permission prompt. You have three options:
...
2. Select which actions Strata Code can perform without asking permission
3. Use the master toggle (leftmost checkbox) to quickly enable/disable all permissions

[![StrataCode Task Timeline](https://img.youtube.com/vi/NBccFnYDQ-k/maxresdefault.jpg)](https://youtube.com/shorts/NBccFnYDQ-k?feature=shared)
...
{% callout type="info" %}
Allow Strata Code to automatically perform operations without requiring approval. Enable these settings only if you fully trust the AI and understand the associated security risks.
{% /callout %}
...
**Description:** When enabled, Strata Code will automatically view directory contents and read files without requiring you to click the Approve button.

**Risk level:** Medium

While this setting only allows reading files (not modifying them), it could potentially expose sensitive data. Still recommended as a starting point for most users, but be mindful of what files Strata Code can access.

#### Read Outside Workspace

**Setting:** "Allow reading files outside the workspace"

**Description:** When enabled, Strata Code can read files outside the current workspace directory without asking for approval.

**Risk level:** Medium-High

This setting extends read permissions beyond your project folder. Consider the security implications:

- Strata Code could access sensitive files in your home directory
...
**Recommendation:** Keep disabled unless you specifically need Strata Code to read files outside your project.
...
This setting allows Strata Code to modify your files without confirmation. The delay timer is crucial:
...
**Description:** When enabled, Strata Code can create or modify files outside the current workspace directory without asking for approval.

**Risk level:** Very High

Use with caution and in controlled environments. It allows Strata Code to:
...
**Description:** When enabled, Strata Code can overwrite or modify files that are normally protected by the [`.stratacodeignore`](/docs/customize/custom-rules) file.
...
This setting allows Strata Code to permanently remove files without confirmation.

**Safeguards:**

- Strata Code still respects `.stratacodeignore` rules
...
Allows Strata Code to control a headless browser without confirmation. This can include:
...
The delay controls how long Strata Code waits before trying again:
...
Allows Strata Code to change between different modes (Code, Architect, etc.) without asking for permission. This primarily affects the AI's behavior rather than system access.
...
Enables Strata Code to create and complete subtasks automatically. This relates to workflow organization rather than system access.
...
Add command prefixes (e.g., `git`, `npm`, `ls`) that Strata Code can run without asking. Use `*` to allow all commands (use with caution).
...
**Description:** Automatically selects the first AI-suggested answer for a follow-up question after a configurable timeout. This speeds up your workflow by letting Strata Code proceed without manual intervention.
...
- Overnight runs where you want Strata Code to continue working
...
This setting allows Strata Code to automatically update task progress and todo lists during work sessions. This includes:
...
This is particularly useful when combined with the Subtasks permission, as it allows Strata Code to maintain a complete picture of project progress without constant approval requests.
...
**"You Only Live Once"** mode enables _all_ auto-approve permissions at once using the master toggle. This gives Strata Code complete autonomy to read files, write code, execute commands, and perform any operation without asking for permission.
...
This is the fastest way to work with Strata Code, but also the riskiest. Use it only when you fully trust the AI and are prepared for the consequences.
{% /callout %}
