---
title: "Settings"
description: "Configure Strata Code settings and preferences"
---
...
The fastest way to change your Strata configuration is to ask the agent to do it for you. The agent has a built-in skill that understands the full `strata.jsonc` schema and can read, create, and update your config files directly.
...
- **Global config:** `~/.config/strata/strata.jsonc` — applies to all projects. On Windows, this is `C:\Users\<username>\.config\strata\strata.jsonc`.
- **Project config:** `strata.jsonc` in your project root, or `.strata/strata.jsonc` for a cleaner setup. The `.strata/` version takes priority if both exist.
...
Config files are also plain-text and portable — you can copy `~/.config/strata/strata.jsonc` between machines directly.
...
{% callout type="warning" %}
If you check `strata.jsonc` into version control, make sure it does not contain API keys or other secrets (e.g., `provider.*.options.apiKey`). Use environment variables for credentials instead.
{% /callout %}
...
- **Global config:** `~/.config/strata/strata.jsonc` -- applies to all projects. On Windows, this is `C:\Users\<username>\.config\strata\strata.jsonc`.
- **Project config:** `strata.jsonc` in the root of your project -- overrides global settings for that project.
...
1. **Legacy Stratacode** -- migrated settings from the VSCode extension
2. **Remote well-known** -- remotely fetched defaults
3. **Global** -- `~/.config/strata/strata.jsonc`
4. **Custom** -- additional custom config paths
5. **Project** -- `strata.jsonc` in the project root
6. **`.strata` directory** -- config from a `.strata/` directory in the project
...
When you create or open a `strata.jsonc` file, the CLI automatically injects a `$schema` property pointing to the config JSON schema. This gives you **autocompletion and validation** in any editor that supports JSON Schema (VS Code, JetBrains, etc.).

### Export and Import

There is no traditional export/import of settings -- the JSONC config files themselves are portable. Copy `~/.config/strata/strata.jsonc` or `strata.jsonc` to another machine and you're done.

For **session** export and import, use the CLI commands:

- `strata export` -- export session data
- `strata import` -- import session data

{% /tab %}
{% tab label="VSCode (Legacy)" %}

Strata Code allows you to manage your configuration settings effectively through export, import, and reset options. These features are useful for backing up your setup, sharing configurations with others, or restoring default settings if needed.

You can find these options at the bottom of the Strata Code settings page, accessible via the gear icon ({% codicon name="gear" /%}) in the Strata Code chat view.

{% image src="/docs/img/settings-management/settings-management.png" alt="Export, Import, and Reset buttons in Strata Code settings" width="800" caption="Export, Import, and Reset buttons" /%}

### Export Settings

Clicking the **Export** button saves your current Strata Code settings to a JSON file.
...
- **Process:**
  1.  Click **Export**.
  2.  A file save dialog appears, suggesting `strata-code-settings.json` as the filename (usually in your `~/Documents` folder).
...
- **Process:**
  1.  Click **Import**.
  2.  A file open dialog appears. Select the `strata-code-settings.json` file (or similarly named file) you want to import.
  3.  Strata Code reads the file, validates its contents against the expected schema, and applies the settings.
...
Clicking the **Reset** button completely clears all Strata Code configuration data and returns the extension to its default state. This is a destructive action intended for troubleshooting or starting fresh.
...
- **What is Reset:**
  ...
  - **Secret Storage:** All API keys and other secrets managed by Strata Code are cleared.
  - **Task History:** The current task stack is cleared.

- **Result:** Strata Code returns to its initial state, as if freshly installed, with default settings and no user configurations.

Use this option only if you are certain you want to remove all Strata Code data or if instructed during troubleshooting. Consider exporting your settings first if you might want to restore them later.
...
Advanced options not exposed in the UI can be configured via the `experimental` key in `strata.jsonc`:
...
Refer to the auto-generated `$schema` in your `strata.jsonc` for the full list of available options.

{% /tab %}
{% tab label="CLI" %}

The CLI does not currently expose the same experimental feature toggles as the **VSCode (Legacy)** version. Configuration of model behavior, file editing strategies, and other advanced options is handled directly in the JSONC config files. Refer to the auto-generated `$schema` in your `strata.jsonc` for the full list of available options.

{% /tab %}
{% tab label="VSCode (Legacy)" %}

{% callout type="info" %}
These features are experimental and may change in future releases. They provide advanced control over Strata Code's behavior for specific use cases.
{% /callout %}

### Concurrent File Edits

When enabled, Strata Code can edit multiple files in a single request. When disabled, Strata Code must edit one file at a time.
...
When enabled, Strata Code will remind the model about the details of its current mode definition more frequently. This leads to stronger adherence to role definitions and custom instructions, but will use more tokens per message.
...
You can find this setting in the Strata Code settings under 'Advanced Settings'.
