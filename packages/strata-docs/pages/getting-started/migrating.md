...
description: "Guide for migrating to Strata Code from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to Strata Code. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

Strata Code supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why Strata Code's Rules System?

Strata Code simplifies AI configuration while adding powerful new capabilities:
...
**2. Create Strata Code directory:**

```bash
mkdir -p .stratacode/rules
```
...
for file in .cursor/rules/*.mdc; do
  basename="${file##*/}"
  cp "$file" ".stratacode/rules/${basename%.mdc}.md"
done
...
- Save to `~/.stratacode/rules/cursor-global.md`

**6. Handle legacy `.cursorrules`:**

```bash
cp .cursorrules .stratacode/rules/legacy-rules.md
```

### Converting Cursor's `globs` Patterns

Cursor's `globs` field specifies which files a rule applies to. Strata Code handles this through **mode-specific directories** instead.
...
**Strata Code approach (Option 1 - Mode-specific directory):**

```bash
mkdir -p .stratacode/rules-code
# Save TypeScript-specific rules here
```

**Strata Code approach (Option 2 - Custom mode):**

```yaml
# .stratacodemodes (at project root)
- slug: typescript
  name: TypeScript
...
Then place rules in `.stratacode/rules-typescript/`

### Flattening Nested Cursor Rules

Cursor supports nested `.cursor/rules/` directories. Strata Code uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# Strata Code: .stratacode/rules/backend-server-api-rules.md
```
...
```bash
cp .windsurf/rules/*.md .stratacode/rules/
```

**4. Migrate global rules:**

```bash
cp ~/.codeium/windsurf/memories/global_rules.md ~/.stratacode/rules/global-rules.md
```

**5. Handle legacy `.windsurfrules`:**

```bash
cp .windsurfrules .stratacode/rules/legacy-rules.md
```
...
# .stratacode/rules/api-conventions.md
# .stratacode/rules/testing-standards.md
# .stratacode/rules/code-style.md
```
...
Then place the rule in `.stratacode/rules-test/`

## AGENTS.md Support

All three tools support the `AGENTS.md` standard. If you have one, it works in Strata Code automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - Strata Code loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). Strata Code also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in Strata Code and require user approval to modify.

## Understanding Mode-Specific Rules

This is Strata Code's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.

### Directory Structure

```bash
.stratacode/rules/              # Apply to ALL modes
.stratacode/rules-code/         # Only in Code mode
.stratacode/rules-debug/        # Only in Debug mode
.stratacode/rules-ask/          # Only in Ask mode
.stratacode/rules-{custom}/     # Only in your custom mode
```
...
mkdir -p .stratacode/rules-test

# 2. Save rule as plain Markdown
cat > .stratacode/rules-test/testing-standards.md << 'EOF'
# Testing Rules
- Write tests for all features
- Maintain >80% coverage
EOF
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in Strata Code panel
- [ ] **Test rule application:** Ask Strata Code to perform tasks following your rules
- [ ] **Organize rules:** Split large files, use clear names
- [ ] **Set up mode-specific rules:** Create directories for specialized workflows
- [ ] **Update team docs:** Document new `.stratacode/rules/` location
- [ ] **Commit to version control:** `git add .stratacode/`
...
```bash
ls -la .stratacode/rules/      # Project rules
ls -la ~/.stratacode/rules/    # Global rules
```
...
- **For always-on rules:** Place in `.stratacode/rules/`
...
- **After migrating:** Organize files accordingly in Strata Code

### Nested Rules Flattened

Cursor's nested directories don't map to Strata Code. Flatten with descriptive names:

```bash
# Bad: .cursor/rules/backend/api/rules.mdc
# Good: .stratacode/rules/backend-api-rules.md
```
...
- **Check setting:** Verify "Use Agent Rules" is enabled in Strata Code settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

Strata Code supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
```bash
mkdir -p .stratacode/rules-review
mkdir -p .stratacode/rules-docs
```

**Note:** `.stratacodemodes` can be in YAML (preferred) or JSON format. For global modes, edit the `custom_modes.yaml` file via Settings > Edit Global Modes.
...
- [Join our Discord](https://strata.ai/discord) for migration support
