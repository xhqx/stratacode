...
description: "Use your own API keys with Strata Gateway while retaining platform features"
---

# Bring Your Own Key (BYOK)

Bring Your Own Key (BYOK) lets you use your own API keys when using the Strata Gateway, while retaining Strata platform features like Code Reviews and Cloud Agents.

A user or organization may want to use BYOK to:

- Utilize new models quickly, Strata Gateway supports most new models in minutes
...
Strata Gateway currently supports BYOK keys for these providers:
...
1. Log into the Strata platform and select the account or organization you want to add the BYOK key to.
...
- When you use the **Strata Gateway** provider, Strata checks if there's a BYOK key for the selected model's provider.
- If a matching BYOK key exists, the request is routed using your key.
- If the key is invalid, the request fails. It does not fall back to using Strata's keys.

## Using BYOK in the Extensions and CLI

- BYOK works with the Strata Gateway provider. Users should ensure that is set as the active [provider](/docs/ai-providers).
