...
description: "How the Strata CLI config JSON Schema is served at app.strata.ai/config.json"
---

# CLI Config Schema

The JSON Schema referenced by `"$schema": "https://app.strata.ai/config.json"` in `strata.json` files is served by the cloud repo. It is a runtime overlay of the upstream opencode schema with Strata-specific additions on top.

## Flow

1. Client fetches `https://app.strata.ai/config.json`.
...
3. `merge()` overlays three sections from `apps/web/src/app/config.json/extras.ts`:
   ...
   - `agents` — Strata primary agents (`ask`, `debug`, `orchestrator`)
   - `experimental` — `codebase_search`, `openTelemetry`

## Adding a new Strata-only config key

The source of truth is the zod schema in `packages/opencode/src/config/config.ts`. The cloud overlay must match it.

1. Add the zod field with a `stratacode_change` marker in `config.ts`.
2. Generate the JSON Schema shape: `bun --bun packages/opencode/script/schema.ts /tmp/strata.json`, then `jq '.properties.<new_key>' /tmp/strata.json`.
3. Paste the shape into the correct bucket in `apps/web/src/app/config.json/extras.ts` in the [cloud repo](https://github.com/Strata-Org/cloud).
...
If step 3 is skipped, users with `$schema: https://app.strata.ai/config.json` will see "unknown property" warnings for the new key.
