...
description: "Overview of the Strata platform architecture"
---

# Architecture Overview

This document provides a high-level overview of the Strata platform architecture to help contributors understand how the different components fit together.

## System Architecture

Strata is an AI coding platform built around a central CLI engine that powers every client surface — the terminal, VS Code, and the cloud. The architecture follows a layered approach where all clients communicate with the CLI over HTTP + SSE, and the CLI connects to AI providers either directly or through Strata Cloud.
...
graph LR
  ...
  subgraph cli ["Strata CLI Engine"]
    provider["Provider Router"]
  end

  subgraph cloud ["Strata Cloud"]
    gateway["Strata Gateway"]
...
## Strata CLI — The Foundation

The CLI (`packages/opencode/`) is the core engine that all products are built on. It contains the AI agent runtime, tool execution, session management, provider integrations, and an HTTP server. Each client spawns or connects to a `strata serve` process and communicates via HTTP + SSE using the `@stratacode/sdk`.

The CLI can run in several modes:

- **`strata`** — Interactive TUI for terminal-based coding
- **`strata run`** — Headless single-prompt execution
- **`strata serve`** — HTTP server mode for client integrations
...
The VS Code extension (`packages/strata-vscode/`) bundles the CLI binary and spawns `strata serve` as a child process. It includes:
...
Strata Cloud is the hosted platform layer that provides authentication, provider routing, and autonomous agent services. The cloud infrastructure lives in a separate repository.

### Strata Gateway

The gateway (`packages/strata-gateway/` in this repo, plus API routes in the cloud) handles:

- **Authentication** — Device flow auth, token management, and account linking
- **Provider Routing** — Routes AI requests through Strata's managed API keys or the user's own keys
...
A Cloudflare Worker within Strata Cloud that runs the Strata CLI in isolated sandbox environments. It powers cloud-based AI coding tasks triggered via the web dashboard, webhooks, or automation workflows. It provides a secure API for:
...
{% image src="/docs/img/strataclaw/strataclaw-architecture.png" alt="StrataClaw infrastructure architecture diagram" width="800" caption="StrataClaw infrastructure architecture" /%}
...
A multi-agent orchestration platform that coordinates autonomous AI coding agents working on real Git repositories. Gas Town runs entirely on Cloudflare — a central Durable Object manages all state, while Docker containers on Cloudflare Containers run agent processes via the Strata CLI.
...
All clients communicate with the CLI via its HTTP + SSE API. The `@stratacode/sdk` package provides a TypeScript client:

```typescript
import { StrataClient } from "@stratacode/sdk"

const client = new StrataClient({ baseUrl: "http://localhost:3000" })
