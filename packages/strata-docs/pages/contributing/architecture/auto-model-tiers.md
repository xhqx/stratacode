...
description: "Architecture of Strata Auto — a family of smart model tiers that match users to the right models without requiring AI expertise"
---
...
Strata Auto is a model routing system that automatically selects the optimal AI model based on the user's current mode (Code, Architect, Debug, etc.). It comes in multiple tiers so that every user — regardless of budget, preference, or expertise — gets a "just works" experience without needing to understand the AI model landscape.
...
| `strata-auto/small`    | Internal — background tasks    | Varies  |
...
3. **Background tasks** — Strata uses small models for things like generating session titles and commit messages. These should be invisible and reliable, not dependent on the user's model selection or credit status.
...
**Who it's for**: Users who want to try Strata without a credit card, students, hobbyists, and anyone exploring AI-assisted coding.
...
**Who it's for**: Not user-facing. Used internally by Strata for lightweight background tasks (session titles, commit messages, conversation summaries).
...
**Implementation**: The `getSmallModel()` function in `packages/opencode/src/provider/provider.ts` prioritizes `strata-auto/small` when the Strata provider is active. If the user's provider doesn't have a dedicated small model, it falls back globally to `strata-auto/small` when available.
...
Auto: Small does not appear in the model picker. It is filtered out by the UI (see `STRATA_AUTO_SMALL_IDS` in the VS Code extension).

### Defaults

- **Authenticated users**: Default to `strata-auto/balanced` (defined in `packages/strata-gateway/src/api/constants.ts`)
- **Unauthenticated users**: Default to `strata-auto/free`
...
Auto Model uses a split client/server architecture. The actual model-to-mode mappings are not hardcoded in the client — they're served dynamically from the Strata API, making it possible to update routing without client releases.

### Server side (Strata API)

The Strata API at `api.strata.ai` defines which underlying models each `strata-auto/*` tier routes to per mode. Each auto model is returned with an `opencode.variants` field — a map of mode-specific provider options:
...
This is fetched via `packages/strata-gateway/src/api/models.ts` which parses the `opencode.variants` field from the API response.

### Client side

The client-side chain works as follows:

1. **Model fetching**: `packages/opencode/src/provider/model-cache.ts` caches Strata Gateway models with a 5-minute TTL, fetching from the Strata API.

2. **Variant passthrough**: `packages/opencode/src/provider/transform.ts` — the `variants()` function passes through server-defined variants for Strata Gateway models directly, rather than computing them locally.

3. **Variant storage**: `packages/opencode/src/provider/provider.ts` stores `variants` on the model object when the provider is `strata`.
...
- Unauthenticated users default to `strata-auto/free` with no configuration required
