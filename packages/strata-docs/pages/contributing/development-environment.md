...
{% callout type="info" %}
**New versions of the VS Code extension and CLI are being developed in [Strata-Org/stratacode](https://github.com/Strata-Org/stratacode)** (extension at `packages/strata-vscode`, CLI at `packages/opencode`). For extension and CLI development, please head over to that repository.
{% /callout %}
...
1. **Fork and Clone the Repository**:
   - **Fork the Repository**:
     - Visit the [Strata Code GitHub repository](https://github.com/Strata-Org/stratacode)
...
2. This will open a new VSCode window with Strata Code loaded
...
```bash
code --install-extension "$(ls -1v bin/strata-code-*.vsix | tail -n1)"
```
...
Strata Code uses several types of tests to ensure quality:
...
- Check the Output panel in VSCode (View > Output) and select "Strata Code" from the dropdown
