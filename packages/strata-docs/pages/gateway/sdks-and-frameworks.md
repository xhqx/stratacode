...
description: "Integrate with the Strata AI Gateway using the Vercel AI SDK, OpenAI SDK, Python, cURL, or any OpenAI-compatible client."
---

# SDKs & Frameworks

The Strata AI Gateway is OpenAI-compatible, meaning any SDK or framework that works with the OpenAI API can work with the Strata Gateway by changing the base URL.
...
const result = streamText({
  model: strata.chat("anthropic/claude-sonnet-4.5"),
...
The official OpenAI SDKs work with the Strata Gateway by setting the base URL.
...
```bash
curl -N -X POST "https://api.strata.ai/api/gateway/chat/completions" \
  -H "Authorization: Bearer $STRATA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "anthropic/claude-sonnet-4.5",
...
func main() {
    ...
    req, _ := http.NewRequest("POST",
        "https://api.strata.ai/api/gateway/chat/completions",
        bytes.NewBuffer(jsonBody))

    req.Header.Set("Authorization", "Bearer "+os.Getenv("STRATA_API_KEY"))
...
uri = URI('https://api.strata.ai/api/gateway/chat/completions')
http = Net::HTTP.new(uri.host, uri.port)
http.use_ssl = true

request = Net::HTTP::Post.new(uri)
request['Authorization'] = "Bearer #{ENV['STRATA_API_KEY']}"
...
The Strata AI Gateway works with any framework that supports OpenAI-compatible APIs:
...
const model = new ChatOpenAI({
  ...
  openAIApiKey: process.env.STRATA_API_KEY,
  configuration: {
    baseURL: "https://api.strata.ai/api/gateway",
  },
})
