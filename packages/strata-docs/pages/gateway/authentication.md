...
description: "Learn how to authenticate with the Strata AI Gateway using API keys, session tokens, and Bring Your Own Key (BYOK)."
---

# Authentication

The Strata AI Gateway supports multiple authentication methods depending on your use case.
...
API keys are JWT tokens tied to your Strata account. See [how to get your API key](/docs/getting-started/setup-authentication#strata-gateway-api-key) for step-by-step instructions.
...
const strata = createOpenAI({
  baseURL: "https://api.strata.ai/api/gateway",
...
client = OpenAI(
    api_key=os.getenv("STRATA_API_KEY"),
    base_url="https://api.strata.ai/api/gateway",
)
```
...
```bash
curl -X POST "https://api.strata.ai/api/gateway/chat/completions" \
...
```
X-StrataCode-OrganizationId: your_org_id
```
...
BYOK lets you use your own provider API keys with the Strata AI Gateway. When a BYOK key is configured, requests are sent to the provider using your key. You are billed directly by the provider -- Strata does not add any markup.
...
1. Add your provider API key in the [Strata dashboard](https://app.strata.ai) or through your Strata Code extension settings
...
4. Usage is tracked but not billed to your Strata balance (cost is set to $0)
5. If your BYOK key fails, the request will not automatically fall back to Strata's keys
