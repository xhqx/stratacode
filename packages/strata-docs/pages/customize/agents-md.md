...
The Strata Code **memory bank** feature has been deprecated in favor of AGENTS.md.
...
1. Examine the contents in `.stratacode/rules/memory-bank/`
2. Move that content into your project's `AGENTS.md` file (or ask Strata to do it for you)
   {% /callout %}

## What is AGENTS.md?

AGENTS.md is an open standard for configuring AI agent behavior in software projects. It's a simple Markdown file placed at the root of your project that contains instructions for AI coding assistants. The standard is supported by multiple AI coding tools, including Strata Code, Cursor, and Windsurf.
...
When working in a subdirectory, Strata Code will load both the root AGENTS.md and any subdirectory AGENTS.md files, with subdirectory files taking precedence for conflicting instructions.

## File Protection

Both `AGENTS.md` and `AGENT.md` are **write-protected files** in Strata Code. This means:
...
## How AGENTS.md Works in Strata Code

### Loading Behavior

When you start a task in Strata Code:

1. Strata Code checks for `AGENTS.md` or `AGENT.md` at the project root
...
```bash
export STRATA_DISABLE_EXTERNAL_SKILLS=true
```
...
```json
{
  "stratacode.useAgentRules": false
}
```
...
- **[Custom Rules](/docs/customize/custom-rules)** - Strata Code-specific rules with more control
