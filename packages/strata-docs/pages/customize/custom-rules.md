...
Strata Code supports two types of custom rules:
...
Project rules are configured via the `instructions` key in your project's `strata.jsonc` file. You can edit this file directly or use the **Settings** webview to manage the `instructions` configuration. Each entry points to a file path or glob pattern:

```jsonc
// strata.jsonc
{
  "instructions": [".strata/rules/formatting.md", ".strata/rules/*.md"],
}
```

You can also place rule files in the **`.strata/`** directory structure:
...
Global rules are configured via the `instructions` key in your global `strata.jsonc` config file (typically at `~/.config/strata/strata.jsonc`).

{% callout type="note" title="Migration" %}
The extension is backward compatible with `.stratacode/rules/` directories. Existing rules will continue to work, but migrating to `strata.jsonc` is recommended.
{% /callout %}
...
Project rules are configured via the `instructions` key in your project's `strata.jsonc` file. Each entry points to a file path or glob pattern:
...
{% callout type="note" title="Migration" %}
The CLI is backward compatible with `.stratacode/rules/` directories. Existing rules will continue to work, but migrating to `strata.jsonc` is recommended.
{% /callout %}
...
Custom rules are primarily loaded from the **`.stratacode/rules/` directory**. This is the recommended approach for organizing your project-specific rules. Each rule is typically placed in its own Markdown file with a descriptive name:
...
Rules are managed by editing the `instructions` array in your `strata.jsonc` config file. You can also use the **Settings** webview in VS Code to edit the configuration.

- **Add a rule**: Add a file path or glob pattern to the `instructions` array
- **Remove a rule**: Remove the entry from the array
- **Disable a rule temporarily**: Comment out the line in `strata.jsonc` (JSONC supports `//` comments)

```jsonc
// strata.jsonc
{
  "instructions": [
    ".strata/rules/formatting.md",
    // ".strata/rules/experimental.md"  -- temporarily disabled
    ".strata/rules/naming_conventions.md",
  ],
}
```
...
Rules are managed by editing the `instructions` array in your `strata.jsonc` config file directly.
...
Strata Code provides a built-in interface for managing your custom rules without manually editing files in the `.stratacode/rules/` directories. To access the UI, click on the <Codicon name="law" /> icon in the **bottom right corner** of the Strata Code window.
...
Rules are loaded in the order they appear in the `instructions` array in `strata.jsonc`:

1. **Global instructions** from the global `strata.jsonc` config
2. **Project instructions** from the project's `strata.jsonc`
...
If `.stratacode/rules/` directories exist in your project, their contents are automatically included for backward compatibility. To fully migrate, move your rule files and reference them in `strata.jsonc`.
{% /callout %}
...
1. **Global rules** from `~/.stratacode/rules/` directory
2. **Project rules** from `.stratacode/rules/` directory
3. **Legacy fallback files** (for backward compatibility):
   - `.roorules`
   - `.clinerules`
   - `.stratacoderules` (deprecated)
...
{% callout type="note" %}
We strongly recommend keeping your rules in the `.stratacode/rules/` folder as it provides better organization and is the preferred approach for future versions. The legacy file-based approach is maintained for backward compatibility but may be subject to change in future releases.
{% /callout %}
...
1. First, it checks for `.stratacode/rules-${mode}/` directory
2. If that doesn't exist or is empty, it falls back to `.stratacoderules-${mode}` file (deprecated)
...
1. Create a `strata.jsonc` file in your project root (if it doesn't exist)
2. Create a `.strata/rules/` directory (or any directory you prefer)
3. Write your rule as a Markdown file in that directory
4. Add the file path or a glob pattern to the `instructions` array in `strata.jsonc`

```jsonc
// strata.jsonc
{
  "instructions": [".strata/rules/my-new-rule.md"],
}
```

Rules are applied on the next interaction. You can also edit `strata.jsonc` through the **Settings** webview in VS Code.
...
{% image src="/docs/img/custom-rules/rules-ui.png" alt="Rules tab in Strata Code" width="400" /%}

The easiest way to create and manage rules is through the built-in UI:

1. Access the rules management interface from the Strata Code panel
...
1. Create the `.stratacode/rules/` directory if it doesn't already exist
...
Rules will be automatically applied to all future Strata Code interactions. Any new changes will be applied immediately.
...
{% image src="/docs/img/custom-rules/custom-rules.png" alt="Strata Code ignores request to read sensitive file" width="600" /%}
...
3. **Check rule locations**:
   ...
   - Ensure rules are in supported locations:
     - Global rules: `~/.stratacode/rules/` directory
     - Project rules: `.stratacode/rules/` directory
     - Legacy files: `.stratacoderules`, `.roorules`, or `.clinerules`
