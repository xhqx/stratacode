...
Strata Code includes two built-in subagents:
...
Add agents to the `agent` section of your `strata.jsonc` config file. Any key that doesn't match a built-in agent name creates a new custom agent.
...
- **Global**: `~/.config/strata/agents/`
- **Project-specific**: `.strata/agents/`
...
2. **Global config** (`~/.config/strata/config.json`)
3. **Project config** (`strata.jsonc` in the project root)
4. **Global agent markdown files** (`~/.config/strata/agents/*.md`)
5. **Project agent markdown files** (`.strata/agents/*.md`)
