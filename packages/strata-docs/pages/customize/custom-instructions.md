...
description: "Provide custom instructions to guide Strata Code"
---

# Custom Instructions

Custom Instructions allow you to personalize how Strata Code behaves, providing specific guidance that shapes responses, coding style, and decision-making processes. Both the **VSCode** and **CLI** versions support custom instructions, though the mechanisms differ.

## What Are Custom Instructions?

Custom Instructions define specific Extension behaviors, preferences, and constraints beyond Strata's basic role definition. Examples include coding style, documentation standards, testing requirements, and workflow guidelines.
...
Strata automatically discovers instruction files at your project root and in parent directories (via `findUp`). The following filenames are recognized:

- **`AGENTS.md`** — The primary instruction file for Strata
...
- **Strata:** `~/.config/strata/AGENTS.md`
...
The `instructions` key in `strata.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. You can manage these in **Settings → Agent Behaviour → Rules** subtab.
...
## Legacy `.stratacoderules` Support

If your project contains `.stratacoderules` files from the VSCode extension, these are still loaded via auto-migration. However, migrating to `AGENTS.md` is recommended for new projects.
...
Each agent can have its own custom prompt defined in its `.md` file (the markdown body) or via the `agent.<name>.prompt` key in `strata.jsonc`:
...
Or as the markdown body in `.strata/agents/code.md`:
...
The `instructions` key in `strata.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. Configure these in your `strata.jsonc`:
...
{% image src="/docs/img/custom-instructions/custom-instructions.png" alt="Strata Code Modes tab showing global custom instructions interface" width="600" caption="Strata Code Modes tab showing global custom instructions interface" /%}

1.  **Open Modes Tab:** Click the <Codicon name="notebook" /> icon in the Strata Code top menu bar
...
Mode-specific instructions can be set using the Modes Tab
    ...
    {% image src="/docs/img/custom-instructions/custom-instructions-3.png" alt="Strata Code Modes tab showing mode-specific custom instructions interface" width="600" caption="Strata Code Modes tab showing mode-specific custom instructions interface" /%}
    * **Open Tab:** Click the <Codicon name="notebook" /> icon in the Strata Code top menu bar
...
- Preferred: `.strata/rules-{mode-slug}/` (directory)
- Fallback: `.stratacoderules-{mode-slug}` (single file)

{% callout type="info" title="Legacy Naming Note" %}
Only `.stratacoderules-{mode-slug}` is recognized as the legacy fallback. Older naming like `.clinerules-{mode-slug}` is not supported.
{% /callout %}
