---
title: "Skills"
description: "Extend Strata Code capabilities with skills"
---

# Skills

Strata Code implements [Agent Skills](https://agentskills.io/home), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
## How Skills Work in Strata Code
...
1. **Discovery**: Skills are scanned from designated directories when Strata Code initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.strata` directory within your Home directory:

- Mac and Linux: `~/.strata/skills/`
- Windows: `\Users\<yourUser>\.strata\skills\`
...
Located in `.strata/skills/` within your project:
...
You can configure extra skill locations and remote skill URLs in your `strata.jsonc` config (project or global):
...
Global skills are located in the `.stratacode` directory within your Home directory.

- Mac and Linux: `~/.stratacode/skills/`
- Windows: `\Users\<yourUser>\.stratacode\`
...
Located in `.stratacode/skills/` within your project:
...
```bash
# For Code mode only
mkdir -p ~/.stratacode/skills-code/typescript-patterns

# For Architect mode only
mkdir -p ~/.stratacode/skills-architect/microservices
```
...
When multiple skills share the same name, project-level skills (`.strata/skills/`) take precedence over global skills (`~/.strata/skills/`). Skills from compatibility directories (`.claude/skills/`, `.agents/skills/`) and additional configured paths are loaded alongside project and global skills.
...
When multiple skills share the same name, Strata Code uses these priority rules:
...
- In the **CLI**: Skills are loaded when you start a new session or run `strata run`
...
Skills are discovered when Strata Code initializes:

- When VSCode starts
- When you reload the VSCode window (`Cmd+Shift+P` → "Developer: Reload Window")

Skills directories are monitored for changes to `SKILL.md` files. However, the most reliable way to pick up new skills is to reload VS or the Strata Code extension.
...
In Strata Code, the `name` field **must match** the parent directory name:
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.strata/skills/api-design
   ```
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.stratacode/skills/api-design
   ```
...
- **[Strata Marketplace repository](https://github.com/Strata-Org/strata-marketplace)** — Browse community skills on GitHub and manually download them into your skills directory
...
- **Strata Marketplace** — Browse skills directly in the Strata Code extension via the Marketplace tab, or explore the [Strata Marketplace repository](https://github.com/Strata-Org/strata-marketplace) on GitHub
- [Agent Skills Specification](https://agentskills.io/home) — The open specification that skills follow, enabling interoperability across different AI agents
...
3. **Check file location**: Ensure `SKILL.md` is directly inside the skill directory (e.g., `.strata/skills/my-skill/SKILL.md`), not nested further.

4. **Check config paths**: If using `skills.paths` or `skills.urls`, verify the paths and URLs are correct in your `strata.jsonc`.
...
1. **Check the Output panel**: Open `View` → `Output` → Select "Strata Code" from dropdown. Look for skill-related errors.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [Strata Marketplace](https://github.com/Strata-Org/strata-marketplace)!

{% tabs %}
{% tab label="VSCode" %}

While the new platform does not yet have a built-in marketplace UI, skills from the [Strata Marketplace repository](https://github.com/Strata-Org/strata-marketplace) can be manually downloaded into your `.strata/skills/` directory or loaded via `skills.urls` in config.
...
3. **Fork the marketplace repository**: Visit [github.com/Strata-Org/strata-marketplace](https://github.com/Strata-Org/strata-marketplace) and create a fork
...
- Follow the [contribution guidelines](https://github.com/Strata-Org/strata-marketplace/blob/main/CONTRIBUTING.md) in the marketplace repository

For more details on contributing to Strata Code, see the [Contributing Guide](/docs/contributing).
