---
title: "Custom Modes"
description: "Create and configure custom modes in Strata Code"
---

# Custom Modes

Strata Code allows you to create **custom modes** (also called **agents**) to tailor Strata's behavior to specific tasks or workflows. Custom modes can be either **global** (available across all projects) or **project-specific** (defined within a single project).

{% callout type="info" %}
The current VS Code extension (built on the Strata CLI) uses **agent Markdown files** to define custom modes. The legacy extension used `custom_modes.yaml` / `.stratacodemodes`. See the tabs below for the relevant approach.
{% /callout %}
...
### 1. Ask Strata! (Recommended)

Ask Strata to create an agent for you:

```
Create a new agent called "docs-writer" that can only read files and edit Markdown files.
```

Strata will generate the agent definition and write it to `.strata/agent/` in your project.
...
```
.strata/agents/my-agent.md
.strata/agent/my-agent.md
...
```
~/.config/strata/agent/my-agent.md
```
...
**Example agent file** (`.strata/agents/docs-writer.md`):
...
### 4. Config File (`strata.jsonc`)

Define agents under the `agent` key in your project's `strata.jsonc`:
...
2. Global config (`~/.config/strata/strata.jsonc`)
3. Project config (`strata.jsonc` at project root)
4. `.strata/` / `.opencode/` directory configs and agent `.md` files
5. Environment variable overrides (`STRATA_CONFIG_CONTENT`)
...
```jsonc
// strata.jsonc — override the built-in "code" agent
{
  "agent": {
    "code": {
      "model": "openai/gpt-4o",
...
Or as a `.md` file (`.strata/agents/code.md`):
...
If you have existing `.stratacodemodes` or `custom_modes.yaml` files from the VSCode extension, the extension automatically migrates them on startup. The migration converts:
...
**Legacy `custom_modes.yaml` is not loaded from `~/.config/strata/`.** If you're migrating from the legacy VSCode extension, global custom modes are read from `~/.stratacode/cli/global/settings/custom_modes.yaml` (not from the CLI's XDG config directory). The recommended approach is to convert legacy modes to agent `.md` files and place them in `~/.config/strata/agent/` instead — see [Markdown files](#3-markdown-files-with-yaml-frontmatter) and [Migration](#migration-from-vscode-extension-modes) below.
{% /callout %}
...
The TUI also **remembers the last model you picked for each agent** across sessions. A config-pinned `model` acts as the default when no manual pick exists. To reset a pick and let the config take over, use the model picker (`Ctrl+X m`) and select a different model, or remove the saved pick from `~/.local/state/strata/model.json`.
...
If you have existing `.stratacodemodes` or `custom_modes.yaml` files from the VSCode extension, the CLI automatically migrates them on startup. The migration converts:
...
{% image src="/docs/img/custom-modes/custom-modes-2.png" alt="Custom mode creation interface in Strata Code" width="600" caption="Custom mode creation interface in Strata Code" /%}

_Strata Code's interface for creating and managing custom modes._
...
Modes are managed from the Modes area in Strata Code. Depending on your UI layout, you can open this from the mode selector in the chat panel or from the notebook icon.
...
5. Strata packages the mode's configuration and any rules into the YAML file
...
4. Choose the import level:
   - **Project:** Available only in current workspace (saved to `.stratacodemodes` file)
...
- **Project agents:** `.strata/agents/*.md` (or `.opencode/agents/*.md`)
- **Global agents:** `~/.config/strata/agents/*.md`
...
### Ask Strata! (Recommended)

You can also have Strata create an agent file for you. For example:

```
Create a new agent called "Documentation Writer". It should only be able to read files and write Markdown files.
```

Strata will create the appropriate `.strata/agents/docs-writer.md` file with the right frontmatter.
...
You can quickly create a basic custom mode by asking Strata Code to do it for you. For example:

```
Create a new mode called "Documentation Writer". It should only be able to read files and write Markdown files.
```

Strata Code will guide you through the process, prompting for necessary information and creating the mode using the preferred YAML format.
...
The interface provides fields for Name, Slug, Description, Save Location, Role Definition, When to Use (optional), Available Tools, and Custom Instructions. After filling these, click the "Create Mode" button. Strata Code will save the new mode in YAML format.

### 3. Manual Configuration (YAML & JSON)

You can directly edit the configuration files to create or modify custom modes. This method offers the most control over all properties. Strata Code now supports both YAML (preferred) and JSON formats.

- **Global Modes:** Edit `custom_modes.yaml` (primary). `custom_modes.json` is a legacy fallback and may still exist in older setups.
- **Project Modes:** Edit `.stratacodemodes` in your project root (YAML preferred; JSON still supported for compatibility).
...
If you see both YAML and JSON mode files, this is usually from legacy configuration. Strata Code reads YAML first and does not keep both files synchronized line-by-line. In practice, edit YAML unless you have a specific reason to stay on JSON.
{% /callout %}
...
- **Usage:** Used internally and in file/directory names for mode-specific rules (e.g., `.strata/rules-{slug}/`)
...
- **Purpose:** The display name shown in the Strata Code UI
...
- **Purpose:** Provides guidance for Strata's automated decision-making, particularly for mode selection and task orchestration
- **Format:** A string describing ideal scenarios or task types for this mode
- **Usage:** Used by Strata for automated decisions and not displayed in the mode selector UI
...
While JSON is still fully supported, new modes created via the UI or by asking Strata will default to YAML.
...
### Project Modes (`.stratacodemodes`)

- No automatic startup migration occurs for project-specific files
- Strata Code can read `.stratacodemodes` files in either YAML or JSON format
- When editing through the UI, JSON files will be converted to YAML format
- For manual conversion, you can ask Strata to help reformat configurations
...
### Preferred Method: Directory (`.strata/rules-{mode-slug}/`)
...
### Fallback Method: Single File (`.stratarules-{mode-slug}`)

```
.
├── .stratarules-docs-writer  # Example for mode slug "docs-writer"
└── ... (other project files)
```

**Rules Directory Scope:**

- **Global modes:** Rules are stored in `~/.strata/rules-{slug}/`
- **Project modes:** Rules are stored in `{workspace}/.strata/rules-{slug}/`
...
1. **Project-level mode configurations** (from `.stratacodemodes` - YAML or JSON)
2. **Global mode configurations** (from `custom_modes.yaml`, then `custom_modes.json` if YAML not found)
3. **Default mode configurations**

**Important:** When modes with the same slug exist in both `.stratacodemodes` and global settings, the `.stratacodemodes` version completely overrides the global one for ALL properties.

## Overriding Default Modes

You can override Strata Code's built-in modes (like 💻 Code, 🪲 Debug, ❓ Ask, 🏗️ Architect, 🪃 Orchestrator) by creating a custom mode with the same slug.
...
**Let Strata Build Your Regex Patterns**

Instead of writing complex regex manually, ask Strata:

```
Create a regex pattern that matches JavaScript files but excludes test files
```

Strata will generate the pattern. Remember to adapt it for YAML (usually single backslashes) or JSON (double backslashes).
...
### Basic Documentation Writer (`.strata/agents/docs-writer.md`)
...
### Test Engineer (`.strata/agents/test-engineer.md`)
...
### Security Reviewer (`.strata/agents/security-review.md`)
...
### Config File Example (`strata.jsonc`)
...
- **Agent not appearing:** Ensure the `.md` file is in a recognized directory (`.strata/agents/`, `.strata/agent/`, `.opencode/agents/`). Check that the `mode` property is `primary` or `all` if you expect it in the agent picker.
...
- **Legacy modes are auto-migrated:** If you have `.stratacodemodes` files, they'll be converted on startup — no manual migration needed
...
- **Keep prompts focused:** The markdown body is your system prompt — write it as if briefing a colleague
- **Use `mode: subagent`** for helper agents that shouldn't be directly selectable by users
- **Test with `strata agent create`** to see how the CLI generates agent definitions, then customize from there
- **Legacy modes are auto-migrated:** If you have `.stratacodemodes` files, they'll be converted on startup — no manual migration needed
...
Ready to explore more? Check out the [Show and Tell](https://github.com/Strata-Org/stratacode/discussions/categories/show-and-tell) to discover and share custom modes and agents created by the community!
