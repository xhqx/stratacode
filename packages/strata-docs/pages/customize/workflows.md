---
title: "Workflows"
description: "Create automated workflows with Strata Code"
platform: new
---

# Workflows

Workflows (also called **slash commands** in the new extension) automate repetitive tasks by defining step-by-step instructions for Strata Code to execute.

{% image src="/docs/img/screenshot-tests/strata-vscode/visual-regression/settings/agent-behaviour-workflows-chromium-linux.png" alt="Workflows tab in Strata Code" width="420" caption="Workflows tab in Strata Code" /%}
...
Workflows are Markdown files stored as **slash commands** in `.strata/commands/`:

- **Global commands**: `~/.config/strata/commands/` (available in all projects)
- **Project commands**: `[project]/.strata/commands/` (project-specific)
...
For example, a file at `.strata/commands/submit-pr.md` is invoked with `/submit-pr`.
...
The new extension automatically migrates legacy workflows from `.stratacode/workflows/` to the new command format on startup. You can also manually move files and remove the `.md` extension from invocations.

{% /tab %}
{% tab label="VSCode (Legacy)" %}

Workflows are markdown files stored in `.stratacode/workflows/`:

- **Global workflows**: `~/.stratacode/workflows/` (available in all projects)
- **Project workflows**: `[project]/.stratacode/workflows/` (project-specific)
...
Create a file called `submit-pr.md` in your `.strata/commands` directory:
...
Create a file called `submit-pr.md` in your `.stratacode/workflows` directory:
