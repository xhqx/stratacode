...
Strata's **Managed Indexing** feature provides semantic search across your repositories using cloud-hosted embeddings. When enabled, Strata indexes your codebase to deliver more relevant, context-aware responses during development.
...
1. Create a `.stratacode/config.json` file in the root of your repository (if it doesn't already exist).
...
- **Storage** — Embeddings are stored in Strata Cloud. Your actual code is never stored, only the vector embeddings.
- **Team sharing** — For teams and enterprise accounts, indexes are shared among all team members.

### Index Retention

Indexes are stored for **7 days**. If a branch or repository index hasn't been updated within that window, it will be garbage collected. The next time you open the project in VS Code with Strata running, it will be re-indexed automatically.
...
A minimal UI is available at [app.strata.ai](https://app.strata.ai) to:
