...
- When Strata Code needs to make precise changes to existing code without rewriting entire files.
...
- Validates file access against `.stratacodeignore` rules.
...
2.  **StrataCodeIgnore Check**: Validates if the target file path is allowed by `.stratacodeignore` rules.
...
Strata Code uses this strategy for applying diffs:
