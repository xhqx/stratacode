...
The `list_files` tool displays the files and directories within a specified location. It helps Strata Code understand your project structure and navigate your codebase effectively.
...
- When Strata Code needs to understand your project structure
- When Strata Code explores what files are available before reading specific ones
- When Strata Code maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When Strata Code needs to check for specific file types (like configuration files) across a project
...
- Marks files ignored by `.stratacodeignore` with a lock symbol (🔒) when `showStrataCodeIgnoredFiles` is enabled
...
5. **Result Filtering**:
   ...
   - Respects `.gitignore` rules when in recursive mode
   - Handles `.stratacodeignore` patterns, either hiding files or marking them with a lock symbol
6. **Formatting**:
   - Marks directories with a trailing slash (`/`)
   - Sorts results to show directories before their contents for logical hierarchy
   - Marks ignored files with a lock symbol (🔒) when `showStrataCodeIgnored` is enabled
...
- Files ignored by `.stratacodeignore` are marked with a lock symbol (🔒) when `showStrataCodeIgnored` is enabled
...
When `.stratacodeignore` files are used and `showStrataCodeIgnored` is enabled:
...
- When starting a new task, Strata Code may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), Strata Code first lists directories to know where to look.
- When providing recommendations for code organization, Strata Code examines the current project structure first.
- When setting up a new feature, Strata Code lists related directories to understand the project conventions.
