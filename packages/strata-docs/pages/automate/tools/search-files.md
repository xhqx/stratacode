...
The `search_files` tool performs regex searches across multiple files in your project. It helps Strata Code locate specific code patterns, text, or other content throughout your codebase with contextual results.
...
- When Strata Code needs to find where specific functions or variables are used
- When Strata Code helps with refactoring and needs to understand usage patterns
- When Strata Code needs to locate all instances of a particular code pattern
- When Strata Code searches for text across multiple files with filtering capabilities
...
- When asked to refactor a function, Strata Code first searches for all places the function is used to ensure comprehensive changes.
- When investigating bugs, Strata Code searches for similar patterns to identify related issues across the codebase.
- When addressing technical debt, Strata Code locates all TODO comments across the project.
- When analyzing dependencies, Strata Code finds all imports of a particular module.
