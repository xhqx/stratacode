...
The `access_mcp_resource` tool retrieves data from resources exposed by connected Model Context Protocol (MCP) servers. It allows Strata Code to access files, API responses, documentation, or system information that provides additional context for tasks.
...
- When Strata Code needs additional context from external systems
- When Strata Code needs to access domain-specific data from specialized MCP servers
- When Strata Code needs to retrieve reference documentation hosted by MCP servers
- When Strata Code needs to integrate real-time data from external APIs via MCP
...
4. **Response Processing**:
   ...
   - Returns the processed resource data to Strata Code for use in the current task
...
- When helping with API development, Strata Code retrieves endpoint specifications from MCP resources to ensure correct implementation.
- When assisting with data visualization, Strata Code accesses current data samples from connected MCP servers.
- When working in specialized domains, Strata Code retrieves technical documentation to provide accurate guidance.
- When generating industry-specific code, Strata Code references compliance requirements from documentation resources.
