...
description: Learn how Strata Code's tools automate your development workflow
---

# Tool Use Overview

Strata Code implements a sophisticated tool system that allows AI models to interact with your development environment in a controlled and secure manner. This document explains how tools work, when they're called, and how they're managed.
...
These tools help Strata Code understand your code and project:
...
These tools help Strata Code make changes to your code:
...
These tools help Strata Code run commands:

- `bash` - Runs shell commands with configurable timeout and working directory

### Web Tools

These tools help Strata Code access web content:

- `webfetch` - Fetches a URL and returns the content
- `websearch` - Searches the web (available to Strata/OpenRouter users)
- `codesearch` - Semantic code search (available to Strata/OpenRouter users)

### Browser Tools

The VS Code extension has a built-in browser automation tool powered by [Playwright MCP](https://www.npmjs.com/package/@playwright/mcp). Enable it in Settings → Browser Automation. When enabled, it registers an MCP server named `strata-playwright` and exposes tools such as:

- `strata-playwright_browser_navigate` - Navigate to a URL
- `strata-playwright_browser_click` - Click an element
- `strata-playwright_browser_type` - Type text into an element
- `strata-playwright_browser_screenshot` - Capture a screenshot
- `strata-playwright_browser_snapshot` - Capture an accessibility snapshot
