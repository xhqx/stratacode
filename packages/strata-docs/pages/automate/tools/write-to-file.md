...
- When Strata Code needs to create a new file from scratch
- When Strata Code needs to completely rewrite an existing file
...
- Access Control: Validates against `.stratacodeignore` restrictions before making changes
...
1. **Parameter Validation**: Validates the required parameters and permissions
   ...
   - Validates the file is allowed (not restricted by `.stratacodeignore`)
...
- When creating a new project, Strata Code generates multiple files but lets you review each before committing changes.
- When setting up configuration files, Strata Code shows the proposed configuration in a diff view for approval.
- When generating documentation, Strata Code creates markdown files but lets you make final adjustments in the diff view.
- When developing a prototype, Strata Code shows complete source files in a diff view where you can fine-tune before saving.
