...
Strata Code uses tools to interact with your code and environment. These specialized helpers perform specific actions like reading files, making edits, running commands, or searching your codebase. Tools provide automation for common development tasks without requiring manual execution.

## Tool Workflow

Describe what you want to accomplish in natural language, and Strata Code will:
...
**Strata Code:** (Proposes the `write` tool)
...
**Strata Code:** (Proposes the `write_to_file` tool as shown in the image above)
...
**Strata Code:** (Confirms file creation)
...
Every tool use is subject to a permission check. The default action for any tool with no matching rule in your config is **`ask`** — meaning Strata will pause and prompt you before executing it.
...
Every tool use requires your explicit approval. When Strata proposes a tool, you'll see:
