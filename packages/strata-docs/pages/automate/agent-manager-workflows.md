...
{% callout type="info" %}
All Agent Manager sessions share a single `strata serve` process. What each worktree isolates is the filesystem and git state — the branch, the directory, the terminal. API keys, models, and configuration are shared.
{% /callout %}
...
- **Run script** — create `.strata/run-script` (or `.ps1` / `.cmd` / `.bat` on Windows) and trigger it with `Cmd+E` / `Ctrl+E`. Runs in whichever worktree is selected. Gets `WORKTREE_PATH` and `REPO_PATH` in the environment.
...
- **`strata review` in CI** — automated PR review. See [Code Reviews](/docs/automate/code-reviews/overview) for the setup.
