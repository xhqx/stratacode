...
The Agent Manager is a control panel for running and orchestrating multiple Strata Code agents, with support for parallel worktree-isolated sessions.

{% tabs %}
{% tab label="VSCode" %}

The Agent Manager is a **full-panel editor tab** built directly into the extension. All sessions share the single `strata serve` backend process. It supports:
...
- Command Palette: "Strata Code: Open Agent Manager"
...
2. Enter a branch name (or let Strata generate one)
...
3. Strata creates one worktree + session per version and runs them in parallel
...
Place an executable script at `.strata/setup-script` in your project root. It runs automatically whenever a new worktree is created (useful for `npm install`, env setup, etc.). Root-level `.env` and `.env.*` files are also auto-copied from the main repo before the setup script runs.
...
Agent Manager state is persisted in `.strata/agent-manager.json`. Sessions, worktrees, and their order are restored on reload.
...
The Agent Manager is a dedicated control panel for running and supervising Strata Code agents as interactive CLI processes. It supports:
...
- Install/update the Strata Code CLI (latest) — see [CLI setup](/docs/code-with-ai/platforms/cli)
- Open a project in VS Code (workspace required)
- Authentication: You must be logged in via the extension settings OR use CLI with stratacode as provider (see [Authentication Requirements](#authentication-requirements))

## Opening the Agent Manager

- Command Palette: "Strata Code: Open Agent Manager"
- Or use the title/menu entry if available in your Strata Code UI
...
Worktrees are created in `.stratacode/worktrees/` within your project directory. This folder is automatically excluded from git via `.git/info/exclude` (a local-only ignore file that doesn't require a commit).

```
your-project/
├── .git/
│   └── info/
│       └── exclude   # local ignore rules (includes .stratacode/worktrees/)
├── .stratacode/
│   └── worktrees/
...
1. **Strata Code Extension (Recommended)**
...
2. **CLI with Strata Code Provider**
   - Use the CLI configured with `stratacode` as the provider
   - Run `stratacode config` to set up authentication
...
To use the Agent Manager with all features enabled, switch to the Strata Code provider or sign in through the extension.

## Remote sessions (Cloud)

When signed in (Strata Cloud), the Agent Manager lists your recent cloud-synced sessions:
...
- Remote sessions not visible
  ...
  - If using BYOK, session syncing is not available — switch to Strata Code provider or sign in through the extension
- Authentication errors
  - Verify you're logged in via extension settings or using CLI with stratacode provider
