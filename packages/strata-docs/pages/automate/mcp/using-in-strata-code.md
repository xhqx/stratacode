---
title: "Using MCP in Strata Code"
description: "How to use MCP servers in Strata Code"
---

# Using MCP in Strata Code

Model Context Protocol (MCP) extends Strata Code's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with Strata Code.
...
MCP server configurations are stored inside the main Strata config file. There are two levels:

1. **Global Configuration**: `~/.config/strata/strata.jsonc` — applies to all projects.
2. **Project-level Configuration**: `strata.jsonc` in your project root, or `.strata/strata.jsonc` for a cleaner setup.

**Precedence**: Project-level configuration takes precedence over global configuration.

### Editing MCP Settings

You can edit MCP settings from the Strata Code settings UI:
...
MCP servers are configured under the `mcp` key in `strata.jsonc`:
...
Remote servers support OAuth 2.0 authentication. If the server supports it, Strata Code will automatically start the OAuth flow when you connect. You can also disable OAuth with `"oauth": false`.
...
_Example of MCP Server config in Strata Code (STDIO Transport)_
...
- **Remote (HTTP/SSE)**: Servers hosted over HTTP/HTTPS. Strata Code tries `StreamableHTTP` first, then falls back to `SSE` automatically.
...
| `strata mcp logout` | Log out from an MCP server      |
| `strata mcp debug`  | Debug an MCP server connection  |
...
Run `strata mcp list` to verify the server status.
...
You can edit both global and project-level MCP configuration files directly from the Strata Code settings.

1. Click the {% codicon name="gear" /%} icon in the top navigation of the Strata Code pane to open `Settings`.
2. Click the `Agent Behaviour` tab on the left side
3. Select the `MCP Servers` sub-tab
4. Click the appropriate button:
   - **`Edit Global MCP`**: Opens the global `mcp_settings.json` file.
   - **`Edit Project MCP`**: Opens the project-specific `.stratacode/mcp.json` file. If this file doesn't exist, Strata Code will create it for you.

{% image src="/docs/img/using-mcp-in-strata-code/mcp-installed-config.png" alt="Edit Global MCP and Edit Project MCP buttons" width="600" caption="Edit Global MCP and Edit Project MCP buttons" /%}
...
{% image src="/docs/img/using-mcp-in-strata-code/using-mcp-in-strata-code-5.png" alt="Delete confirmation box" width="400" caption="Delete confirmation box" /%}
...
{% image src="/docs/img/using-mcp-in-strata-code/using-mcp-in-strata-code-6.png" alt="Network Timeout pulldown" width="400" caption="Network Timeout pulldown" /%}
...
**In your config file:** Add the tool name (or a wildcard pattern) to the `permission` key in `strata.jsonc`:
...
{% image src="/docs/img/using-mcp-in-strata-code/using-mcp-in-strata-code-7.png" alt="Always allow checkbox for MCP tools" width="120" caption="Always allow checkbox for MCP tools" /%}

When enabled, Strata Code will automatically approve this specific tool without prompting. Note that the global "Use MCP servers" setting takes precedence - if it's disabled, no MCP tools will be auto-approved.
...
Strata Code does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Strata Marketplace:** Browse community-contributed MCP server configurations and agent skills in the [Strata Marketplace](https://github.com/Strata-Org/strata-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask Strata Code:** You can ask Strata Code to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend Strata Code with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, Strata Code will automatically detect available tools and resources. To use them:

1. Type your request in the Strata Code chat interface
2. Strata Code will identify when an MCP tool can help with your task
...
- **Server Not Responding:** Check if the server process is running. Use `strata mcp debug <server-name>` to inspect the connection.
- **Permission Errors:** Ensure API keys and credentials are set in your `strata.jsonc` config or via `{env:VARIABLE_NAME}` references.
...
- **Permission Errors:** Ensure proper API keys and credentials are configured in your `mcp_settings.json` (for global settings) or `.stratacode/mcp.json` (for project settings).
- **Tool Not Available:** Confirm the server is properly implementing the tool and it's not disabled in settings
