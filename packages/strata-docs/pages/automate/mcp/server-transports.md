...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Strata Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Strata Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Strata Code
...
- Single-client scenarios (one Strata Code instance per server)
...
1. The client (Strata Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Strata Code
...
STDIO servers run locally on the same machine as Strata Code, which has several important implications:
...
- **Execution**: Starts and stops with Strata Code (child process lifecycle)
...
- Start when needed by Strata Code
- Not require network configuration
- Need to be installed alongside Strata Code or via a package manager
...
## Configuring Transports in Strata Code

For detailed information on configuring STDIO and SSE transports in Strata Code, including example configurations, see the [Understanding Transport Types](using-in-strata-code#understanding-transport-types) section in the Using MCP in Strata Code guide.
