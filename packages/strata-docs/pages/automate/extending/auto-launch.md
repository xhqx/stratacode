...
Auto-Launch Configuration allows you to automatically start a Strata Code task when opening a workspace, with support for specific profiles and modes. This was originally developed as an internal test feature, but we decided to expose it to users in case anyone finds it useful!
...
When you open a workspace in VS Code, Strata Code automatically checks for a launch configuration JSON file. If found, it:
...
1. Create a `.stratacode` directory in your workspace root (if it doesn't exist)
2. Create a `launchConfig.json` file inside the `.stratacode` directory
...
- **`mode`** (string): The Strata Code mode to use for this task. Available modes:
...
- Auto-launch triggers approximately 500ms after Strata Code extension activation
...
1. Verify file location: `.stratacode/launchConfig.json` in workspace root
