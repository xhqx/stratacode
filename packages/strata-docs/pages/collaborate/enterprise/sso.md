...
Strata Enterprise lets your organization securely manage access using **Single Sign-On (SSO)**. With SSO enabled, team members can sign in to Strata using your company's existing identity provider, such as Okta, Github, Google Workspace, etc.

{% callout type="warning" %}
**IDP-initiated logins are not currently supported.** Users must navigate to the [Strata Web App](https://app.strata.ai) to log in. Logging in directly from your identity provider's dashboard is not supported at this time.
{% /callout %}
...
- Admin or Owner permissions for your Strata organization.
- Access to your **Identity Provider (IdP)** (e.g. Okta, Google Workspace, Azure AD).

## Initiating SSO Configuration

### 1. Open [Organization](https://app.strata.ai/organizations) Dashboard
...
Once the Strata team has enabled SSO for your organization, your named admin will get an email from WorkOS to configure SSO.

{% callout type="warning" %}
**Save domain policy for last.**

If you configure domain policy before setting up SSO, you may lock users out of Strata.
{% /callout %}
