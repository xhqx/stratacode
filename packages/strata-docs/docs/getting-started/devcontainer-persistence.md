...
description: How to preserve Strata Code threads and settings in dev containers
---

# Dev Container Persistence

When using Strata Code in development containers (VS Code Dev Containers, GitHub Codespaces, etc.), your threads and settings can persist across container rebuilds by properly configuring volume mounts.

## Why Persistence Matters

Dev containers are ephemeral by default - when you rebuild the container, all data is lost unless explicitly persisted. Strata Code stores important data including:

- **Conversation threads**: Your ongoing discussions with Strata Code
- **Settings**: API configurations, custom modes, and preferences
- **Cache**: Vector store for code indexing and browser tool data

## Required Configuration

The Strata Code dev container is pre-configured with named volumes to preserve your data. If you're setting up your own dev container, add these mounts to your `devcontainer.json`:
...
```json
{
  ...
  "mounts": [
    {
      "source": "stratacode-global-storage",
      "target": "/root/.vscode-remote/data/User/globalStorage/stratacode.strata-code",
      "type": "volume"
    },
    {
      "source": "stratacode-settings",
...
2. Start new conversations with Strata Code
...
docker cp <container-name>:/root/.vscode-remote/data/User/globalStorage/stratacode.strata-code ./stratacode-backup
```
...
"mounts": [
  {
    "source": "${localWorkspaceFolder}/.stratacode-data",
    "target": "/home/vscode/stratacode-data",
    "type": "bind"
  }
]
```
...
2. Set the custom storage path in VS Code settings:
   ...
   - Search for "Strata Code: Custom Storage Path"
   - Enter: `/home/vscode/stratacode-data`
...
GitHub Codespaces automatically persists your VS Code settings and extensions. For Strata Code threads, the pre-configured dev container includes the necessary volume mounts.
