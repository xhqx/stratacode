...
# Migrating from Cline to Strata

A practical guide for developers switching from Cline to Strata.

## Why Switch?

**Speed.** The Strata Platform is designed to reduce friction across the entire development workflow. Beyond feature parity with Cline, Strata includes Code Reviews, App Builder, Cloud Agents, and one-click Deploy.

**Multi-platform.** Work in VS Code, JetBrains IDEs, CLI, or the web. Your Sessions sync across all of them automatically.

**Specialized modes.** Instead of one agent doing everything, Strata has five specialized modes optimized for different parts of your workflow.

**500+ models.** More providers, more options, better pricing flexibility. Switch models mid-conversation if you want.

---

## Understanding Strata's Mode System

Cline uses a single agent with a Plan/Act toggle. Strata takes a different approach: five specialized modes, each optimized for a specific part of development. You select modes from a dropdown in the interface in the Strata Extension, or with slash commands in the CLI.
...
**Strata:** Select **Ask** from the dropdown
...
**Why this matters:** In Cline, you might accidentally make changes while exploring. In Strata, Ask and Architect modes can't write files, so you're safe to explore without worry. Strata's **Orchestrator** mode lets you intelligently generate and execute subtasks that are optimized for each mode.
...
2. Search "Strata Code"
3. Click Install
4. Find the Strata icon in your sidebar
...
4. Find Strata icon in right sidebar
...
Visit [app.strata.ai](https://app.strata.ai/) and sign in. This gives you access to:

- **Cloud Agents:** Run Strata without a local machine
- **App Builder:** Build and preview apps directly in your browser
- **Strata Deploy:** One-click deployments
...
1. Click "Try Strata Code for Free" in the Strata panel
2. Sign in with OAuth at strata.ai
3. You'll be redirected back to your IDE

### Configure your provider

**Option 1: Use Strata Gateway (easiest)**

1. Open settings (⚙️ icon)
2. Select "Strata Gateway" as provider
...
## Beyond the IDE: Strata's Platform Features

One of the biggest differences from Cline is that Strata isn't just an IDE extension. It's a platform with multiple interfaces that can all share your Sessions and context.

### Cloud Agents

Run Strata from [app.strata.ai/cloud](https://app.strata.ai/cloud) without needing your local machine. Great for:
...
Build live apps and sites directly from the web with a real-time preview. Similar to Lovable, but integrated with your Strata Sessions. Deploy with one click when you're ready.

### Strata Deploy

One-click deployments from directly within Strata. Go from code to production without leaving your workflow.
...
Semantic search across your repositories using cloud-hosted embeddings. Strata indexes your codebase to deliver more relevant, context-aware responses.
...
- **One-click Deploy:** Ship directly from Strata
- **Code Reviews:** AI-powered PR analysis
...
**Q: Can I use both Cline and Strata side-by-side?** Yes. They're separate extensions.
...
1. Install Strata in your primary IDE
...
3. Try Cloud Agents at [app.strata.ai](https://app.strata.ai/)
4. Install the Strata CLI with `npm install -g @stratacode/cli`
