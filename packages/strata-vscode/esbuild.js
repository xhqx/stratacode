...
/**
 * Force all solid-js imports (from strata-ui and the webview) to resolve to
...
const solidDedupePlugin = {
  name: "solid-dedupe",
  setup(build) {
    // Resolve these bare specifiers to the strata-vscode-local copy
    ...
    build.onLoad({ filter: /sprite\.svg$/ }, (args) => {
      ...
      return {
        contents: `
          ...
          const inject = () => {
            if (!document.getElementById("strata-sprite")) {
...
async function main() {
  ...
  // Build StrataClaw webview (SolidJS, standalone chat panel)
  const strataClawCtx = await createBrowserWebviewContext("webview-ui/strataclaw/index.tsx", "dist/strataclaw.js")
