...
const providers = {
  strata: makeProvider("strata", "Strata Gateway", ["strata-auto/free"]),
...
describe("parseModelString", () => {
  ...
  it("keeps slashes inside strata model ids", () => {
    expect(parseModelString("strata/strata-auto/free")).toEqual({
      providerID: "strata",
      modelID: "strata-auto/free",
    })
  })
...
describe("resolveModelSelection", () => {
  ...
  it("uses strata auto as the explicit final fallback", () => {
    const result = resolveModelSelection({
      providers,
      connected: [],
      fallback: STRATA_AUTO,
    })
    expect(result).toEqual(STRATA_AUTO)
  })

  it("keeps the explicit fallback even when strata is missing from the loaded catalog", () => {
    const result = resolveModelSelection({
