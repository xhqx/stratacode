...
} from "../../src/strata-provider/handlers/permission-handler"
...
describe("recoveryDirs", () => {
  ...
  it("returns workspace root plus each unique worktree directory", () => {
    const dirs = new Map([
      ["s1", "/workspace/.strata/worktrees/alpha"],
      ["s2", "/workspace/.strata/worktrees/beta"],
      ["s3", "/workspace/.strata/worktrees/alpha"],
    ])
    expect(recoveryDirs("/workspace", dirs)).toEqual([
      "/workspace",
      "/workspace/.strata/worktrees/alpha",
      "/workspace/.strata/worktrees/beta",
    ])
  })
})
...
describe("fetchAndSendPendingPermissions", () => {
  ...
  it("queries workspace root plus each unique worktree directory", async () => {
    ...
    expect(queries).toContain("/workspace/.strata/worktrees/alpha")
    expect(queries).toContain("/workspace/.strata/worktrees/beta")
  ...
  it("deduplicates directories", async () => {
    const dirs = new Map([
      ["s1", "/workspace/.strata/worktrees/alpha"],
      ["s2", "/workspace/.strata/worktrees/alpha"],
    ])
    const { fake, queries } = ctx({ tracked: ["s1", "s2"], dirs })
    await fetchAndSendPendingPermissions(fake)
    expect(queries.filter((d) => d === "/workspace/.strata/worktrees/alpha")).toHaveLength(1)
  })
