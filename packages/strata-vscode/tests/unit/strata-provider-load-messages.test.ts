...
function makeProvider(client: ReturnType<typeof createClient>) {
  ...
  const provider = new StrataProvider({} as never, connection as never)
...
describe("StrataProvider.handleLoadMessages / focus mode freshness", () => {
...
describe("StrataProvider.loadMessages / sub-agent viewer full history", () => {
...
describe("StrataProvider.handleLoadMessages / prepend into deleted session", () => {
