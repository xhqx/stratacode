...
const STRATA_PROVIDER_FILE = path.join(ROOT, "src/StrataProvider.ts")
...
describe("Extension — package.json command sync", () => {
  ...
  it("every contributes.commands entry has a registerCommand() call", () => {
    ...
    const dynamic: Record<string, string> = {
      "strata-code.new.agentManager.jumpTo": "registerCommand(`strata-code.new.agentManager.jumpTo${",
    }
  ...
  /**
   * All declared commands must use the strata-code.new. prefix.
   * The legacy strata-code.* namespace (without .new.) belongs to the old
   * extension and must not be reintroduced.
   */
  it("all declared commands use the strata-code.new. prefix", () => {
    const bad = declared.filter((cmd) => !cmd.startsWith("strata-code.new."))
    expect(
      bad,
      `Commands without "strata-code.new." prefix — use the namespaced form:\n` + bad.map((b) => `  - ${b}`).join("\n"),
    ).toEqual([])
  })
})

// ---------------------------------------------------------------------------
// StrataProvider handler wiring — every new StrataProvider() must get
// setContinueInWorktreeHandler() called before resolving its webview.
//
// Regression: tab panels created via openStrataInNewTab() and the TabPanel
...
describe("Extension — StrataProvider handler wiring", () => {
  const ext = fs.readFileSync(EXTENSION_FILE, "utf-8")

  /**
   * Every `new StrataProvider(` in extension.ts must be followed (before the
   * next `new StrataProvider(`) by a `setContinueInWorktreeHandler` call.
   * This prevents future tab/panel additions from silently missing the handler.
   */
  it("every StrataProvider instance gets setContinueInWorktreeHandler wired", () => {
    const pattern = /new StrataProvider\(/g
    ...
    expect(
      instances.length,
      "expected at least 3 StrataProvider instances (sidebar, tab, deserializer)",
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      if (!region.includes("setContinueInWorktreeHandler")) {
        ...
        missing.push(`StrataProvider at line ${line}`)
      }
    }

    expect(
      missing,
      `These StrataProvider instances are missing setContinueInWorktreeHandler.\n` +
  ...
  it("openStrataInNewTab wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const fn = ext.indexOf("function openStrataInNewTab")
    expect(fn, "openStrataInNewTab must exist").toBeGreaterThan(-1)
  ...
  it("TabPanel deserializer wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const serializer = ext.indexOf('"strata-code.new.TabPanel"')
...
// ---------------------------------------------------------------------------
// StrataProvider — continueInWorktree error fallback
//
...
describe("StrataProvider — continueInWorktree error fallback", () => {
  const helper = fs.readFileSync(path.join(ROOT, "src/strata-provider/continue-worktree.ts"), "utf-8")
