...
import type { Config } from "@stratacode/sdk/v2/client"

// vscode mock is provided by the shared preload (tests/setup/vscode-mock.ts)
const { StrataProvider } = await import("../../src/StrataProvider")
...
describe("StrataProvider indexing refresh", () => {
  it("reloadAfterAuthChange fetches config first, then indexing status", async () => {
    const provider = new StrataProvider({} as never, {} as never)
  ...
  it("handleUpdateConfig no longer eagerly fetches indexing status", async () => {
    ...
    const provider = new StrataProvider({} as never, conn.service as never)
    const internal = provider as unknown as Internals
  ...
  it("fetchAndSendIndexingStatus uses current session directory header", async () => {
    const worktree = "/repo/.strata/.stratacode/worktrees/feature"
    ...
    try {
      ...
      const auth = Buffer.from("strata:secret").toString("base64")
      expect(headers.get("Authorization")).toBe(`Basic ${auth}`)
      expect(headers.get("x-strata-directory")).toBe(worktree)
    } finally {
