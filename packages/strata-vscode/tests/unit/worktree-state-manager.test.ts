...
describe("WorktreeStateManager", () => {
  ...
  beforeEach(() => {
    ...
    // Pre-create .strata dir so fire-and-forget saves don't race on mkdir
  ...
  describe("persistence", () => {
    ...
    it("creates .strata directory if missing", async () => {
      ...
      expect(fs.existsSync(path.join(fresh, ".strata", "agent-manager.json"))).toBe(true)
    })
  })
  ...
  describe("tab order", () => {
    ...
    it("does not persist empty tab order", async () => {
      ...
      const content = fs.readFileSync(path.join(root, ".strata", "agent-manager.json"), "utf-8")
  ...
  describe("validate", () => {
    ...
    it("resolves relative paths against root", async () => {
      const relative = ".strata/worktrees/test-branch"
  ...
  describe("load with corrupt data", () => {
    it("handles malformed JSON gracefully", async () => {
      const file = path.join(root, ".strata", "agent-manager.json")
  ...
  describe("legacy .stratacode migration", () => {
    it("migrates and loads state from .stratacode when .strata is absent", async () => {
      // Remove the .strata dir created in beforeEach
      fs.rmSync(path.join(root, ".strata"), { recursive: true, force: true })

      // Write state to legacy .stratacode dir (migration will move it to .strata)
      const legacyDir = path.join(root, ".stratacode")
    ...
    it("skips migration when .strata state already exists", async () => {
      // .strata state already present — migration should skip agent-manager.json
      fs.writeFileSync(
        path.join(root, ".strata", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-new": {
              branch: "new-branch",
    ...
    it("rewrites stale .stratacode paths in worktree entries (unix)", async () => {
      fs.writeFileSync(
        path.join(root, ".strata", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-stale": {
              branch: "fix",
              path: "/repo/.stratacode/worktrees/fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
      ...
      expect(manager.getWorktrees()[0].path).toBe(`/repo/.strata/worktrees/fix`)
    })

    it("rewrites stale .stratacode paths with backslashes (windows)", async () => {
      fs.writeFileSync(
        path.join(root, ".strata", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-win": {
              branch: "fix",
              path: "C:\\.stratacode\\worktrees\\fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
      ...
      expect(manager.getWorktrees()[0].path).toBe("C:\\.strata\\worktrees\\fix")
    })
  })
})
