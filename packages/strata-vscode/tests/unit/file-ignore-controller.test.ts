...
async function createTempWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "strata-ignore-"))
  tempDirs.push(dir)
  return dir
}

describe("FileIgnoreController", () => {
  describe("when .stratacodeignore exists", () => {
    it("applies only .stratacodeignore patterns", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".stratacodeignore"), "secret/**\n*.snap\n")
      ...
      expect(controller.getInstructions()).toContain(".stratacodeignore")
    })

    it("does NOT block .env files unless explicitly listed", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".stratacodeignore"), "dist/\n")
    ...
    it("does NOT apply .gitignore patterns", async () => {
      ...
      // .stratacodeignore pattern should apply
      expect(controller.validateAccess(path.join(workspace, "dist", "bundle.js"))).toBe(false)
    })
  })

  describe("when no .stratacodeignore exists (fallback)", () => {
    ...
    it("blocks .env files via hardcoded sensitive patterns", async () => {
      const workspace = await createTempWorkspace()
      // No .stratacodeignore, no .gitignore
  ...
  describe("Windows cross-drive paths", () => {
    it("does not throw for a Windows-style absolute path from another drive", async () => {
      ...
      const cross =
        "c:/Users/User/AppData/Roaming/Code/User/globalStorage/stratacode.strata-code/settings/mcp_settings.json"
    ...
    it("does not throw for file:// URIs with Windows drive letters", async () => {
      ...
      const uri =
        "file:///c:/Users/User/AppData/Roaming/Code/User/globalStorage/stratacode.strata-code/settings/mcp_settings.json"
