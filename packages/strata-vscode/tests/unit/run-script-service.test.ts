...
describe("RunScriptService", () => {
  ...
  it("builds commands for shell, PowerShell, and CMD scripts", () => {
    ...
    expect(buildRunTaskCommand({ path: "C:\\repo\\.strata\\run-script.ps1", kind: "powershell" })).toEqual({
      command: "powershell.exe",
      args: ["-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "C:\\repo\\.strata\\run-script.ps1"],
    })
    expect(buildRunTaskCommand({ path: "C:\\repo path\\.strata\\run-script.cmd", kind: "cmd" })).toEqual({
      command: "cmd.exe",
      args: ["/d", "/s", "/c", '"C:\\repo path\\.strata\\run-script.cmd"'],
    })
  })
  ...
  it("rejects symlinks pointing outside the .strata directory", () => {
  ...
  it("accepts symlinks pointing inside the .strata directory", () => {
