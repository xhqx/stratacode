...
import { mapSSEEventToWebviewMessage } from "../../src/strata-provider-utils"
import { configFeatures } from "../../src/features"
import type { EventIndexingStatus, IndexingStatus } from "@stratacode/sdk/v2/client"
...
describe("indexing feature detection", () => {
  it("requires experimental.semantic_indexing when indexing plugin is present", () => {
    expect(configFeatures({ plugin: ["strata-indexing"] }).indexing).toBe(false)
    expect(configFeatures({ plugin: ["strata-indexing"], experimental: {} }).indexing).toBe(false)
    expect(configFeatures({ plugin: ["strata-indexing"], experimental: { semantic_indexing: false } }).indexing).toBe(
      false,
    )
  })
  ...
  it("detects supported indexing plugin specifiers when experimental.semantic_indexing is true", () => {
    ...
    expect(
      configFeatures({
        plugin: ["file:///tmp/.opencode/plugin/strata-indexing.js"],
        experimental: { semantic_indexing: true },
      }).indexing,
    ).toBe(true)
    expect(
      configFeatures({
        plugin: ["file:///tmp/node_modules/@stratacode/strata-indexing/index.js"],
        experimental: { semantic_indexing: true },
      }).indexing,
    ).toBe(true)
  })
  ...
  it("ignores unrelated plugin lists", () => {
    expect(configFeatures({ plugin: ["@stratacode/strata-gateway"] }).indexing).toBe(false)
