 ...
 * 2. Every WebviewMessage type handled in StrataProvider has a corresponding member in the WebviewMessage union
...
const STRATA_PROVIDER_UTILS_FILE = path.join(ROOT, "src/strata-provider-utils.ts")
...
describe("StrataProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in StrataProvider exist in WebviewMessage union", () => {
    const providerContent = readFile(STRATA_PROVIDER_FILE)
    ...
    expect(
      unrecognized,
      `StrataProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
...
describe("mapSSEEventToWebviewMessage output types", () => {
  it("all output types from mapSSEEventToWebviewMessage exist in ExtensionMessage", () => {
    const utilsContent = readFile(STRATA_PROVIDER_UTILS_FILE)
    const messagesContent = readMessageTypeSources()
