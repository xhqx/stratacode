/**
 * Runtime contract tests for strata-vscode's dependencies on @stratacode/strata-ui.
 *
 * These tests import the upstream UI modules directly and verify at runtime
 * that the exports strata-vscode depends on still exist with the expected shape.
 *
 * Because the upstream modules use SolidJS JSX (jsxImportSource: "solid-js"),
 * they must be loaded from within packages/strata-ui/ where bun picks up the
...
const STRATA_UI_DIR = path.join(MONOREPO_ROOT, "packages/strata-ui")
...
const STRATA_MESSAGE_PART_FILE = path.join(MONOREPO_ROOT, "packages/strata-ui/src/components/message-part.tsx")
...
/**
 * Tool names that strata-vscode overrides or uses directly.
 * Sources:
...
describe("ToolRegistry tool name contract (runtime)", () => {
  it("all tools used by strata-vscode are registered in ToolRegistry", () => {
...
describe("Edit tool diff-first click contract (source)", () => {
  const src = fs.readFileSync(STRATA_MESSAGE_PART_FILE, "utf-8")
