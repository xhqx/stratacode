...
import {
  ...
  type ProviderInfo,
} from "../../src/strata-provider-utils"
import { mergeFileSearchItems } from "../../src/strata-provider/file-search-items"
...
describe("mergeFileSearchItems", () => {
  ...
  it("normalizes Windows separators for matching and output", () => {
    const result = mergeFileSearchItems({
      query: "strata-vscode",
      files: ["packages\\strata-vscode\\src\\StrataProvider.ts"],
      folders: ["packages\\strata-vscode\\"],
    })
    expect(result).toEqual([
      { path: "packages/strata-vscode/", type: "folder" },
      { path: "packages/strata-vscode/src/StrataProvider.ts", type: "file" },
    ])
  })
})
...
describe("getErrorMessage", () => {
  ...
  it("reads the first Zod issue message from ConfigInvalidError", () => {
    const err = {
      ...
      data: {
        path: "/Users/me/.config/strata/strata.json",
        issues: [
...
describe("getConfigErrorDetails", () => {
  ...
  it("formats a single-issue ConfigInvalidError", () => {
    const err = {
      data: {
        path: "/home/me/.config/strata/strata.json",
