...
describe("Agent Manager CSS Prefix", () => {
  ...
  it("all CSS custom properties should use am- prefix", () => {
    ...
    // Allow strata-ui design tokens, vscode theme variables, and third-party
    // library tokens (@pierre/diffs, strata-ui sticky-accordion) used as fallbacks
...
describe("Agent Manager Webview — non-git sessionsLoaded fix", () => {
  ...
  /**
   * Regression: when isGitRepo is false, the Strata server never sends a
...
// ---------------------------------------------------------------------------
// StrataProvider — pendingSessionRefresh race condition fix
// ---------------------------------------------------------------------------

describe("StrataProvider — pending session refresh on reconnect", () => {
  const provider = fs.readFileSync(STRATA_PROVIDER_FILE, "utf-8")
  const utils = fs.readFileSync(path.join(ROOT, "src/strata-provider-utils.ts"), "utf-8")
   ...
   * strata-provider-utils) sets it when httpClient is unavailable, and
  ...
  it("loadSessions sets pendingSessionRefresh when client is null", () => {
    ...
    expect(start, "loadSessions must exist in strata-provider-utils").toBeGreaterThan(-1)
...
//
// Regression: PR #7473 moved StrataNotifications into MessageList. It calls
