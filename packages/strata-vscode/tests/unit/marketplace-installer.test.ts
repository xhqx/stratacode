...
const tmpDir = path.join(os.tmpdir(), `strata-test-${Date.now()}`)

class TestPaths extends MarketplacePaths {
  override configPath(scope: "project" | "global", workspace?: string): string {
    if (scope === "global") return path.join(tmpDir, "global", "strata.json")
    return path.join(tmpDir, "project", ".strata", "strata.json")
  }
