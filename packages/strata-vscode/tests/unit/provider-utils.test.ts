...
describe("isModelValid", () => {
  ...
  it("rejects a disconnected non-strata provider", () => {
    expect(isModelValid(providers, [], { providerID: "openai", modelID: "gpt-4o" })).toBe(false)
  })

  it("accepts strata models when present in the catalog", () => {
    expect(isModelValid(providers, [], { providerID: "strata", modelID: "strata-auto/free" })).toBe(true)
  })
