...
describe("providerSortKey", () => {
  it("returns 0 for strata gateway", () => {
    expect(providerSortKey(STRATA_GATEWAY_ID)).toBe(0)
  })
  ...
  it("sorts providers correctly when used with sort", () => {
    const ids = ["google", "anthropic", "strata", "openai", "github-copilot"]
    const sorted = ids.slice().sort((a, b) => providerSortKey(a) - providerSortKey(b))
    expect(sorted).toEqual(["strata", "anthropic", "github-copilot", "openai", "google"])
  })
})
...
describe("stripSubProviderPrefix", () => {
  ...
  it("does not strip 'Strata: ' prefix", () => {
    expect(stripSubProviderPrefix("Strata: Auto")).toBe("Strata: Auto")
    expect(stripSubProviderPrefix("strata: Auto")).toBe("strata: Auto")
  })
})
...
describe("buildTriggerLabel", () => {
  it("returns resolved model name for non-strata provider unchanged", () => {
    expect(buildTriggerLabel("GPT-4o", "openai", undefined, null, false, "", true, labels)).toBe("GPT-4o")
  })

  it("strips sub-provider prefix from resolved name for strata gateway models", () => {
    expect(
      buildTriggerLabel("Anthropic: Claude Sonnet", STRATA_GATEWAY_ID, undefined, null, false, "", true, labels),
    ).toBe("Claude Sonnet")
  })

  it("does not strip prefix for non-strata provider even if name contains ': '", () => {
  ...
  it("returns providerName / resolvedName for non-strata provider with providerName", () => {
