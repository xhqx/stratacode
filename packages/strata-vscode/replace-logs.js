...
function processFile(filePath, componentName) {
  ...
  if (!content.includes('import { WebviewLogger }')) {
    ...
    if (lastImportIndex !== -1) {
      ...
      content = content.slice(0, endOfLastImport + 1) + `import { WebviewLogger } from "${importPath}"\n` + content.slice(endOfLastImport + 1);
    } else {
  ...
  const prefixes = [
    `\\[Strata(?: New)?\\](?: ${componentName}:)?`,
    `\\[${componentName}\\]`
  ];
  
  for (const prefix of prefixes) {
    content = content.replace(new RegExp(`WebviewLogger\\.(info|warn|error|debug)\\("${componentName}",\\s*"${prefix}\\s*`, 'g'), `WebviewLogger.$1("${componentName}", "`);
  }
