...
export async function migrate(
  ...
  try {
    ...
    const project = await client.stratacode.sessionImport.project(payload.project, { throwOnError: true })
    const projectID = project.data?.id ?? payload.project.id
    const session = await client.stratacode.sessionImport.session(
      {
        ...payload.session,
        projectID,
    ...
    for (const msg of payload.messages) {
      await client.stratacode.sessionImport.message(msg, { throwOnError: true })
    }

    for (const part of payload.parts) {
      await client.stratacode.sessionImport.part(part, { throwOnError: true })
    }
