...
// TODO: Remove the legacy .stratacode -> .strata migration helpers below after the
// GA release cleanup tracked in https://github.com/Strata-Org/stratacode/issues/6986.
...
/** Strata config directory name (project-level and inside worktrees). */
export const STRATA_DIR = ".strata"

/** Legacy config directory name for backward compatibility reads. */
export const LEGACY_DIR = ".stratacode"

/** Agent Manager files that should be migrated from .stratacode/ to .strata/. */
...
export interface MigrationResult {
  /** Number of git worktree refs that were rewritten from .stratacode → .strata. */
  refsFixed: number
}

/**
 * Migrate Agent Manager data from .stratacode/ to .strata/.
 *
 * Moves individual Agent Manager files/directories (worktrees, state,
 * setup scripts) from the legacy .stratacode/ into .strata/. Skips items
 * that already exist in .strata/ (the new location wins). This is safe
 * because Agent Manager exclusively owns these files.
 *
 * Fixes git worktree internal references (.git/worktrees/{name}/gitdir)
 * whenever .strata/worktrees/ exists so partially migrated repos recover too.
 *
...
export async function migrateAgentManagerData(root: string, log: (msg: string) => void): Promise<MigrationResult> {
  ...
  const target = path.join(root, STRATA_DIR)
  ...
  if (await isDirectory(legacy)) {
    ...
    for (const item of AGENT_MANAGER_ITEMS) {
      ...
      if (await exists(dst)) {
        log(`Skipping ${item}: already exists in ${STRATA_DIR}`)
        continue
      }

      try {
        await fs.promises.rename(src, dst)
        log(`Migrated ${item} from ${LEGACY_DIR} to ${STRATA_DIR}`)
      } catch (err) {
...
/**
 * After moving worktrees from .stratacode/ to .strata/, fix git internal refs.
 *
 ...
 * gitdir files that reference the old .stratacode path.
 *
...
async function fixGitWorktreeRefs(root: string, log: (msg: string) => void): Promise<number> {
  ...
  const newSegment = path.join(root, STRATA_DIR) + path.sep
  let fixed = 0
