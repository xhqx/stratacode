 ...
 *
 * Ported from stratacode/src/core/stratacode/agent-manager/WorktreeManager.ts.
 * Handles creation, discovery, and cleanup of worktrees stored in
 * {projectRoot}/.strata/worktrees/
 */
...
const TEMP_PREFIX = ".strata-delete-"
...
export class WorktreeManager {
  ...
  constructor(root: string, log: (msg: string) => void, ops?: GitOps) {
    this.root = root
    this.dir = path.join(root, STRATA_DIR, "worktrees")
    this.git = simpleGit(root)
    this.ops = ops
    this.log = log
  }

  /** Run once before first read/write to migrate Agent Manager data from .stratacode → .strata. */
  ...
  /** Remove orphaned .strata-delete-* temp dirs left by interrupted deletions. */
  ...
  async writeMetadata(worktreePath: string, sessionId: string, parentBranch: string, remote?: string): Promise<void> {
    const dir = path.join(worktreePath, STRATA_DIR)
  ...
  async readMetadata(
    ...
    // Check .strata/ first, then legacy .stratacode/
    for (const dirName of [STRATA_DIR, LEGACY_DIR]) {
  ...
  async ensureGitExclude(): Promise<void> {
    ...
    const items = [
      [".strata/worktrees/", "Strata Code agent worktrees"],
      [".strata/agent-manager.json", "Strata Agent Manager state"],
      [".strata/setup-script", "Strata Code worktree setup script"],
      [".strata/setup-script.sh", "Strata Code worktree setup script"],
      [".strata/setup-script.ps1", "Strata Code worktree setup script"],
      [".strata/setup-script.cmd", "Strata Code worktree setup script"],
      [".strata/setup-script.bat", "Strata Code worktree setup script"],
      [".stratacode/worktrees/", "Strata Code legacy agent worktrees"],
      [".stratacode/agent-manager.json", "Strata Agent Manager legacy state"],
      [".stratacode/setup-script", "Strata Code legacy worktree setup script"],
      [".stratacode/setup-script.sh", "Strata Code legacy worktree setup script"],
      [".stratacode/setup-script.ps1", "Strata Code legacy worktree setup script"],
      [".stratacode/setup-script.cmd", "Strata Code legacy worktree setup script"],
      [".stratacode/setup-script.bat", "Strata Code legacy worktree setup script"],
    ] as const
