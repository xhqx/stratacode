...
export async function createSessionInWorktree(provider: AgentManagerProvider, worktreePath: string, branch: string, worktreeId?: string): Promise<Session | null> {
let client: StrataClient
try {
  client = provider.connectionService.getClient()
} catch (err) {
  provider.log("createSessionInWorktree: client not available:", err)
