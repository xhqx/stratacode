...
export function adoptFollowupInWorktree(provider: AgentManagerProvider, session: Session, directory: string): void {
...
provider.registerWorktreeSession(session.id, directory)
provider.pushState()
...
provider.log(`Adopted follow-up session ${session.id} into worktree ${worktree.id}`)
}
