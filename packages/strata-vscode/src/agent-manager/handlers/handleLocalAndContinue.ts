...
export function handleLocalAndContinue(provider: AgentManagerProvider, m: AgentManagerInMessage): boolean {
if (m.type === "agentManager.openLocally") {
  provider.panel?.sessions.clearSessionDirectory(m.sessionId)
...
if (m.type === "continueInWorktree") {
  void provider.continueFromSidebar(m.sessionId, (status, detail, error) => {
    provider.panel?.postMessage({ type: "continueInWorktreeProgress", status, detail, error })
  })
  return true
}
return false
