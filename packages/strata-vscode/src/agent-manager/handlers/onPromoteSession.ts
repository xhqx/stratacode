...
export async function onPromoteSession(provider: AgentManagerProvider, sessionId: string): Promise<null> {
await provider.waitForStateReady("onPromoteSession")
const created = await provider.createWorktreeOnDisk({})
...
provider.registerWorktreeSession(sessionId, created.result.path)
provider.notifyWorktreeReady(sessionId, created.result, created.worktree.id)
provider.log(`Promoted session ${sessionId} to worktree ${created.worktree.id}`)
return null
}
