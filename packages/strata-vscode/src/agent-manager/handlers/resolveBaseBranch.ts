...
export async function resolveBaseBranch(provider: AgentManagerProvider, manager: WorktreeManager, state: WorktreeStateManager, explicit?: string): Promise<string | undefined> {
...
if (result.stale) {
  provider.clearStaleDefaultBaseBranch(state, result.stale)
}
