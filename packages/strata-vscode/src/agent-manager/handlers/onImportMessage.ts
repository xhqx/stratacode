...
export function onImportMessage(provider: AgentManagerProvider, m: AgentManagerInMessage): ChainResult {
switch (m.type) {
  case "agentManager.requestBranches":
    void provider.importer.branches()
    return handled()
  case "agentManager.requestExternalWorktrees":
    void provider.importer.external()
    return handled()
  case "agentManager.importFromBranch":
    void provider.importer.branch(m.branch)
    return handled()
  case "agentManager.importFromPR":
    void provider.importer.pr(m.url)
    return handled()
  case "agentManager.importExternalWorktree":
    void provider.importer.path(m.path, m.branch)
    return handled()
  case "agentManager.importAllExternalWorktrees":
    void provider.importer.all()
    return handled()
  default:
    return unhandled()
}
