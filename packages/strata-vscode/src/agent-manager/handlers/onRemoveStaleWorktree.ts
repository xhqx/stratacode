...
export async function onRemoveStaleWorktree(provider: AgentManagerProvider, worktreeId: string): Promise<null> {
const state = provider.getStateManager()
if (!state) return null
if (!provider.staleWorktreeIds.has(worktreeId)) {
  provider.log(`Ignored stale removal for non-stale worktree ${worktreeId}`)
  return null
}

const worktree = state.getWorktree(worktreeId)
if (!worktree) {
  provider.clearStaleTracking(worktreeId)
  provider.pushState()
  return null
}
...
for (const session of orphaned) {
  provider.panel?.sessions.clearSessionDirectory(session.id)
}
provider.clearStaleTracking(worktreeId)
provider.pushState()
provider.log(`Removed stale worktree entry ${worktreeId} (${worktree.branch})`)
return null
}
