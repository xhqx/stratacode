...
export function onSessionMessage(provider: AgentManagerProvider, m: AgentManagerInMessage, msg: Record<string, unknown>): ChainResult {
if (provider.handleLocalAndContinue(m)) return handled()
if (provider.handlePersistSession(m)) return handled()
if (provider.handleLoadClearSession(m)) return handled(msg)

if ((m.type === "sendMessage" || m.type === "sendCommand") && m.draftID && !m.sessionID) {
  provider.activeSessionId = m.draftID
  return handled(msg)
}

if (m.type === "requestTerminalContext") {
  if (m.sessionID) provider.terminalManager.showExisting(m.sessionID)
  return handled(msg)
}

if (m.type === "abort") {
  provider.host.capture("Agent Manager Session Stopped", {
    source: PLATFORM,
...
if (m.type === "agentManager.openSessions") {
  provider.connectionService.registerOpen("agent-manager", m.sessionIDs)
  return handled()
}
