...
export function openPanel(provider: AgentManagerProvider): void {
if (provider.panel) {
  provider.log("Panel already open, revealing")
  provider.panel.reveal()
  provider.postToWebview({ type: "action", action: "focusInput" })
  return
}
provider.log("Opening Agent Manager panel")
provider.host.capture("Agent Manager Opened", { source: PLATFORM })

provider.attachPanel(
  provider.host.openPanel({
    onBeforeMessage: (msg) => provider.onMessage(msg),
  }),
)
}
