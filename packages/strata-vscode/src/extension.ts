...
import { StrataConnectionService } from "./services/cli-backend"
...
import { registerCodeActions, registerTerminalActions, StrataCodeActionProvider } from "./services/code-actions"
...
export function activate(context: vscode.ExtensionContext): StrataPluginAPI {
  ...
  const connectionService = new StrataConnectionService(context)
  ...
  const tabPanels = new Map<vscode.WebviewPanel, StrataProvider>()
  ...
  const provider = new StrataProvider(context.extensionUri, connectionService, context)
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(StrataProvider.viewType, provider, {
  ...
  // commands by default.
  const skip = ["strata-code.new.agentManagerOpen", "strata-code.new.agentManager.showTerminal"]
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("strata-code.new.TabPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  for (const suffix of settingsViews) {
    context.subscriptions.push(
      vscode.window.registerWebviewPanelSerializer(`strata-code.new.${suffix}`, {
        deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("strata-code.new.SubAgentViewerPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("strata-code.new.plusButtonClicked", () => {
    ...
    vscode.commands.registerCommand("strata-code.new.cycleAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("strata-code.new.generateTerminalCommand", async () => {
  ...
  context.subscriptions.push(
    vscode.window.registerUriHandler({
      async handleUri(uri: vscode.Uri) {
        const match = uri.path.match(/^\/stratacode\/s\/([a-zA-Z0-9_-]+)$/)
        if (!match) return
...
async function openStrataInNewTab(
  ...
  const panel = vscode.window.createWebviewPanel("strata-code.new.TabPanel", EXTENSION_DISPLAY_NAME, targetCol, {
    enableScripts: true,
  ...
  panel.iconPath = {
    light: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "strata-light.svg"),
    dark: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "strata-dark.svg"),
  }
