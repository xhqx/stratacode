...
const STRATACODEIGNORE = ".stratacodeignore"
const GITIGNORE = ".gitignore"

/**
 * Patterns for sensitive environment files, applied only when no .stratacodeignore exists.
 */
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    // Try .stratacodeignore first — if it exists, use only that.
    ...
    const stratacodeignorePath = path.join(this.workspacePath, STRATACODEIGNORE)
    if (fs.existsSync(stratacodeignorePath)) {
      const stratacodeignoreContent = fs.readFileSync(stratacodeignorePath, "utf-8")
      if (stratacodeignoreContent.trim()) {
        this.ignoreInstance.add(stratacodeignoreContent)
        this.ignoreInstance.add(STRATACODEIGNORE)
        this.loadedContents.push({ file: STRATACODEIGNORE, content: stratacodeignoreContent })
        return
      }
    }
