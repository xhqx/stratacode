...
export class AutocompleteModel {
  ...
  constructor(connectionService?: StrataConnectionService) {
   ...
   */
  public setConnectionService(service: StrataConnectionService): void {
    this.connectionService = service
  }

  /**
   * Generate a FIM (Fill-in-the-Middle) completion via the CLI backend.
   * Uses the SDK's strata.fim() SSE endpoint which handles auth and streaming.
   *
  ...
  public async generateFimResponse(
    ...
    const { stream } = await client.strata.fim(
      {
        prefix,
        suffix,
