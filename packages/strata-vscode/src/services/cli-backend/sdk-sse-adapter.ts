import type { StrataClient, GlobalEvent, Event } from "@stratacode/sdk/v2/client"
...
export class SdkSSEAdapter {
  ...
  constructor(private readonly client: StrataClient) {}
  ...
  connect(): void {
    if (this.abortController) {
      console.log("[Strata New] SSE: ⚠️ Already connected, skipping")
      return
    }

    console.log("[Strata New] SSE: 🔌 connect() called")
    this.abortController = new AbortController()
    console.log('[Strata New] SSE: 🔄 Setting state to "connecting"')
    this.notifyState("connecting")
    void this.consumeLoop(this.abortController.signal).catch((err) => {
      console.error("[Strata New] SSE: Unhandled error in consumeLoop:", err)
   ...
   */
  disconnect(): void {
    console.log("[Strata New] SSE: 🔌 disconnect() called")
  ...
  reconnect(): void {
    if (!this.attemptController) {
      console.log("[Strata New] SSE: ⚠️ reconnect() called but no active attempt")
      return
    }
    console.log("[Strata New] SSE: 🔄 reconnect() — aborting current attempt")
  ...
  private async consumeLoop(signal: AbortSignal): Promise<void> {
    while (!signal.aborted) {
      ...
      try {
        console.log("[Strata New] SSE: 🎬 Calling SDK global.event()...")
        const events = await this.client.global.event({
          ...
          onSseError: (error) => {
            ...
            console.error("[Strata New] SSE: ❌ SDK SSE error callback:", error)
            this.notifyError(error instanceof Error ? error : new Error(String(error)))
          },
        })

        console.log("[Strata New] SSE: ✅ Stream opened successfully")
        ...
        for await (const event of events.stream) {
          ...
          if (type !== "server.heartbeat") {
            console.log("[Strata New] SSE: 📨 Event:", type)
          }
          this.notifyEvent(globalEvent.payload as Event, globalEvent.directory)
        }

        console.log("[Strata New] SSE: 📭 Stream ended normally")
      } catch (error) {
        ...
        if (!aborted) {
          console.error("[Strata New] SSE: ❌ Stream error:", error)
          this.notifyError(error instanceof Error ? error : new Error(String(error)))
        }
      } finally {
      ...
      console.log(`[Strata New] SSE: 🔄 Reconnecting in ${SdkSSEAdapter.RECONNECT_DELAY_MS}ms...`)
      this.notifyState("connecting")
  ...
  private resetHeartbeat(attempt: AbortController): void {
    ...
    this.heartbeatTimer = setTimeout(() => {
      console.log("[Strata New] SSE: ⏰ Heartbeat timeout — aborting stale connection")
      attempt.abort()
  ...
  private notifyEvent(event: Event, directory?: string): void {
    for (const handler of this.handlers) {
      try {
        ...
        console.error("[Strata New] SSE: Error in event handler:", error)
      }
    }
  }
  ...
  private notifyError(error: Error): void {
    for (const handler of this.errorHandlers) {
      try {
        ...
        console.error("[Strata New] SSE: Error in error handler:", err)
      }
    }
  }
  ...
  private notifyState(state: "connecting" | "connected" | "disconnected"): void {
    for (const handler of this.stateHandlers) {
      try {
        ...
        console.error("[Strata New] SSE: Error in state handler:", error)
      }
    }
  }
}
