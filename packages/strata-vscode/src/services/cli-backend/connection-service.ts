...
import { createStrataClient, type StrataClient, type Event } from "@stratacode/sdk/v2/client"
 ...
 * The network namespace is not yet in the SDK StrataClient type (pending SDK regeneration),
...
/**
 * Shared connection service that owns the single ServerManager, StrataClient (SDK), and SdkSSEAdapter.
 * Multiple StrataProvider instances subscribe to it for SSE events and state changes.
 */
export class StrataConnectionService {
  private readonly serverManager: ServerManager
  private client: StrataClient | null = null
   ...
   */
  async getClientAsync(dir?: string): Promise<StrataClient> {
  ...
  /**
   * Subscribe to notification dismiss events broadcast from any StrataProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a notification dismiss event to all subscribed StrataProvider instances.
   */
  ...
  /**
   * Subscribe to language change events broadcast from any StrataProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a language change event to all subscribed StrataProvider instances.
   */
  ...
  /**
   * Subscribe to profile change events broadcast from any StrataProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a profile change event to all subscribed StrataProvider instances.
   */
  ...
  /**
   * Subscribe to migration-complete events broadcast from any StrataProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a migration-complete event to all subscribed StrataProvider instances.
   */
  ...
  /**
   * Subscribe to favorites change events broadcast from any StrataProvider. Returns unsubscribe function.
   */
  ...
  /**
   * Broadcast a favorites change event to all subscribed StrataProvider instances.
   */
   ...
   * StrataProvider (root + worktree dirs). Used by drainPendingPrompts() to
   ...
   * directory known to any currently-mounted StrataProvider.
   *
  ...
  private async checkHealth(baseUrl: string, password: string): Promise<boolean> {
    try {
      ...
      const res = await fetch(`${baseUrl}/global/health`, {
        headers: { Authorization: `Basic ${Buffer.from(`strata:${password}`).toString("base64")}` },
  ...
  private async doConnect(workspaceDir: string): Promise<void> {
    ...
    const authHeader = `Basic ${Buffer.from(`strata:${server.password}`).toString("base64")}`
    this.client = createStrataClient({
...
async function drainSuggestions(client: StrataClient, directory: string): Promise<void> {
