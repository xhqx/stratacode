...
export class ServerManager {
  ...
  async getServer(): Promise<ServerInstance> {
    console.log("[Strata New] ServerManager: 🔍 getServer called")
    if (this.instance) {
      console.log("[Strata New] ServerManager: ♻️ Returning existing instance:", { port: this.instance.port })
      return this.instance
    }

    if (this.startupPromise) {
      console.log("[Strata New] ServerManager: ⏳ Startup already in progress, waiting...")
      return this.startupPromise
    }

    console.log("[Strata New] ServerManager: 🚀 Starting new server instance...")
    this.startupPromise = this.startServer()
    try {
      this.instance = await this.startupPromise
      console.log("[Strata New] ServerManager: ✅ Server started successfully:", { port: this.instance.port })
      return this.instance
    } finally {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    console.log("[Strata New] ServerManager: 📍 CLI path:", cliPath)
    console.log("[Strata New] ServerManager: 🔐 Generated password (length):", password.length)
    ...
    console.log("[Strata New] ServerManager: 📄 CLI isFile:", stat.isFile())
    console.log("[Strata New] ServerManager: 📄 CLI mode (octal):", (stat.mode & 0o777).toString(8))
    ...
    return new Promise((resolve, reject) => {
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        ...
        env: {
          ...
          STRATA_CLIENT: "vscode",
          STRATA_ENABLE_QUESTION_TOOL: "true",
          STRATACODE_FEATURE: "vscode-extension",
          STRATA_TELEMETRY_LEVEL: vscode.env.isTelemetryEnabled && !process.env.STRATA_DISABLE_CLOUD ? "all" : "off",
          STRATA_APP_NAME: "strata-code",
          STRATA_EDITOR_NAME: vscode.env.appName,
          STRATA_PLATFORM: "vscode",
          STRATA_MACHINE_ID: vscode.env.machineId,
          STRATA_APP_VERSION: this.context.extension.packageJSON.version,
          ...
          ...(process.env.STRATA_DISABLE_CLOUD && { STRATA_DISABLE_CLOUD: process.env.STRATA_DISABLE_CLOUD }),
        },
        stdio: ["ignore", "pipe", "pipe"],
        detached: true,
      })
      console.log("[Strata New] ServerManager: 📦 Process spawned with PID:", serverProcess.pid)
      ...
      serverProcess.stdout?.on("data", (data: Buffer) => {
        ...
        console.log("[Strata New] ServerManager: 📥 CLI Server stdout:", output)

        const port = parseServerPort(output)
        if (port !== null && !resolved) {
          resolved = true
          console.log("[Strata New] ServerManager: 🎯 Port detected:", port)
      ...
      serverProcess.stderr?.on("data", (data: Buffer) => {
        ...
        console.error("[Strata New] ServerManager: ⚠️ CLI Server stderr:", errorOutput)
        stderrLines.push(errorOutput)
      })

      serverProcess.on("error", (error) => {
        console.error("[Strata New] ServerManager: ❌ Process error:", error)
        if (!resolved) {
          reject(error)
        }
      })
      ...
      serverProcess.on("exit", (code) => {
        console.log("[Strata New] ServerManager: 🛑 Process exited with code:", code)
      ...
      setTimeout(() => {
        if (!resolved) {
          console.error(`[Strata New] ServerManager: ⏰ Server startup timeout (${STARTUP_TIMEOUT_SECONDS}s)`)
  ...
  private getCliPath(): string {
    ...
    const binName = process.platform === "win32" ? "strata.exe" : "strata"
    const cliPath = path.join(this.context.extensionPath, "bin", binName)
    console.log("[Strata New] ServerManager: 📦 Using CLI path:", cliPath)
    return cliPath
  }
  ...
  dispose(): void {
    ...
    console.log("[Strata New] ServerManager: 🔴 Disposing — sending SIGTERM to process group, PID:", proc.pid)
    ...
    const timer = setTimeout(() => {
      if (proc.exitCode === null) {
        console.warn("[Strata New] ServerManager: ⚠️ Process did not exit after SIGTERM, sending SIGKILL")
