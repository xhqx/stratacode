 ...
 *
 * Uses a full StrataProvider so the viewer has backend connectivity
...
export class SubAgentViewerProvider implements vscode.Disposable {
  ...
  private providers = new Map<string, StrataProvider>()
  ...
  openPanel(sessionID: string, title?: string): void {
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage(async (msg) => {
      ...
      try {
        ...
        console.error("[Strata New] SubAgentViewerProvider: Failed to load session:", err)
      }
    })
    ...
    panel.onDidDispose(() => {
      console.log("[Strata New] Sub-agent viewer panel disposed:", sessionID)
