...
export function handleGlobalAndServerEvents(provider: StrataProvider, event: Event): boolean {
if (event.type === "global.disposed") {
  void provider.reloadAfterAuthChange()
  return true
}
if (event.type === "server.instance.disposed") {
  ...
  if (dir && path.resolve(dir) !== path.resolve(provider.getWorkspaceDirectory())) return true
  void provider.reloadAfterAuthChange()
  return true
}
if (event.type === "global.config.updated") {
  void provider.fetchAndSendConfigUpdated()
  return true
}
return false
}
