...
import { StrataProvider, MessageLoadMode, mapAgent } from "../StrataProvider";

export async function processMessagePage(provider: StrataProvider, sessionID: string, dir: string, mode: MessageLoadMode, limit: number, before?: string, abort?: AbortController): Promise<void> {
try {
  const page = await fetchMessagePage(provider.client!, {
    sessionID,
    workspaceDir: dir,
    limit,
    before,
  ...
  // no abort controller, so provider guard prevents ghost entries.
  if (!provider.trackedSessionIds.has(sessionID)) return
  ...
  if (mode === "replace" || mode === "reconcile") provider.streams.drop(sessionID)
  if (mode === "reconcile") provider.lastReconciledAt.set(sessionID, Date.now())
  ...
  provider.postMessage({ type: "error", message: getErrorMessage(error) || "Failed to load messages", sessionID })
}
}
