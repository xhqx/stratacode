...
export async function handleDismissNotification(provider: StrataProvider, notificationId: string): Promise<void> {
if (!provider.extensionContext) return
...
if (!existing.includes(notificationId)) {
  await provider.extensionContext.globalState.update("strata.dismissedNotificationIds", [...existing, notificationId])
}
...
if (provider.cachedNotificationsMessage) {
  ...
  if (!cached.dismissedIds.includes(notificationId)) {
    provider.cachedNotificationsMessage = {
      ...cached,
...
provider.connectionService.notifyNotificationDismissed(notificationId)
}
