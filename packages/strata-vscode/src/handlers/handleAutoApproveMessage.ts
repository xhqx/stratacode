...
export function handleAutoApproveMessage(provider: StrataProvider, msg: any): void {
if (msg.type === "permissionRequest" || msg.type === "questionRequest") {
  ...
  if (config) {
    ...
    provider.scheduleAutoApproveTimer(msg, config, agentName, isQuestion)
  }
}
if (msg.type === "permissionResolved" || msg.type === "permissionError") {
  if (provider.autoApproveTimer.isTimerRunningFor(msg.permissionID)) provider.autoApproveTimer.clearTimer()
}
if (msg.type === "questionResolved" || msg.type === "questionError") {
  if (provider.autoApproveTimer.isTimerRunningFor(msg.requestID)) provider.autoApproveTimer.clearTimer()
}
}
