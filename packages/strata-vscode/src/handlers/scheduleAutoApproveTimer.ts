...
export function scheduleAutoApproveTimer(provider: StrataProvider, msg: any, config: any, agentName: string, isQuestion: boolean) {
...
provider.autoApproveTimer.startTimer(reqId, timeoutSeconds, () => {
  if (!isQuestion) {
    handlePermissionResponse(provider.permissionCtx, reqId, msg.permission.sessionID, "once", [], [], undefined, undefined)
  } else {
    const firstQ: any = msg.question.questions[0]
    const answers = firstQ?.options?.length ? [firstQ.options[0].label] : [""]
    handleQuestionReply(provider.questionCtx, reqId, [answers], msg.question.sessionID)
  }
})
}
