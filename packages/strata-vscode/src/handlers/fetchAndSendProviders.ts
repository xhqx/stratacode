...
export async function fetchAndSendProviders(provider: StrataProvider): Promise<void> {
const next = ++provider.providersGeneration
if (provider.providersRefresh) {
  provider.providersQueued = true
  await provider.providersRefresh
  return
}
const task = (async () => {
  let generation = next
  while (true) {
    provider.providersQueued = false
    const client = provider.client
    if (!client) {
      if (provider.cachedProvidersMessage && generation === provider.providersGeneration)
        provider.postMessage(provider.cachedProvidersMessage)
      return
    }
    try {
      const { response, authMethods, authStates } = await fetchProviderData(client, provider.getWorkspaceDirectory())
      if (generation !== provider.providersGeneration || client !== provider.client) {
        if (!provider.providersQueued) return
        generation = provider.providersGeneration
        continue
      }
      ...
      const message = {
        ...
        defaultSelection: computeDefaultSelection(
          provider.cachedConfigMessage as { config?: { model?: string } } | null,
      ...
      provider.cachedProvidersMessage = message
      provider.postMessage(message)
    } catch (error) {
      if (generation !== provider.providersGeneration) {
        if (!provider.providersQueued) return
        generation = provider.providersGeneration
        continue
      }
...
const done = task.finally(() => {
  if (provider.providersRefresh === done) provider.providersRefresh = null
})
provider.providersRefresh = done
await done
}
