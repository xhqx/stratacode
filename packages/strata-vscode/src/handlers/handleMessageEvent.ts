...
export function handleMessageEvent(provider: StrataProvider, event: Event, sessionID: string | undefined, directory?: string): void {
if (event.type === "indexing.status" && directory) {
  const current = path.resolve(provider.getWorkspaceDirectory(provider.currentSession?.id))
...
if (msg.type === "partUpdated") {
  provider.streams.push({ ...msg, part: provider.slimPart(msg.part) })
  return
}
if (msg.type === "indexingStatusLoaded") {
  provider.cachedIndexingStatusMessage = msg
}
if (sessionID) provider.streams.flush(sessionID)
provider.postMessage(msg)

provider.handleAutoApproveMessage(msg)
}
