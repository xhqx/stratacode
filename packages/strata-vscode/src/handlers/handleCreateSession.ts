...
export async function handleCreateSession(provider: StrataProvider): Promise<void> {
if (!provider.client) {
...
try {
  const workspaceDir = provider.getContextDirectory()
  const { data: session } = await provider.client.session.create({ directory: workspaceDir }, { throwOnError: true })
  provider.currentSession = session
  provider.contextSessionID = session.id
  provider.trackDirectory(session.id, workspaceDir)
  provider.trackedSessionIds.add(session.id)

  // Notify webview of the new session
  provider.postMessage({
    type: "sessionCreated",
    session: provider.sessionToWebview(provider.currentSession!),
  })
} catch (error) {
