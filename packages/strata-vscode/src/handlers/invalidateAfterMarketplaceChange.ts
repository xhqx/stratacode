...
export async function invalidateAfterMarketplaceChange(provider: StrataProvider, scope: "project" | "global"): Promise<void> {
...
if (scope === "global") {
  ...
  await provider.client.global.config.update({ config: {} }).catch((e: unknown) => {
...
await provider.client.instance.dispose({ directory: dir }).catch((e: unknown) => {
