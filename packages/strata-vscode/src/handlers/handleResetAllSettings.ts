...
export async function handleResetAllSettings(provider: StrataProvider): Promise<void> {
...
await provider.extensionContext?.globalState.update("variantSelections", undefined)
await provider.extensionContext?.globalState.update("recentModels", undefined)
await provider.extensionContext?.globalState.update("kanbanTasks", undefined)
await provider.extensionContext?.globalState.update("strata.dismissedNotificationIds", undefined)

// Re-send all settings to the webview so the UI reflects the reset
provider.postMessage(AutocompleteSettingsManager.getInstance().buildAutocompleteSettingsMessage())
...
await ModelState.reset(provider.client, (msg) => provider.postMessage(msg))
...
await provider.fetchAndSendNotifications()
