...
export async function gatherEditorContext(provider: StrataProvider): Promise<EditorContext> {
const workspaceDir = provider.getWorkspaceDirectory()
const controller = await provider.getIgnoreController(workspaceDir)
...
const openTabs = [...(await provider.getOpenTabPaths(workspaceDir))].slice(0, 20)
...
const { repoMap, projectMemory } = await provider.gatherRepoContext(visibleFiles)
