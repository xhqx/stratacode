...
export async function handleCompact(provider: StrataProvider, sessionID?: string, providerID?: string, modelID?: string): Promise<void> {
if (!provider.client) {
...
const target = sessionID || provider.currentSession?.id
if (!target) {
...
if (!providerID || !modelID) {
  ...
  provider.postMessage({
    type: "error",
    message: "No model selected. Connect a provider to compact provider session.",
  })
  return
}

try {
  const workspaceDir = provider.getWorkspaceDirectory(target)
  await provider.client.session.summarize(
