...
export async function handleSendMessage(provider: StrataProvider, text: string, messageID?: string, sessionID?: string, draftID?: string, providerID?: string, modelID?: string, agent?: string, variant?: string, files?: MessageFile[]): Promise<void> {
if (!provider.client) {
...
try {
  resolved = await provider.resolveSession(sessionID, draftID)
  ...
  const editorContext = await provider.gatherEditorContext()

  if (messageID) {
    provider.connectionService.recordMessageSessionId(messageID, sid)
  }

  markPending(sid)

  await runWithMessageConfirmation(provider.confirmations, messageID, "StrataProvider: Message request", () =>
    provider.withRetry(
      () =>
        provider.client!.session.promptAsync({
          sessionID: sid,
          directory: dir,
          messageID,
          parts,
