...
import { StrataProvider, MessageLoadMode } from "../StrataProvider";

export async function handleLoadMessages(provider: StrataProvider, sessionID: string, options: { mode?: MessageLoadMode; before?: string; limit?: number } = {}): Promise<void> {
const mode = options.mode ?? "replace"
if (mode !== "prepend") {
  provider.trackedSessionIds.add(sessionID)
  provider.focusSession(sessionID)
  provider.contextSessionID = sessionID
}
if (!provider.client) {
  provider.postMessage({ type: "error", message: "Not connected to CLI backend", sessionID })
  return
}
const dir = provider.getWorkspaceDirectory(sessionID)
if (mode === "focus") {
  provider.refreshSessionDetails(sessionID, dir)
  // Reconcile tail so SSE drops self-heal. Throttled to skip rapid tab-switching bursts.
  if (Date.now() - (provider.lastReconciledAt.get(sessionID) ?? 0) < 1000) return
  await provider.handleLoadMessages(sessionID, { mode: "reconcile", limit: options.limit ?? MESSAGE_PAGE_LIMIT })
  return
}
...
if (abort) {
  provider.loadMessagesAbort?.abort()
  provider.loadMessagesAbort = abort
  provider.refreshSessionDetails(sessionID, dir, abort.signal)
}
await provider.processMessagePage(sessionID, dir, mode, options.limit ?? MESSAGE_PAGE_LIMIT, options.before, abort)
}
