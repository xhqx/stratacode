...
export async function fetchAndSendAgents(provider: StrataProvider): Promise<void> {
if (!provider.client) {
  if (provider.cachedAgentsMessage) {
    provider.postMessage(provider.cachedAgentsMessage)
  }
  return
}
...
try {
  ...
  const { data: agents } = await retry(() =>
    provider.client!.app.agents({ directory: workspaceDir }, { throwOnError: true }),
  )
  ...
  provider.cachedAgentsMessage = message
