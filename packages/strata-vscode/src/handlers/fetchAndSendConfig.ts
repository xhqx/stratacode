...
export async function fetchAndSendConfig(provider: StrataProvider): Promise<void> {
if (!provider.client || provider.connectionState !== "connected") {
  if (provider.cachedConfigMessage) {
    provider.postMessage(provider.cachedConfigMessage)
  }
  return
}
...
try {
  ...
  const { data: config } = await retry(() =>
    provider.client!.config.get({ directory: workspaceDir }, { throwOnError: true }),
  )
  ...
  provider.cachedConfigMessage = message
