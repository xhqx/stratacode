...
export async function fetchAndSendIndexingStatus(provider: StrataProvider): Promise<void> {
if (!provider.client) {
  if (provider.cachedIndexingStatusMessage) {
    provider.postMessage(provider.cachedIndexingStatusMessage)
  }
  return
}

const config = provider.connectionService.getServerConfig()
if (!config) return

try {
  const dir = provider.getWorkspaceDirectory(provider.currentSession?.id)
  ...
  provider.cachedIndexingStatusMessage = message
