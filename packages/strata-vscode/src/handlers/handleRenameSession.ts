...
export async function handleRenameSession(provider: StrataProvider, sessionID: string, title: string): Promise<void> {
if (!provider.client) {
...
try {
  ...
  const { data: updated } = await provider.client.session.update(
    { sessionID, directory: workspaceDir, title },
    { throwOnError: true },
  )
  if (provider.currentSession?.id === sessionID) {
    provider.currentSession = updated
  }
  provider.postMessage({ type: "sessionUpdated", session: provider.sessionToWebview(updated) })
} catch (error) {
