...
export async function fetchAndSendMcpStatus(provider: StrataProvider): Promise<void> {
if (!provider.client) {
  if (provider.cachedMcpStatusMessage) {
    provider.postMessage(provider.cachedMcpStatusMessage)
  }
  return
}

try {
  const directory = provider.getWorkspaceDirectory()
  const { data } = await retry(() => provider.client!.mcp.status({ directory }))
  if (data) {
    const message = { type: "mcpStatusLoaded", status: data }
    provider.cachedMcpStatusMessage = message
