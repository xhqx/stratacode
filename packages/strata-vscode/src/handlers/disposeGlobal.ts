...
export async function disposeGlobal(provider: StrataProvider): Promise<void> {
if (!provider.client) return

await provider.client.global
  .dispose()
  .catch((e: unknown) => Logger.warn("StrataProvider", "global.dispose() after org switch failed:", e))

// Org switch succeeded — refresh profile and providers independently (best-effort)
try {
  const profileResult = await provider.client!.strata.profile()
  // Broadcast to all webviews (sidebar, profile tab, agent manager, etc.)
  provider.connectionService.notifyProfileChanged(profileResult.data ?? null)
} catch (error) {
