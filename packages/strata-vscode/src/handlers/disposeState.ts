...
export function disposeState(provider: StrataProvider): void {
provider.loadMessagesAbort?.abort()
provider.loadMessagesAbort = null
for (const controller of provider.retryAbortControllers.values()) controller.abort()
provider.retryAbortControllers.clear()
provider.confirmations.clear()
provider.followupListeners.length = 0
provider.streams.dispose()
provider.isWebviewReady = false
provider.promptRecoveryQueued = false
clearNetworkWaits(provider.trackedSessionIds)
provider.trackedSessionIds.clear()
provider.syncedChildSessions.clear()
provider.sessionDirectories.clear()
provider.sessionStatusMap.clear()
provider.lastReconciledAt.clear()
}
