...
export function handleChildSessionEvent(provider: StrataProvider, event: Event, sessionID?: string): void {
if (event.type === "message.part.updated") {
  ...
  if (childId && !provider.trackedSessionIds.has(childId)) {
    Logger.info("StrataProvider", "🔗 Auto-adopting child session from task tool", { childId })
    void provider.handleSyncSession(childId, part.sessionID ?? sessionID)
  }
}
}
