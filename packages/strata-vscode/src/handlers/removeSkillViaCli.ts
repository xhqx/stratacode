...
export async function removeSkillViaCli(provider: StrataProvider, location: string): Promise<boolean> {
if (!provider.client) return false
try {
  const dir = provider.getWorkspaceDirectory()
  const result = await provider.client.stratacode.removeSkill({ location, directory: dir })
  if (result.error) {
    Logger.error("StrataProvider", "removeSkill returned error:", result.error)
    provider.cachedSkillsMessage = null
    provider.clearCommandsCache()
    await Promise.all([provider.fetchAndSendSkills(), provider.fetchAndSendCommands()])
    return false
  }
} catch (error) {
  Logger.error("StrataProvider", "Failed to remove skill:", error)
  provider.cachedSkillsMessage = null
  provider.cachedCommandsMessage = null
  await Promise.all([provider.fetchAndSendSkills(), provider.fetchAndSendCommands()])
  return false
}
provider.cachedSkillsMessage = null
