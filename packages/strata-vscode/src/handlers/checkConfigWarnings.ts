...
export async function checkConfigWarnings(provider: StrataProvider, from: string): Promise<void> {
if (provider.configWarningsShown) {
...
try {
  ...
  const result = await provider.client.config.warnings({ directory: dir })
  const list = result?.data ?? []
  Logger.info("StrataProvider", "config warnings fetched", { from, count: list.length })
  if (list.length === 0) return
  provider.configWarningsShown = true
