...
export function refreshSessionDetails(provider: StrataProvider, sessionID: string, dir: string, signal?: AbortSignal): void {
if (!provider.client) return
provider.client.session
  .get({ sessionID, directory: dir })
  .then((r) => {
    if (r.data && !signal?.aborted) {
      provider.currentSession = r.data
      provider.contextSessionID = r.data.id
    }
  })
  .catch((e: unknown) => Logger.warn("StrataProvider", "getSession failed (non-critical):", e))
provider.postMessage({ type: "workspaceDirectoryChanged", directory: provider.getWorkspaceDirectory(sessionID) })
provider.client.session
  .status({ directory: dir })
  .then((r) => {
    if (!r.data || signal?.aborted) return
    for (const [sid, info] of Object.entries(r.data) as [string, SessionStatus][]) {
      if (!provider.trackedSessionIds.has(sid)) continue
