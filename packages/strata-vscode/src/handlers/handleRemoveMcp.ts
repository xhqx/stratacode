...
export async function handleRemoveMcp(provider: StrataProvider, name: string): Promise<void> {
...
await provider.removeLegacyMcp(name)

// Remove from global config via CLI (preserves JSONC comments)
try {
  await provider.client!.global.config.update({ config: { mcp: { [name]: null } } as any }, { throwOnError: true })
} catch (err) {
...
provider.cachedAgentsMessage = null
provider.cachedConfigMessage = null
const dir = provider.getWorkspaceDirectory()
try {
  await provider.client!.instance.dispose({ directory: dir })
} catch (err) {
  Logger.debug("StrataProvider", "instance dispose after MCP removal:", err)
}
await Promise.all([provider.fetchAndSendAgents(), provider.fetchAndSendConfig(), provider.fetchAndSendSkills()])
}
