...
export async function handleSendCommand(provider: StrataProvider, command: string, args: string, messageID?: string, sessionID?: string, draftID?: string, providerID?: string, modelID?: string, agent?: string, variant?: string, files?: MessageFile[]): Promise<void> {
if (!provider.client) {
...
try {
  ...
  if (messageID) {
    provider.connectionService.recordMessageSessionId(messageID, resolved!.sid)
  }
  ...
  await runWithMessageConfirmation(provider.confirmations, messageID, "StrataProvider: Command request", () =>
    provider.withRetry(
      () =>
        provider.client!.session.command({
          sessionID: sid,
          directory: dir,
          command,
          arguments: args,
