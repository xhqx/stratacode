...
export function handleEvent(provider: StrataProvider, event: Event, directory?: string): void {
if (event.type === "strata-sessions.remote-status-changed") {
  provider.remoteService?.updateFromEvent({ enabled: event.properties.enabled, connected: event.properties.connected })
  return
}

if (provider.handleWorkerEvent(event)) return

if (event.type === "message.updated") {
  provider.confirmations.confirm(event.properties.info.id)
}

if (event.type === "session.status") {
  provider.handleSessionStatusEvent(event, event.properties.sessionID)
  return
}

if (event.type === "session.created" && provider.adoptPendingFollowup(event.properties.info)) {
  return
}

const sessionID = provider.connectionService.resolveEventSessionId(event)

if (provider.isEventDropped(event, sessionID)) return

if (provider.handleGlobalAndServerEvents(event)) return

provider.handleSessionLifecycleEvents(event)
provider.handleChildSessionEvent(event, sessionID)
provider.handlePromptEvents(event)

handleNetworkEvent(event.type as string, event.properties as any, provider.client, (s) => provider.getWorkspaceDirectory(s))
provider.handleMessageEvent(event, sessionID, directory)
}
