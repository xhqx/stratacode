...
export async function fetchAndSendCommands(provider: StrataProvider): Promise<void> {
if (!provider.client) {
  if (provider.cachedCommandsMessage) {
    provider.postMessage(provider.cachedCommandsMessage)
  }
  return
}
...
try {
  ...
  const message = await loadCommands(provider.client, dir)

  provider.cachedCommandsMessage = message
