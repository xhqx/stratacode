...
export function adoptPendingFollowup(provider: StrataProvider, session: Session) {
const now = Date.now()
const match = provider.matchesPendingFollowup(session)
if (!match) {
  if (
    provider.pendingFollowup &&
    !matchFollowup({ pending: provider.pendingFollowup, dir: provider.pendingFollowup.dir, now })
  ) {
...
provider.pendingFollowup = null
provider.trackDirectory(session.id, session.directory)
for (const cb of provider.followupListeners) cb(session, session.directory)
provider.registerSession(session)
void provider.handleLoadMessages(session.id)
return true
}
