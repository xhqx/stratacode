...
export async function fetchAndSendSkills(provider: StrataProvider): Promise<void> {
if (!provider.client) {
  if (provider.cachedSkillsMessage) {
    provider.postMessage(provider.cachedSkillsMessage)
  }
  return
}
...
try {
  ...
  const { data: skills } = await retry(() =>
    provider.client!.app.skills({ directory: workspaceDir }, { throwOnError: true }),
  )
  ...
  provider.cachedSkillsMessage = message
