...
export async function handleDeleteSession(provider: StrataProvider, sessionID: string): Promise<void> {
if (!provider.client) {
  provider.postMessage({ type: "error", message: "Not connected to CLI backend" })
  return
}

try {
  const workspaceDir = provider.getWorkspaceDirectory(sessionID)
  await provider.client.session.delete({ sessionID, directory: workspaceDir }, { throwOnError: true })
  provider.trackedSessionIds.delete(sessionID)
  provider.streams.drop(sessionID)
  provider.syncedChildSessions.delete(sessionID)
  provider.sessionDirectories.delete(sessionID)
  provider.lastReconciledAt.delete(sessionID)
  provider.connectionService.pruneSession(sessionID)
  if (provider.currentSession?.id === sessionID) {
    provider.currentSession = null
    provider.focusSession(undefined)
  }
  provider.postMessage({ type: "sessionDeleted", sessionID })
} catch (error) {
