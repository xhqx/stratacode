...
export async function handleAbort(provider: StrataProvider, sessionID?: string): Promise<void> {
if (!provider.client) {
  return
}

const targetSessionID = sessionID || provider.currentSession?.id
if (!targetSessionID) {
  return
}
...
try {
  await abortSession({
    ...
    dir: provider.getWorkspaceDirectory(targetSessionID),
  })
} catch (error) {
