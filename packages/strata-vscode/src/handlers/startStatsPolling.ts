...
export function startStatsPolling(provider: StrataProvider): void {
provider.statsPoller?.stop()
provider.statsGitOps?.dispose()
const git = new GitOps({ log: () => {} })
provider.statsGitOps = git
provider.statsPoller = new GitStatsPoller({
  getWorktrees: () => [],
  ...
  onLocalStats: (stats: LocalStats) => {
    ...
    provider.cachedStats = msg
    provider.postMessage(msg)
  },
  log: () => {},
  hiddenIntervalMs: 60000,
})
provider.statsPoller.setEnabled(true)
provider.statsPoller.setVisible(true)
}
