...
export async function handleProviderAction(provider: StrataProvider, msg: Record<string, unknown>): Promise<void> {
...
const set = (m: unknown) => {
  provider.cachedConfigMessage = m
}
...
if (msg.type === "disconnectProvider") return disconnectProviderAction(ctx, rid, pid, provider.cachedConfigMessage, set)
if (msg.type === "saveCustomProvider" && config)
  return saveCustomProviderAction(ctx, rid, pid, config, key, keyChanged, provider.cachedConfigMessage, set)
}
