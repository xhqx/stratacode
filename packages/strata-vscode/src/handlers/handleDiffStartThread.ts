...
export async function handleDiffStartThread(provider: StrataProvider, threadId: string, file: string, line: number, endLine: number | undefined, text: string, side?: "left" | "right"): Promise<void> {
try {
  ...
  const res = await client.session.prompt(
    {
      sessionID: provider.diffExplainSession,
      directory: root,
      agent: "explainer",
