...
export async function resolveSession(provider: StrataProvider, sessionID?: string, draftID?: string): Promise<{ sid: string; dir: string } | undefined> {
if (!provider.client) return undefined

const dir = sessionID ? provider.getWorkspaceDirectory(sessionID) : provider.getContextDirectory()

if (!sessionID && !provider.currentSession) {
  const { data: session } = await provider.client.session.create({ directory: dir }, { throwOnError: true })
  provider.currentSession = session
  provider.contextSessionID = session.id
  provider.trackDirectory(session.id, dir)
  provider.trackedSessionIds.add(session.id)
  if (draftID) provider.contextSessionID = session.id
  provider.postMessage({
    type: "sessionCreated",
    session: provider.sessionToWebview(session),
    draftID,
  })
}

const sid = sessionID || provider.currentSession?.id
if (!sid) throw new Error("No session available")
provider.trackedSessionIds.add(sid)
return { sid, dir }
}
