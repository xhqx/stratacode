import type { Session, SessionStatus } from "@stratacode/sdk/v2/client"
...
export async function handleForkSession(ctx: ForkContext, sessionId: string, messageId?: string): Promise<void> {
  const status =
    ...
    (await Promise.resolve()
      ...
      .catch((e) => {
        console.error("[Strata New] refreshForkStatus failed:", e)
  ...
  await forkSession(
    {
      ...
      registerSession: () => {},
      log: (...args) => console.log("[Strata New] StrataProvider:", ...args),
    },
    sessionId,
    undefined,
    messageId,
