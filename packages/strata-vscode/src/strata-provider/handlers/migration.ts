/**
 * Legacy migration handlers — extracted from StrataProvider.
 *
 * Manages the migration wizard for users upgrading from Strata Code v5.x.
...
export async function checkAndShowMigrationWizard(ctx: MigrationContext): Promise<void> {
  ...
  console.log("[Strata New] StrataProvider: 🔄 Legacy data detected, showing migration wizard")
  ctx.postMessage({
    type: "migrationState",
    needed: true,
    data: {
...
export async function handleStartLegacyMigration(
  ...
  try {
    ...
    console.error("[Strata New] StrataProvider: ❌ Migration failed", error)
    ctx.postMessage({
