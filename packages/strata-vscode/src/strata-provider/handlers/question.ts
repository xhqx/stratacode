/**
 * Question handlers — extracted from StrataProvider.
 *
...
export async function fetchAndSendPendingQuestions(ctx: QuestionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Strata New] StrataProvider: Failed to fetch pending questions:", error)
  }
}
...
export async function handleQuestionReply(
  ...
  try {
    ...
    console.error("[Strata New] StrataProvider: Failed to reply to question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
...
export async function handleQuestionReject(
  ...
  try {
    ...
    console.error("[Strata New] StrataProvider: Failed to reject question:", error)
    ctx.postMessage({ type: "questionError", requestID })
    return false
  }
}
