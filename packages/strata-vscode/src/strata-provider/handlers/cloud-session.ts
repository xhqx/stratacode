/**
 * Cloud session handlers — extracted from StrataProvider.
 *
...
import type { StrataClient, Session, TextPartInput, FilePartInput } from "@stratacode/sdk/v2/client"
import type { CloudSessionData, EditorContext } from "../../services/cli-backend/types"
import { getErrorMessage, sessionToWebview, mapCloudSessionMessageToWebviewMessage } from "../../strata-provider-utils"
...
export async function handleRequestCloudSessions(
  ...
  try {
    const result = await ctx.client.strata.cloudSessions({
    ...
    console.error("[Strata New] StrataProvider: Failed to fetch cloud sessions:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleRequestCloudSessionData(ctx: CloudSessionContext, sessionId: string): Promise<void> {
  ...
  try {
    const result = await ctx.client.strata.cloud.session.get({ id: sessionId })
    ...
    console.error("[Strata New] Failed to load cloud session data:", err)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId: sessionId,
...
export async function handleImportAndSend(
  ...
  try {
    const result = await ctx.client.strata.cloud.session.import({
      sessionId: cloudSessionId,
      directory: dir,
    })
    session = result.data as Session | undefined
  } catch (error) {
    console.error("[Strata New] StrataProvider: ❌ Cloud session import failed:", error)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId,
  ...
  try {
    ...
    console.error("[Strata New] Failed to send message after cloud import:", err)
    ctx.postMessage({
