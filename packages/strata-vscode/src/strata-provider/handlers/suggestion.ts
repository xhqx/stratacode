/**
 * Suggestion handlers — extracted from StrataProvider.
 *
 * Manages suggestion accept and dismiss flows plus recovery after SSE reconnects.
 * No vscode dependency.
 */

import type { StrataClient, SuggestionRequest } from "@stratacode/sdk/v2/client"
...
export async function handleSuggestionAccept(
  ...
  try {
    ...
    console.error("[Strata New] StrataProvider: Failed to accept suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function handleSuggestionDismiss(
  ...
  try {
    ...
    console.error("[Strata New] StrataProvider: Failed to dismiss suggestion:", error)
    ctx.postMessage({ type: "suggestionError", requestID })
  }
}
...
export async function fetchAndSendPendingSuggestions(ctx: SuggestionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[Strata New] StrataProvider: Failed to fetch pending suggestions:", error)
  }
}
