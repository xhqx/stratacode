 ...
 * user to press Enter; `strata run` auto-retries with backoff. This module implements auto-reply
 ...
 */
export function handleNetworkEvent(type: string, props: Props, client: StrataClient | null, dir: GetDir) {
  ...
  if (type === "session.network.restored" && props.requestID) {
    ...
    if (!entry) return
    console.log("[Strata New] network: auto-replying to restore", props.requestID)
