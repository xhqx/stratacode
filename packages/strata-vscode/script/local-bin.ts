...
/**
 * Ensures the VS Code extension has a CLI binary at `packages/strata-vscode/bin/strata`.
 *
 * Strategy:
 * 1) If `bin/strata` already exists -> ok.
 ...
 * 4) Copy the resulting binary into `packages/strata-vscode/bin/strata` and chmod +x.
 *
 * This script is intended to be run from `packages/strata-vscode` as part of build/package.
 */
...
async function findStrataBinaryInOpencodeDist(): Promise<string | null> {
  ...
  const preferred = join(distDir, `@stratacode`, tag, "bin", binName)
  try {
    statSync(preferred)
    return preferred
  } catch {
  ...
  // Fallback: find any dist/**/bin/strata or strata.exe
  const queue = [distDir]
  while (queue.length) {
    const dir = queue.pop()
    if (!dir) continue
    ...
    for (const e of entries) {
      ...
      if (e.isFile() && (e.name === "strata" || e.name === "strata.exe") && basename(dirname(p)) === "bin") {
        return p
      }
    }
  }
...
async function ensureBuiltBinary(): Promise<string> {
  const found = await findStrataBinaryInOpencodeDist()
  if (found) return found

  log(
    `No prebuilt binary found under ${relative(strataVscodeDir, join(opencodeDir, "dist"))} - attempting build via bun.`,
  )
  ...
  const built = await findStrataBinaryInOpencodeDist()
  if (!built) {
    throw new Error(
      `CLI build completed but no binary was found in ${join(opencodeDir, "dist")} (expected dist/**/bin/strata).`,
    )
  }
  return built
}
...
async function main() {
  ...
  if (exists && !rebuild) {
    ...
    log(
      `CLI binary already present at ${relative(strataVscodeDir, targetBinPath)} (${Math.round(st.size / 1024 / 1024)}MB). Use --force to rebuild.`,
    )
    return
  }
  ...
  if (exists && rebuild) {
    ...
    if (existsSync(distDir)) {
      ...
      log(`Removed ${relative(strataVscodeDir, distDir)} to force rebuild.`)
    }
  }
  ...
  log(`Copied CLI binary from ${relative(packagesDir, sourceBinPath)} -> ${relative(strataVscodeDir, targetBinPath)}`)
}
