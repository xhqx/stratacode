...
Strata Code is an open source AI coding agent platform. It ships as multiple products that all build on the same backend. This package (`packages/strata-vscode/`) is the **VS Code extension** — one of several clients.

### Products and How They Relate

All products are thin clients over the **CLI** (`packages/opencode/`, published as `@stratacode/cli`). The CLI is a fork of upstream [OpenCode](https://github.com/anomalyco/opencode) with Strata-specific additions (gateway auth, telemetry, migration, code review, branding). It contains the full AI agent runtime, tool execution, session management, provider integrations (500+ models), and an HTTP API server.

Every client spawns or connects to a `strata serve` process and communicates via HTTP REST + SSE using the auto-generated `@stratacode/sdk`.

```
                        @stratacode/cli  (packages/opencode/)
                     ┌────────────────────────────────┐
...
| `packages/strata-docs/`      | `@stratacode/strata-docs`      | Documentation site (Next.js + Markdoc)                                                                                       |

### Upstream OpenCode Packages (not Strata-specific)
...
The extension bundles its own CLI binary at `bin/strata` — it does NOT use a system-installed CLI. To build it:
...
The script checks for a prebuilt binary in `packages/opencode/dist/`, builds the CLI if needed, and copies it to `bin/strata`.

## Architecture

### Extension ↔ CLI Backend

The extension is a client of the CLI. At startup it spawns `bin/strata serve --port 0`, captures the dynamically-assigned port from stdout, and communicates over HTTP + SSE. A random password is generated and passed via `STRATA_SERVER_PASSWORD` env var for basic auth.

```
Extension (Node.js)                          CLI Backend (child process)
┌──────────────────────────┐                ┌──────────────────────┐
│ StrataConnectionService    │── HTTP/SSE ──> │ strata serve --port 0  │
│   ├── ServerManager      │                │   Hono REST API      │
│   ├── HttpClient         │                │   SSE event stream   │
│   └── SSEClient          │                │   Session management │
│                          │                │   AI agent runtime   │
│ StrataProvider (sidebar)   │                └──────────────────────┘
│ StrataProvider (agent mgr) │
│ StrataProvider (open tabs) │
└──────────────────────────┘
```

- **`StrataConnectionService`** (`src/services/cli-backend/connection-service.ts`) is a singleton shared across all webviews. It owns the server process, HTTP client, and SSE connection.
- **`ServerManager`** (`src/services/cli-backend/server-manager.ts`) spawns the CLI binary and manages the process lifecycle.
- Multiple **`StrataProvider`** instances (sidebar, Agent Manager, "open in tab" panels) subscribe to the shared connection. SSE events are filtered per-webview via a `trackedSessionIds` Set.
...
- CSP requires nonce for scripts and `font-src` for bundled fonts — see [`StrataProvider.ts`](src/StrataProvider.ts:777)
- HTML root has `data-theme="strata-vscode"` to activate strata-ui's VS Code theme bridge
...
3. **StrataProvider** (`src/StrataProvider.ts`): Add a `fetchAndSend*()` method using the cached message pattern, and handle the corresponding `request*` message from the webview in `handleWebviewMessage()`
...
- **Cached messages** (e.g. `cachedProvidersMessage`, `cachedAgentsMessage` in StrataProvider): Ensures webview refreshes get data immediately without waiting for a new HTTP round-trip
...
All Agent Manager sessions share the **single `strata serve` process** managed by `StrataConnectionService`. No separate server is spawned per session. Session isolation comes from directory scoping — worktree sessions pass the worktree path to the CLI backend, which creates a session scoped to that directory.
...
## Webview UI (strata-ui)

New webview features must use **`@stratacode/strata-ui`** components instead of raw HTML elements with inline styles. This is a Solid.js component library built on `@kobalte/core`.

- Import via deep subpaths: `import { Button } from "@stratacode/strata-ui/button"`
...
- Global styles imported via `import "@stratacode/strata-ui/styles"` in [`index.tsx`](webview-ui/src/index.tsx:2)
- [`chat.css`](webview-ui/src/styles/chat.css) is being progressively migrated — when replacing a component with strata-ui, remove the corresponding CSS rules from it
- New CSS for components not yet in strata-ui goes into `chat.css` grouped by comment-delimited sections (`/* Component Name */`). Once a strata-ui equivalent exists, remove the section.
- **Check the desktop app first**: [`packages/app/src/`](../../packages/app/src/) is the reference implementation for how strata-ui components are composed together. Always check how the app uses a component before implementing it in the webview — don't just look at the component API in isolation.
- **`data-component` and `data-slot` attributes carry CSS styling** — strata-ui uses `[data-component]` and `[data-slot]` attribute selectors, not class names. When the app uses e.g. `data-component="permission-prompt"` and `data-slot="permission-actions"`, these get strata-ui styling for free.
- **Prefer strata-ui styles**: Always reuse existing strata-ui CSS variables, tokens, and component styles instead of writing custom CSS. If a style doesn't exist in strata-ui yet, add it there and reuse it rather than inlining or duplicating styles in the webview.
- **Icons**: strata-ui has 75+ custom SVG icons in [`packages/ui/src/components/icon.tsx`](../../packages/ui/src/components/icon.tsx). To list all available icon names: `node -e "const c=require('fs').readFileSync('../../packages/ui/src/components/icon.tsx','utf8');[...c.matchAll(/^\\s{2}[\"']?([\\w-]+)[\"']?:\\s*\x60/gm)].map(m=>m[1]).sort().forEach(n=>console.log(n))"`. Icon names use both hyphenated (`arrow-left`) and bare-word (`brain`, `console`, `providers`) keys.
...
Generated screenshot baselines live under `packages/strata-docs/public/img/screenshot-tests/` and are referenced from docs as `/docs/img/screenshot-tests/...`. If a generated VS Code visual-regression screenshot is used in docs, add the docs usage to the `DOCS` map in `tests/visual-regression.spec.ts` and keep `tests/visual-regression.spec.mts` in sync while that file exists.
...
- All debug output must be prepended with `[Strata New]` for easy filtering

## Naming Conventions

- All VSCode commands must use `strata-code.new.` prefix (not `strata-code.`)
- All view IDs must use `strata-code.new.` prefix, **except** the sidebar view which uses `strata-code.SidebarProvider` to preserve user sidebar position when upgrading from the legacy extension

## Stratacode Change Markers

This package is entirely Strata-specific — `stratacode_change` markers are NOT needed in any files under `packages/strata-vscode/`. The markers are only necessary when modifying shared upstream opencode files.
...
The `spawn` wrapper covers long-lived processes (e.g. `strata serve`). The `exec` wrapper covers short commands (e.g. `git`, `tar`). If you need the raw callback form of `execFile` for some reason, pass `windowsHide: true` explicitly in the options object.
