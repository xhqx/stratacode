...
import type { ClawStatus, ChatMessage, StrataClawOutMessage } from "../lib/types"
...
export function ClawProvider(props: { children: JSX.Element }) {
  ...
  const handler = (event: MessageEvent) => {
    const msg = event.data as StrataClawOutMessage
    if (!msg?.type?.startsWith("strataclaw.")) return

    switch (msg.type) {
      case "strataclaw.state": {
        const s = msg.state
        setPhase(s.phase)
        setLocale(s.locale)
        if (s.phase === "ready") {
      ...
      case "strataclaw.message":
        ...
        break
      case "strataclaw.messageUpdated": {
        setMessages((prev) => {
          const idx = prev.findIndex((m) => m.id === msg.message.id)
          if (idx === -1) {
            const next = [...prev, msg.message]
      ...
      case "strataclaw.presence":
        setOnline(msg.online)
        break
      case "strataclaw.status":
        setStatus(msg.data)
        break
      case "strataclaw.locale":
        setLocale(msg.locale)
        break
      case "strataclaw.error":
  ...
  onMount(() => {
    vscode.postMessage({ type: "strataclaw.ready" })
  })
  ...
  const ctx: ClawCtx = {
    ...
    connected,
    error,
    send: (text: string) => vscode.postMessage({ type: "strataclaw.send", text }),
    openExternal: (url: string) => vscode.postMessage({ type: "strataclaw.openExternal", url }),
    retry: () => vscode.postMessage({ type: "strataclaw.ready" }),
  }
