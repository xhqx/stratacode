...
export function convertToMentionPath(path: string, cwd: string): string {
  ...
  try {
    ...
    console.error("[Strata New] Failed to decode dropped URI:", err, cleaned)
  }
 ...
 */
export const STRATA_FILE_PATH_MIME = "application/x-strata-file-path"

/**
 * Extract file paths from a drop's DataTransfer.
 * Checks (in order):
 * 1. Internal relative-path drag (application/x-strata-file-path)
...
export function extractDropPaths(dt: DataTransfer): string[] | null {
  ...
  const strata = dt.getData(STRATA_FILE_PATH_MIME)
  if (strata) {
    const paths = strata.split(/\r?\n/).filter((line) => line.trim() !== "")
