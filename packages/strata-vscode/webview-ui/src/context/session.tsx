...
export const SessionProvider: ParentComponent = (props) => {
  ...
  function handleExtensionMessage(message: ExtensionMessage): void {
    ...
    switch (message.type) {
      ...
      case "cloudSessionImportFailed":
        ...
        console.error("[Strata New] Cloud session import failed:", message.error)
        break
  ...
  function handlePartUpdated(
    ...
    if (!effectiveMessageID) {
      console.warn("[Strata New] Part updated without messageID:", part.id, part.type)
      return
    }
  ...
  function sendMessage(
    ...
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot send message: not connected")
      return
    }
  ...
  function sendCommand(
    ...
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot send command: not connected")
      return
    }
  ...
  function abort() {
    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[Strata New] Cannot abort: no current session")
      return
    }
  ...
  function compact() {
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot compact: not connected")
      return
    }

    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[Strata New] Cannot compact: no current session")
      return
    }
  ...
  function createSession() {
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot create session: not connected")
      return
    }
  ...
  function loadSessions() {
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot load sessions: not connected")
      return
    }
  ...
  function selectSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot select session: not connected")
      return
    }
    if (id.startsWith("cloud:")) {
      console.warn("[Strata New] Cannot select cloud preview session via selectSession")
      return
    }
  ...
  function selectCloudSession(cloudSessionId: string) {
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot select cloud session: not connected")
      return
    }
    const key = `cloud:${cloudSessionId}`
  ...
  function deleteSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot delete session: not connected")
      return
    }
  ...
  function renameSession(id: string, title: string) {
    if (!server.isConnected()) {
      console.warn("[Strata New] Cannot rename session: not connected")
      return
    }
