...
const STORAGE_KEY = "strata.prompt-history.v1"
...
function load(): string[] {
  try {
    ...
    console.warn("[Strata New] prompt history load failed", err)
    return []
  }
}
...
function save(items: string[]) {
  try {
    ...
    console.warn("[Strata New] prompt history save failed", err)
  }
}
