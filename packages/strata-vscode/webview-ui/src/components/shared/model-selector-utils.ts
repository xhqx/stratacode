...
import {
  STRATA_PROVIDER_ID as STRATA_GATEWAY_ID,
  PROVIDER_PRIORITY as PROVIDER_ORDER,
  providerOrderIndex,
} from "../../../../src/shared/provider-model"

export { STRATA_GATEWAY_ID, PROVIDER_ORDER }

export const STRATA_AUTO_SMALL_IDS = new Set(["strata-auto/small", "auto-small"])

export function isSmall(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return model.providerID === STRATA_GATEWAY_ID && STRATA_AUTO_SMALL_IDS.has(model.id)
}
...
export function stripSubProviderPrefix(name: string): string {
  ...
  if (prefix.toLowerCase() === STRATA_GATEWAY_ID) return name
...
export function buildTriggerLabel(
  ...
  if (resolvedName) {
    if (providerID === STRATA_GATEWAY_ID) return stripSubProviderPrefix(resolvedName)
    if (providerName) return `${providerName} / ${resolvedName}`
    return resolvedName
  }
  if (raw?.providerID && raw?.modelID) {
    return raw.providerID === STRATA_GATEWAY_ID ? raw.modelID : `${raw.providerID} / ${raw.modelID}`
  }
