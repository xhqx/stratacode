...
export function PlanningTaskDialog(props: Props) {
  ...
  const [startAt, setStartAt] = createSignal(props.task?.startAt ? new Date(props.task.startAt).toISOString().slice(0, 16) : "")
    ...
    <Dialog title={isEdit ? language.t("planning.editTask") : language.t("planning.addTask")} fit>
      <div style={{ "max-width": "500px", "max-height": "90vh", "overflow-y": "auto", padding: "16px" }}>
        ...
        <div style={{ display: "flex", "flex-direction": "column", gap: "12px", "padding": "16px 0" }}>
          <div>
            <label style={{ "font-size": "0.9em", "font-weight": 600, display: "block", "margin-bottom": "4px" }}>Title *</label>
            <TextField value={title()} onInput={(e) => setTitle(e.currentTarget.value)} placeholder="E.g. Setup Database" autofocus />
          </div>

          <div>
            <label style={{ "font-size": "0.9em", "font-weight": 600, display: "block", "margin-bottom": "4px" }}>Prompt *</label>
            <textarea 
              value={prompt()} 
              onInput={(e) => setPrompt(e.currentTarget.value)} 
              placeholder="Instructions for the agent..." 
              style={{ width: "100%", "min-height": "80px", "background": "var(--vscode-input-background)", color: "var(--vscode-input-foreground)", border: "1px solid var(--vscode-input-border)", padding: "4px" }}
            />
          </div>

          <div>
            <label style={{ "font-size": "0.9em", "font-weight": 600, display: "block", "margin-bottom": "4px" }}>Description</label>
            <TextField value={description()} onInput={(e) => setDescription(e.currentTarget.value)} />
          </div>

          <div style={{ display: "flex", gap: "12px" }}>
            <div style={{ flex: 1 }}>
              <label style={{ "font-size": "0.9em", "font-weight": 600, display: "block", "margin-bottom": "4px" }}>{language.t("planning.startAt")}</label>
              <input 
                type="datetime-local" 
                value={startAt()} 
                onInput={(e) => setStartAt(e.currentTarget.value)}
                style={{ width: "100%", "background": "var(--vscode-input-background)", color: "var(--vscode-input-foreground)", border: "1px solid var(--vscode-input-border)", padding: "4px" }}
              />
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ "font-size": "0.9em", "font-weight": 600, display: "block", "margin-bottom": "4px" }}>{language.t("planning.duration")}</label>
              <TextField type="number" value={duration()} onInput={(e) => setDuration(e.currentTarget.value)} placeholder="minutes" />
            </div>
          </div>

          <div>
            <label style={{ "font-size": "0.9em", "font-weight": 600, display: "block", "margin-bottom": "4px" }}>{language.t("planning.priority")}</label>
            <select 
              value={priority()} 
          ...
          <Show when={availableTasks().length > 0}>
            <div>
              <label style={{ "font-size": "0.9em", "font-weight": 600, display: "block", "margin-bottom": "4px" }}>{language.t("planning.dependsOn")}</label>
              <div style={{ "max-height": "100px", "overflow-y": "auto", border: "1px solid var(--vscode-input-border)", padding: "4px" }}>
        ...
        <div style={{ display: "flex", "justify-content": "flex-end", gap: "8px", "margin-top": "16px" }}>
          <Button variant="secondary" onClick={props.onClose}>{language.t("kanban.cancel")}</Button>
