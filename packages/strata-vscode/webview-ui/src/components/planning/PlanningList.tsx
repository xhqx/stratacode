...
export function PlanningList() {
  ...
  const priorityColor = (priority: number) => {
    switch (priority) {
      case 1: return "var(--vscode-charts-red)"
      case 2: return "var(--vscode-charts-orange)"
      case 3: return "var(--vscode-charts-yellow)"
      case 4: return "var(--vscode-charts-blue)"
      case 5: return "var(--vscode-charts-green)"
  ...
  const statusColor = (status: string) => {
    switch (status) {
      case "planned": return "var(--vscode-foreground)"
      case "ready": return "var(--vscode-charts-blue)"
      case "dispatched": return "var(--vscode-charts-orange)"
      case "needs_review": return "var(--vscode-charts-purple)"
          ...
          <Card style={{ padding: "12px", display: "flex", "flex-direction": "column", gap: "8px" }}>
            <div style={{ display: "flex", "justify-content": "space-between", "align-items": "flex-start" }}>
              ...
              <div style={{ display: "flex", gap: "4px" }}>
                <Show when={task.status === "planned" || task.status === "ready"}>
                  ...
                  <IconButton icon="play" size="small" variant="ghost" onClick={() => planning.dispatch(task.id)} title={language.t("planning.dispatch")} />
                </Show>
                <Show when={task.status === "needs_review"}>
                  <IconButton icon="check" size="small" variant="ghost" onClick={() => planning.confirm(task.id)} title={language.t("planning.confirm")} />
                </Show>
                <Show when={task.status === "failed"}>
                  <IconButton icon={"refresh" as any} size="small" variant="ghost" onClick={() => planning.dispatch(task.id)} title={language.t("planning.retry")} />
                </Show>
            ...
            <div style={{ display: "flex", gap: "6px", "flex-wrap": "wrap" }}>
              ...
              <Show when={task.startAt}>
                <Tag><Icon name={"calendar" as any} size="small"/> {new Date(task.startAt!).toLocaleString()}</Tag>
              </Show>
              <Show when={task.duration}>
                <Tag><Icon name={"watch" as any} size="small"/> {task.duration}m</Tag>
              </Show>
              <Show when={task.dependsOn && task.dependsOn.length > 0}>
                <Tag><Icon name="link" size="small"/> {task.dependsOn?.length} deps</Tag>
              </Show>
            </div>
          </Card>
        )}
