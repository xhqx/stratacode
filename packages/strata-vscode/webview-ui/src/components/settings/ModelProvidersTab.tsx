...
import { disabledProviderOptions, providersWithStrataFallback, visibleConnectedIds } from "./provider-visibility"
import { STRATA_PROVIDER_ID, CUSTOM_PROVIDER_PACKAGE } from "../../../../src/shared/provider-model"
import { createProviderAction } from "../../utils/provider-action"

type ProviderSource = "env" | "api" | "config" | "custom"
type ProviderOption = { value: string; label: string }

const ModelProvidersTab: Component = () => {
  ...
  const popularProviders = createMemo(() => {
    ...
    return sortProviders(
      all.filter(
        (item) =>
          item.id !== STRATA_PROVIDER_ID &&
          !item.id.startsWith("acp-") &&
  ...
  return (
    <div>
      ...
      {/* Connected providers (excluding Strata) */}
