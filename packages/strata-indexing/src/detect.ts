export const INDEXING_PLUGIN_NAMES = ["strata-indexing", "@stratacode/strata-indexing"] as const
...
function isPathSpecifier(value: string): boolean {
  ...
  if (normalized.includes("/.opencode/") || normalized.includes("/.strata/") || normalized.includes("/.stratacode/")) {
    return true
  }
...
function normalizePath(value: string): string {
  ...
  const scoped = parts.findIndex((part, i) => part === "@stratacode" && parts[i + 1] === "strata-indexing")
  if (scoped >= 0) return "@stratacode/strata-indexing"

  const workspace = parts.findIndex((part, i) => part === "packages" && parts[i + 1] === "strata-indexing")
  if (workspace >= 0) return "@stratacode/strata-indexing"
