...
describe("loadIgnore", () => {
  ...
  test("loads root .stratacodeignore in addition to root .gitignore", async () => {
    await writeFile(path.join(root, ".gitignore"), "dist/\n")
    await writeFile(path.join(root, ".stratacodeignore"), "secret/\n")
  ...
  test("preserves existing .gitignore-only behavior when .stratacodeignore is absent", async () => {
  ...
  test("ignores the ignore files themselves", async () => {
    ...
    expect(ig.ignores(".stratacodeignore")).toBe(true)
  })
})
