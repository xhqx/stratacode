...
const reg = process.env.REGISTRY ?? "ghcr.io/strata-org" // stratacode_change
...
const setup = async () => {
  ...
  if (list.includes("strata")) {
    await $`docker buildx use strata`
    return
  }
  await $`docker buildx create --name strata --use`
