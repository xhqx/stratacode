...
description: Write Storybook stories and visual regression tests for the Strata VS Code extension webview UI
---

Use this skill when the user asks you to add visual regression tests, screenshot tests, or Storybook stories for components in `packages/strata-vscode/`.
...
Stories live in `packages/strata-vscode/webview-ui/src/stories/`. Existing files and their scope:
...
Snapshots live at `packages/strata-vscode/tests/visual-regression.spec.ts-snapshots/`:
...
Key settings in `packages/strata-vscode/playwright.config.ts`:
...
- `packages/util/**`
- `packages/sdk/js/**`
- `packages/strata-vscode/webview-ui/**`
- `packages/strata-vscode/.storybook/**`
- `packages/strata-vscode/tests/visual-regression*`
...
strata-ui components are imported via deep subpaths:

```tsx
import { Part } from "@stratacode/strata-ui/message-part"
import { BasicTool } from "@stratacode/strata-ui/basic-tool"
...
```tsx
import type { AssistantMessage as SDKAssistantMessage, TextPart, ToolPart } from "@stratacode/sdk/v2"
...
```
globals=colorScheme:dark;theme:strata-vscode;vscodeTheme:dark-modern
```

The `.storybook/preview.tsx` applies these via a decorator that calls `applyVscodeTheme()` / `applyStrataTheme()` from strata-ui. Stories do NOT need to handle theming — it happens automatically.
