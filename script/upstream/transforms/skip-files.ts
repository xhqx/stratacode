 ...
 *
 * These are files that exist in upstream but should NOT exist in Strata fork.
 ...
 * - Kept deleted if they don't exist in Strata
 */
...
export async function skipFiles(options: SkipOptions = {}): Promise<SkipResult[]> {
  ...
  for (const file of allFiles) {
    ...
    // Check if file existed in Strata before merge (HEAD~1 or the merge base)
    const existedInStrata = await fileExistsInRef(file, "HEAD")

    if (existedInStrata) {
      debug(`Skipping ${file} - exists in Strata, not removing`)
      results.push({ file, action: "skipped", dryRun: options.dryRun ?? false })
      continue
    }

    // File doesn't exist in Strata - should be removed
    if (options.dryRun) {
