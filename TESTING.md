...
How to spin up the **local main-branch** Strata backend and test it with `curl` / `fetch`. Aimed at a running Strata CLI agent iterating on backend fixes without rebuilding the VS Code extension, TUI, or desktop app.
...
STRATA_SERVER_PASSWORD="$PASS" bun dev serve --port 0 >/tmp/strata-serve.log 2>&1 &
echo $! >/tmp/strata-serve.pid
while ! grep -q "strata server listening" /tmp/strata-serve.log 2>/dev/null; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/strata-serve.log | grep -oE "[0-9]+$")
AUTH="Authorization: Basic $(printf 'strata:%s' "$PASS" | base64 | tr -d '\n')"
BASE="http://127.0.0.1:$PORT"

# Call any endpoint
curl -sS -H "$AUTH" -H "x-strata-directory: $PWD" "$BASE/global/health"

# Stop when done
kill "$(cat /tmp/strata-serve.pid)" 2>/dev/null || true
rm -f /tmp/strata-serve.pid /tmp/strata-serve.log
```

## 1. What this doc is for

Testing local backend fixes against a real running server, talking to it over HTTP the same way the VS Code extension, TUI, and `strata run --attach` do — but without any of those clients. Every request is a `curl` the agent can copy-paste.

For in-process tests (no socket, fastest loop) see `packages/opencode/test/stratacode/server/permission-allow-everything.test.ts` for the `Server.Default().app.request(...)` pattern. That's the right tool inside the `packages/opencode/` test suite; this doc is for out-of-process HTTP testing.
...
Do **not** use `createStrataServer()` from `@stratacode/sdk/v2` to test local code: it spawns the PATH `strata` binary (`packages/sdk/js/src/v2/server.ts:38-136`), which is the wrong tool here.
...
```bash
STRATA_SERVER_PASSWORD=$(openssl rand -hex 16) \
  bun dev serve --port 0 >/tmp/strata-serve.log 2>&1 &
echo $! >/tmp/strata-serve.pid
while ! grep -q "strata server listening" /tmp/strata-serve.log; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/strata-serve.log | grep -oE "[0-9]+$")
```
...
```bash
STRATA_SERVER_PASSWORD=secret \
  bun dev serve --port 4096 --hostname 127.0.0.1 \
  >/tmp/strata-serve.log 2>&1 &
echo $! >/tmp/strata-serve.pid
while ! grep -q "strata server listening" /tmp/strata-serve.log; do sleep 0.1; done
PORT=4096
```
...
Omit `STRATA_SERVER_PASSWORD` entirely. The server prints `Warning: STRATA_SERVER_PASSWORD is not set; server is unsecured.` and the auth middleware is bypassed — fine for throwaway local testing, never for anything else.
...
### Auth header (only if `STRATA_SERVER_PASSWORD` was set)

```bash
AUTH="Authorization: Basic $(printf 'strata:%s' "$STRATA_SERVER_PASSWORD" | base64 | tr -d '\n')"
```

The username is literally `strata` (same as the VS Code extension). Skip this whole block if you launched without a password.

### Directory header on every call

```bash
DIR_HEADER="x-strata-directory: $PWD"
```
...
```bash
PORT="$PORT" STRATA_SERVER_PASSWORD="$STRATA_SERVER_PASSWORD" bun /tmp/probe.ts
rm /tmp/probe.ts
```

Reminder: this script **connects to** the server you launched in Section 3 — it does not start one. `createStrataServer()` from the SDK would spawn the PATH `strata` binary (production CLI), which defeats the point of testing local code.

## 9. Pitfalls

- Running `strata serve` instead of `bun dev serve` runs the installed prod binary, not your edits.
- Missing `x-strata-directory` (or `?directory=`) returns `400` from `InstanceMiddleware`.
...
- `STRATA_SERVER_PASSWORD` must be set **before** launch — changing it after doesn't rotate credentials.
- When sharing `/tmp/strata-serve.log` / `.pid` across terminals, unique-suffix the paths to avoid clobbering parallel runs.
